...
struct Trie {
    ...
    void insert(ll x) {
        ...
        for(int i=ln; i>=0; --i) {
            int bit=1&(x>>i);
            if(!it->ch[bit]) it->ch[bit]=new TrieNode();
            it=it->ch[bit]; it->cnt++;
        }
    }
    ll query(ll x) {
        ...
        for(int i=ln; i>=0; --i) {
            ...
            int bx = 1 & (x>>i);
            int bk = 1 & (k>>i);
            // Every xor combination==0 works (since going big->small), then check ==1
            if(bk == 1) {
                if(it->ch[bx]) res+=it->ch[bx]->cnt;
                it=it->ch[1-bx];
            }
            // Must have combination==0
            else {
                it=it->ch[bx];
            }
        }
...
void solve() {
    ...
    vi nums(n);
    f(i, n) cin>>nums[i];
    f(i, n-1) nums[i+1] ^= nums[i];  // nums = prefix
    Trie r;
    r.insert(0);
    ll res=0;
    f(i, n) {
