...
struct PersSegtree {
    ...
    // Usage: set roots[i] = build(0, n-1) ... and then call add/update/query from that scope of logic
...
// Persistent segment tree - treat the compressed value of the coin values as tree indices while original array indices -> prefix queries on the node array
int32_t main() {
    setIO();
    cin>>n>>k;
    vi a(n); read(a);
