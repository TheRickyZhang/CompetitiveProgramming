...
void solve() {
    int rr; cin>>n>>m>>rr;
    // Hurdles = {first safe, distance needed}, Powerups = {pos, value}
    vpii h(n), p(m);
    f(i, n) {
        int a, b; cin>>a>>b;
        h[i] = {a-1, b-a+2};
    ...
    sort(all(p)); sort(all(h));
    pq<int> boosts; // Greedily take the largest boost available only when needed
    int res = 0;
    int curr = 1; // Current jump value;
    int i=0, j=0;
    while(i < n) {
        if(j == m || h[i].ff < p[j].ff) {
            while(curr < h[i].ss) {
                if(boosts.empty()) {
                    cout<<-1<<en; return;
                }
                curr += boosts.top(); boosts.pop();
                res++;
            }
            i++;
        } else {
