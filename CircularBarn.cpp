...
int32_t main() {
    ...
    // Cost starting from i, consisting of j rooms
    vvi cost(2*n, vi(n+1, 0));
    f(i, n) {
        fe(j, n) {
            cost[i][j] = cost[i+n][j] = cost[i][j-1] + a[i+j-1]*(j-1);
        }
    }
    int res = INFL;
    vvi dp(n+1, vi(k, 0));
    f(rot, n) {
        f(i, n) dp[i][0] = cost[rot][i+1];
        // Divide and Conquer DP (Binary Search DP)
        function<void(int, int, int, int, int)> dpp = [&](int k, int l, int r, int a, int b) {
            if(l > r) return;
            int m = (l+r)/2;
            int best=INFL, pos=-1;
            rep(i, a, min(b, m)) {
                // We are iterating over placing new door at i
                // And covering up to room mid = length mid-i+1
                int x = dp[i-1][k-1] + cost[rot+i][m-i+1];
                if(x < best) {
                    best = x, pos = i;
                } else if(x==best) {
                    pos = i;
            ...
            dp[m][k] = best;
            if(l==r) return;
            // Note that while m/m+1, we have pos in both cases
            dpp(k, l, m, a, pos);
            dpp(k, m+1, r, pos, b);
        };
        rep(i, 1, k-1) {
