...
int32_t main() {
    ...
    cin>>n>>MOD;
    // This only works if MOD is prime
    // vi fact(N+1, 1);
    // fe(i, N) fact[i] = (fact[i-1] * i) % MOD;
    // vi inv(N+1);
    // inv[N] = fastPow(fact[N], MOD-2, MOD);
    // repr(i, N-1, 0) inv[i] = (inv[i+1] * (i+1)) % MOD;
    ...
    //     int res = 0;
    //     rep(i, 0, x) {
    //         res = (res + (i % 2 == 0 ? 1 : -1) * fact[x] * inv[i] + MOD*MOD) % MOD;
    //     }
    //     cout<<res<<en;
    // }
    int x = 1;
