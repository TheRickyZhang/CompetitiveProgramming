...
int32_t main() {
    ...
    fvii dfs = [&](int u, int p) {
        ...
        for(int v : adj[u]) {
            ...
            if(tin[v] == -1) {
                ...
                dfs(v, u);
                d[u] += d[v];
            } else if(tin[v] < tin[u]) { // This is important, undirected -> see edges twice -> only want back edge
                res.pb({u, v});
                d[u]++; d[v]--;
            }
        }
    ...
    fe(i, n-1) { // Note we don't include root since nothing will get above it
        if(d[i] == 0) {
            cout<<0<<en; return 0;
        }
    }
