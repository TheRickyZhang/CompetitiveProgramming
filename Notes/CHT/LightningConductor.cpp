...
v<double> solve(vi& a) {
    v<double> res(n);
    deque<pii> s; // Contains {index, last index it's the best}
    s.emplace_back(0, 0);
    res[0] = a[0];
    fe(i, n-1) {
        while(!s.empty()) {
            auto& [j, bj] = s.back();
            if (a[j] + rt[i-j] < a[i]) { bj = i-1; }
            else {
                // This can be optimized with an O(1) check
                bj = lstTrue(i, n-1, [&](int k) {
                    return a[i] + rt[k-i] <= a[j] + rt[k-j];
                });
            }
            int sz = s.size();
            if(sz > 1 && s[sz-2].ss >= bj) s.pop_back();
            else break;
        }
        // May always be bad, but will pop it lazily before answering any queries
        s.emplace_back(i, n);
        // We can do this after the f(i, n) loop, but cleaner to do it here
        while(!s.empty() && s[0].ss < i) s.pop_front();
        auto [j, bj] = s[0];
        res[i] = a[j] + rt[i-j];
    }
    return res;
...
int32_t main() {
    ...
    reverse(all(r));
    reverse(all(a));
    f(i, n) cout<<ceil(max(l[i], r[i])) - a[i]<<en;
}