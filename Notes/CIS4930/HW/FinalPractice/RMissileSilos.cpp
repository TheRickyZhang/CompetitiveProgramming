...
int main() {
    ...
    f(i, m) {
        ...
        if(dist[u] < k && dist[u]+w > k) {
            x=k-dist[u];
            y=w-x;
            if(dist[v]+y >= k) res++; // Path from u must be shorter than from v
            if(dist[v]+y == k) continue; // Equidistant = multiplicity 1
        }
        if(dist[v] < k && dist[v]+w > k) {
            x=k-dist[v];
            y=w-x;
            if(dist[u]+y >= k) res++; // Other way might also be valid
        }
    }
    cout<<res<<en;
}