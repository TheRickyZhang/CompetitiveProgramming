...
int32_t main() {
    ...
    // CHANGED: Instead of map<pii,int>, store frequency by (x,y) and subset size
    // mp[{x,y}][k] = number of subsets of size k summing to (x,y) in first half
    ...
    for(int mask = 0; mask < (1 << m); mask++) {
        int x = 0, y = 0;
        int popcnt = 0; // track how many elements included
        ...
        // Ensure mp[{x,y}] has size m+1 before incrementing
        mp[{x,y}].resize(m + 1);
        mp[{x,y}][popcnt]++;
    }

    // Store final answers for each subset size from 0..n
    ...
    for(int mask = 0; mask < (1 << r); mask++) {
        int x = 0, y = 0;
        int popcnt = 0; // track subset size in second half
        ...
        // Look up complement (a - x, b - y)
        auto need = make_pair(a - x, b - y);
        if(mp.count(need)) {
            const auto &v = mp[need];
            // Combine subset sizes: first-half size + second-half size
            for(int s = 0; s <= m; s++) {
                res[s + popcnt] += v[s];
            }
        }
    ...
    // Print results for each subset size i = 1..n
    for(int i = 1; i <= n; i++) {
        cout << res[i] << "\n";
    }
}
