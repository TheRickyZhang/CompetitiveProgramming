...
tpl_<tn_ T> ostream& operator<<(ostream& os,const BIT<T>& bit){os<<"BIT:\n";for(int i=1,maxRows=20;i<=bit.n && i<=maxRows;++i)os<<string(__builtin_ctz(i)*2,' ')<<"i"<<i<<": "<<bit.t[i]<<" ([ "<<(i-(i&-i))<<","<<(i-1)<<"])\n";return os;}
...
void solve() {
    ...
    suf[n-2] = (s[n-1]=='0' ? -1 : 1);
    repr(i, n-3, 0) {
        suf[i] = (s[i+1]=='0' ? -1 : 1) + suf[i+1];
    }
    sort(rall(suf));
    f(i, n-2) suf[i+1] += suf[i];

    int sz = suf.size();
    int res = fstTrue(0LL, sz-1, [&](int m) {
        return suf[m] >= k;
    });
    if(res == sz) cout<<-1<<en;
    else cout<<res+2<<en;
