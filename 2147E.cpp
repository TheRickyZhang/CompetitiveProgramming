...
//   int tot = count_if(all(cnt), [](int x) { return x > 0;});
//   vi need(b, 0);
//   // Cost of updating the zero at num i, index j to 1 while preserving all other bits
//   auto cost = [&](int i, int j) {
//     int x = 0;
//     f(k, j) {
//       if(bits[i][k] > 0 && cnt[k] - bits[i][k] > 0) {
//         x += (1LL<<k);
//       }
//     }
//     return (1LL<<j) - x;
...
//       }
//     }
//     // if(j < 8) cout<<"best for "<<j<<" is "<<bestpos<<endl;
//     update(bestpos, j);
//     dp[tot + pos + 1] = dp[tot + pos] + best;
//   }
//   // cout<<"dp"<<dp<<endl;
//
//   vpii queries(k);
...
//   vi ans(k);
//   int j = 0;
//   for(auto [x, anspos] : queries) {
//      while(j+1 < b && x >= dp[j+1]) j++;
//      ans[anspos] = j;
//   }
//   cout<<ans<<endl;
// }
...
signed main()
{
    ...
    while (t--) {
        ...
        for (int k = 0; k < BB; ++k) if (!((calcor(a, n) >> k)&1ll)) {
            ll curops = 0;
