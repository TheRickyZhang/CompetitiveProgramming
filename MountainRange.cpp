...
tpl_<tn_ T> struct Segtree { int n; v<T> t, nums; T z; fn<T(T, T)> c;
    ...
    Segtree(int sz, T zero, fn<T(T, T)> combine, const v<T>& init = {}) : n(sz), t(4 * sz, zero), nums(sz, zero), z(zero), c(std::move(combine)) { if (!init.empty()) { nums = init; build(1, 0, n-1); } }
...
int32_t main() {
    ...
    repr(i, n-1, 0) {
        ...
        r[i] = (s.empty() ? n : s.top().ss);
        s.push({a[i], i});
    }
    vi res(n, 1);
    Segtree<int> tree(n, 0, [&](int x, int y) {
        return max(x, y);
    });
    vpii b(n);
    f(i, n) b[i] = {a[i], i};
    sort(all(b));
    for(auto [x, i] : b) {
        int mx = max(tree.query(l[i]+1, i-1), tree.query(i+1, r[i]-1));
        res[i] = mx+1;
        tree.update(i, mx+1);
    }
