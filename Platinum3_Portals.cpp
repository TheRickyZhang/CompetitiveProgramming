...
//     }
//     auto min_at = [&](pii x) {
//         return min(b[x.ff], b[x.ss]);
//     };
//     sort(all(pos), [&](pii x, pii y) {
//         return min_at(x) > min_at(y);
//     });
//     sort(all(lines), [&](pii x, pii y) {
//         return x.ss > y.ss;
//     });
//     v<fr> res(n, fr(0, 0));
//     // CHT cht;
...
//             auto [a, b] = lines[j];
//             int sz = h.size();
//             int meet = lstTrue(0LL, sz-2, [&](int m) {
//                 Line l1 = h[m], l2 = h[m+1];
//                 return (b-l1.b) * (l1.a-l2.a) <= (l2.b-l1.b) * (l1.a-a);
//             });
//             res[pos[i].ff] = reduce(fr(b-h[meet].b, h[meet].a-a));
//             i++;
//         }
//     }
//     f(i, n) cout<<pos[i].ff<<" / "<<pos[i].ss<<sp;
// }
int32_t main(){
    setIO("falling");
    int n; cin>>n;
    vi b(n); f(i,n) cin>>b[i];
    vi q(n); f(i,n) cind>>q[i];

    struct W{ int y,d,id; }; // y=A[i], d=-(i+1), id=i
    v<W> lines(n), worlds(n);
    f(i,n){ lines[i]={b[i],-(i+1),i}; worlds[i]=lines[i]; }
    sort(lines.begin(), lines.end(), [&](const W& x,const W& y){ return x.y<y.y; }); // asc A

    auto ev=[&](const W& u,const W& v)->pii{ return { llabs(u.y-v.y), llabs(u.d-v.d) }; }; // (num,den)
    auto cmp=[&](const W& a,const W& b,const W& c)->int{
        pii L=ev(a,c), R=ev(b,c);
        __int128 l=(__int128)L.ff*R.ss, r=(__int128)R.ff*L.ss;
        if(l<r) return -1; if(l>r) return 1; return 0;
    };
    auto bad=[&](const W& x,const W& y,const W& z)->bool{ // cut(x,y) >= cut(y,z) ?
        // (y.y-x.y)/(x.d-y.d) >= (z.y-y.y)/(y.d-z.d)
        __int128 L=(__int128)(y.y-x.y)*(y.d-z.d);
        __int128 R=(__int128)(z.y-y.y)*(x.d-y.d);
        return L>=R;
    };
    ...
    // pass 1: lower hull (A[q] < A[i])
    for(int i=n-1;i>=0;--i){
        W w=lines[i];
        while(!cur.empty()){
            if(w.d < cur.back().d){ cur.pop_back(); continue; }           // enforce nondecreasing slopes
            if(cur.size()>1 && bad(w,cur.back(),cur[cur.size()-2])){ cur.pop_back(); continue; }
            break;
        }
        cur.pb(w);
        int u=lines[i].id, v=q[u];
        if(b[u]>b[v]){
            if(cur[0].d > -(v+1)){ num[u]=-1; continue; }
            int l=0, r=(int)cur.size()-1;
            while(l<r){
                int m=(l+r)>>1;
                int LOK = (cur[m+1].d > -(v+1)); // target slope left of next cut guard
                if(LOK){ r=m; continue; }
                int s=cmp(cur[m],cur[m+1], worlds[v]);
                if(s<0) r=m;
                else if(s==0) l=r=m;
                else l=m+1;
            }
            pii got=ev(cur[l], worlds[v]); int g=std::gcd(got.ff, got.ss);
            num[u]=got.ff/g; den[u]=got.ss/g;
        }
    }

    // pass 2: upper hull (A[q] > A[i])
    cur.clear();
    f(i,n){
        W w=lines[i];
        while(!cur.empty()){
            if(w.d > cur.back().d){ cur.pop_back(); continue; }           // enforce nonincreasing slopes
            if(cur.size()>1 && bad(w,cur.back(),cur[cur.size()-2])){ cur.pop_back(); continue; }
            break;
        }
        cur.pb(w);
    ...
    f(i,n){
        if(num[i]<0) cout<<-1<<en;
        else cout<<num[i]<<'/'<<den[i]<<en;
    }
    return 0;
}
