...
int32_t main(){
    ...
    int n; cin>>n;
    vi a(n+1), S(n+1), ans(n+1);
    fe(i,n){ cin>>a[i]; S[i]=S[i-1]+a[i]; }

    deque<int> dq;
    dq.pb(0);
    auto better = [&](int j1,int j2,int i){
        return (S[i]-S[j1])*(i-j2) >= (S[i]-S[j2])*(i-j1);
    };
    auto obsolete = [&](int j1,int j2,int i){
        return (S[j2]-S[j1])*(i-j2) >= (S[i]-S[j2])*(j2-j1);
    };
    ...
    fe(i,n){
        ...
        if(m<2) j = dq[0];
            ...
            int lo=0, hi=m-2, idx=m-1;
            while(lo<=hi){
                int mid=(lo+hi)/2;
                if(better(dq[mid], dq[mid+1], i)){
                    idx=mid; hi=mid-1;
                } else lo=mid+1;
            }
            j = dq[idx];
        ...
        while(dq.size()>1){
            ...
            if(obsolete(j1,j2,i)) dq.pop_back();
            else break;
        }
        dq.pb(i);
    }

    fe(i,n) cout<<ans[i]<<(i==n?'\n':' ');
    return 0;
}