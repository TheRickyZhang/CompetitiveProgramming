...
void solve() {
    ...
    // Don't do binary search here, since bounded by n, and not necessarily monotonic
    // Function to check if a deck size s is possible
    auto good = [&](ll m) -> bool {
        ll decks = (tot+k) / m;
        if(decks * m < tot || decks < mx) return false;
        return true;
    };

    int res = lstTrue(1LL, n, good);
    cout<<res<<en;
}
