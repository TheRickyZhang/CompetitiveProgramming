...
int32_t main() {
  ...
  // cout<<"x"<<sp<<x<<en;
  // cout<<a<<en;
  // cout<<"dist"<<sp<<dist<<en;

  double res = 0;
  while(x > 0) {
    auto [av, i] = q.top(); q.pop();
    if(used[i]) continue;
    if(av * cnt[i] * dist[i] > x) {
      res += x/av;
      cout<<res<<en;
      return 0;
    }
    // cout<<"subtracting "<<av*cnt[i]*dist[i]<<en;
    x -= av*cnt[i]*dist[i];
    // cout<<"x"<<sp<<x<<en;
    res += cnt[i]*dist[i];
    used[i] = true;
    double nav = (av * cnt[i] + weight[i+1]) / (cnt[i]+1);
    cnt[i+1] = cnt[i] + 1;
    q.push({nav, i+1});
  } 
  // cout<<"huh"<<en;
