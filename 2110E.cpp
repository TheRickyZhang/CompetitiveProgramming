...
void solve() {
    ...
    f(i, n) {
        int x, y; cin>>x>>y;
        a[i] = {x, y};
        s.insert(x); t.insert(y);
    }
    map<int, int> mpa, mpb;
    int it = 0;
    for(int x : s) mpa[x] = it++;
    for(int x : t) mpb[x] = it++;
    map<pii, int> mp;
    it = 0;
    for(auto& [x, y] : a) {
        x = mpa[x], y = mpb[y];
        mp[{min(x, y), max(x, y)}] = it++;
    }
    ...
    vi odd;
    f(i, m) {
        if(adj[i].size() & 1) odd.pb(i);
    }
    if(odd.size() > 2) quit("NO");
