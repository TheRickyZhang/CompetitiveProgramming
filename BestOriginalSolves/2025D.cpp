...
int32_t main() {
    ...
    int cnt = 1;
    vi aa(m+1, 0), bb(m+1, 0);
    vi curr(m+1, 0);
    f(i, n) {
        if(a[i]==0) {
            repr(i, cnt-1, 1) bb[i-1] += bb[i];
            f(i, cnt-1) aa[i+1] += aa[i];
            // cout<<"accum "<<en;; f(i, cnt) cout<<aa[i]<<sp;
            // cout<<en; f(i, cnt) cout<<bb[i]<<sp;
            // cout<<en;
            f(i, cnt) curr[i] += aa[i] + bb[i];
            f(i, cnt) aa[i] = bb[i] = 0;

            curr[cnt] = curr[cnt-1];
            repr(i, cnt-1, 1) {
                curr[i] = max(curr[i], curr[i-1]);
            }
            // cout<<"   "; f(i, cnt) cout<<curr[i]<<sp;
            // cout<<en;
            cnt++;
        } else {
            int x = abs(a[i]); if(x >= cnt) continue;
            if(a[i] > 0) aa[x]++;
            else bb[cnt-x-1]++;
        }
    }
