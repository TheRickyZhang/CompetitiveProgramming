...
void solve() {
    ...
    auto bfs = [&](vvi& adj, vi& dist) {
        queue<pii> q;
        q.push({0, 0});
        while(!q.empty()) {
            auto [u, x] = q.front(); q.pop();
            if(x >= dist[u]) continue; dist[u]=x;
            for(int v : adj[u]) {
                q.push({v, x+1});
            }
        }
    ...
    vi dist(n, INFL); bfs(adj, dist);
    // cout<<dist<<en;

    vvi res(n, vi(2, INFL));
    f(i, n) res[i][0] = dist[i];
    vb vis(n, false);
    // cout<<res<<en;
    function<void(int)> dfs = [&](int u) {
        ...
        for(int v : adj[u]) {
            if(dist[v] > dist[u]) {
                dfs(v);
                ckmn(res[u][0], res[v][0]);
                ckmn(res[u][1], res[v][1]);
            } else {
                ckmn(res[u][1], res[v][0]);
            }
        }
    };
    dfs(0);
    f(i, n) cout<<min(res[i][0], res[i][1])<<sp;
    cout<<en;
}
