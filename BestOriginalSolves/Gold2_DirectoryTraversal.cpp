...
#define setIO(name) static ifstream cin(name".in"); static ofstream cout(name".out");
...
int32_t main() {
    ...
    f(i, n) {
        ...
        if(t==0) {
            weights[i] = s.size();
            sz[i] = 1; // Only files have size 1 to start
        } else {
            weights[i] = s.size() + 1;
    ...
    autotree dfs2 = [&](int u, int p) {
        ckmn(res, down[u] + dp[u]);
        for(auto [v, w] : adj[u]) {
            if(v==p) continue;
            // Propagate, add previous downs excluding this one, add cost of going up
            dp[v] = dp[u] + (down[u] - (down[v] + w*sz[v])) + 3*(sz[0]-sz[v]);
            dfs2(v, u);
        }
    };
    dfs2(0, -1);
