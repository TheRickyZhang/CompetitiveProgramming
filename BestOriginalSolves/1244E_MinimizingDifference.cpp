...
int32_t main() {
    ...
    vi a(n); read(a);
    sort(all(a));
    int res = fstTrue(0LL, a[n-1]-a[0], [&](int m) {
        int l=0, r=n-1;
        int kk = k;
        while(true) {
            ...
            if(l >= r) return true;
            // There has to be a cleaner way to check this
            if((l+1) < (n-r)) { // Less expensive to move the left
                if(kk - (l+1) * (a[l+1] - a[l]) < 0) {
                    // cout<<"l"<<m<<sp<<l<<sp<<r<<en;
                    return a[r] - a[l] - kk/(l+1) <= m;
                }
                kk -= (l+1) * (a[l+1] - a[l]);
                l++;
            } else {
                if(kk - (n-r) * (a[r] - a[r-1]) < 0) {
                    // cout<<"r"<<m<<sp<<l<<sp<<r<<en;
                    return a[r] - a[l] - kk/(n-r) <= m;
                }
                kk -= (n-r) * (a[r] - a[r-1]);
                r--;
            }
