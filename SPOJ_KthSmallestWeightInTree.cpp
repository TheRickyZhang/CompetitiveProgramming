...
int getLCA(const vvi& up,const vi& dep, int u, int v, const vi& tin = {}, const vi& tout = {}) {
    if(!tin.empty() && !tout.empty()) {if(tin[u] <= tin[v] && tout[v] <= tout[u]) return u; if(tin[v]<=tin[u] && tout[u]<=tout[v]) return v;}
...
struct PersSegtree {
      ...
      : n(n), c(std::move(c)), z(z)
    {
        ...
        nodes(z, 0, 0); // To avoid -1 checks we can set 0 as a sentinel. Only fails if we want lazy tags as well.xx
    }
...
// Using Persistent Segment Tree on a Tree problem
int32_t main() {
    setIO();
    int n, q; cin>>n>>q;
    vi a(n); read(a); // weights
    vi b = a; sort(all(b));
    ...
    PersSegtree tree(m, ad, 0);
    vi roots(n, 0);
    roots[0] = 0;
    vi tin(n), tout(n), par(n, 0), dep(n, 0); // Note that we want par[0] = 0 to avoid repetitive -1 checks
    int t = 0;
    fn<void(int, int, int)> dfs = [&](int u, int p, int d) {
        roots[u] = tree.add(roots[p], a[u], 1);
        par[u] = p, dep[u] = d;
        tin[u] = t++;
        for(int v : adj[u]) {
            if(v==p) continue;
