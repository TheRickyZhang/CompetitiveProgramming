...
int32_t main() {
    ...
    stack<pii> s;
    f(i, n) {
        // Best candidate to extend with new building at b[i]
        int curr = (i==0 ? -INFL : dp[i-1]);
        // When we have a new building, all >= height buildings before become irrelevant
        while(!s.empty() && a[s.top().ss] >= a[i]) {
            // This is critically important and how we manage the range max without using segment tree
            // If we imagine a low building with negative view, then we might have dp[i] = 10, dp[i+1] = 5
            // BUT if a[i+2] < a[i+1], we can "prevent" the deficit of building i+1 via i+2 -> ckmx(10+x) instead of (5+x)
            ckmx(curr, s.top().ff);
            s.pop();
        }
        // s.empty() -> this is lowest, we can extend everything to this
        // !s.empty() -> this is lower, we can instead just continue this until i
        dp[i] = (s.empty() ? b[i] : dp[s.top().ss]);

        // Select the best split point, noting that curr =
        ckmx(dp[i], curr + b[i]);
        s.push({curr, i});
    }
    cout<<dp[n-1]<<en;
