...
tpl_<tn_ T, tn_ C> struct Segtree {
  ...
  Segtree(int sz, C c, const v<T> &init = {}, const T &z = T())
      : n(sz), t(4 * sz, z), nums(sz, z), c(move(c)) {
    if (!init.empty()) {
      nums = init;
      build(1, 0, n - 1);
    }
...
private:
  ...
  void modify(int i, int a, int b, int p, T x, bool upd) {
    ...
    int m = (a + b) / 2;
    p <= m ? modify(2 * i, a, m, p, x, upd)
           : modify(2 * i + 1, m + 1, b, p, x, upd);
    t[i] = c(t[2 * i], t[2 * i + 1]);
  }
...
struct pairHash {
  tpl_<tn_ A, tn_ B> size_t op_()(const pair<A, B> &p) const {
    return hash<A>{}(p.ff) ^ (hash<B>{}(p.ss) << 1);
  }
};
struct vHash {
  tpl_<tn_ T> size_t op_()(const v<T> &x) const {
    size_t h = 0;
    for (auto &i : x)
...
int32_t main() {
  ...
  vi VCsum(10,0);      // VC count per first vowel
  vi VC(10*52,0);      // VC[v*52 + c]
  vi VCVsum(100,0);    // VCV per (v1,v2) summed over middle consonant
  vi VCVbyC(52*100,0); // VCV keyed by middle consonant: [c*100 + v1*10+v2]
  vi VCVCsum(100,0);   // VCVC per (v1,v2)
  // sums over VCVC rows/cols
  vi row(10,0), col(10,0);
  int totVCVC=0;
  ...
  for (int x : a) {
    if(isVowel(x)){
      ...
      d[4] += totVCVC - row[v] - col[v]; // + row[v]col[v], but this is 0 since must be distinct

      int totVC=0; f(i,10) totVC+=VCsum[i];
      d[2] += totVC - VCsum[v];

      f(v1,10) if(v1!=v){
        int addAll=VCsum[v1]; if(addAll) VCVsum[v1*10+v]+=addAll;
        f(c,52){ int add=VC[v1*52+c]; if(add) VCVbyC[c*100 + v1*10 + v]+=add; }
      }
      ...
      int totVCV=0; f(i,100) totVCV+=VCVsum[i];
      int bad=0; f(i,100) bad+=VCVbyC[c*100 + i];
      d[3]+=totVCV - bad;

      f(p,100){
        int add=VCVsum[p] - VCVbyC[c*100 + p];
        if(!add) continue;
        VCVCsum[p]+=add; totVCVC+=add;
        row[p/10]+=add; col[p%10]+=add;
      }

      d[1]+=d[0];
      f(v,10){ int add=vcnt[v]; if(add){ VC[v*52+c]+=add; VCsum[v]+=add; } }
    }
    // cout<<d<<en;
  }
  cout << d[4] << endl;
