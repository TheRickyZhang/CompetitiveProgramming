...
int32_t main() {
    ...
    // Maintain the closest index with same value to left and right, or n if none
    // Only one direction is used for the segment tree (left) but the other makes updates easier, similar to linked list
    vi r(n, n), l(n, n);
    f(i, n) {
        auto& v = loc[a[i]];
        auto rit = v.upper_bound(i);
        if(rit != v.end()) r[i] = *rit;
        auto lit = v.lower_bound(i);
        if(lit != v.begin()) l[i] = *prev(lit);
    }
    Segtree<int> tree(n, [&](int x, int y) {
        return min(x, y);
    }, r, n);
    f(_, k) {
        int t, i, x; cin>>t>>i>>x;
        if(t == 1) {
            ...
            if(lit != v.begin()) {
                l[i] = *prev(lit);
                r[*prev(lit)] = i;
                tree.update(*prev(lit), r[*prev(lit)]);
            }
            v.insert(i);
            a[i] = x;
        } else {
            int ll = i-1, rr = x-1;
            int res = tree.query(ll, rr);
            // Note we check for mx > rr, not == n
            cout<<(res > rr ? "YES" : "NO")<<en;
        }
    }
}