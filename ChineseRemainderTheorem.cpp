...
// Returns {a, b, c} such that a*x + b*y = c = gcd(x, y);
iii bezout(int x, int y) {
    if(y == 0) return {1, 0, x};
    auto [a, b, c] = bezout(y, x % y);
    return {b, a - (x/y)*b, c};
}

// Find unique {r3, m3 = lcm(m1, m2)} that uniquely satisfies r3 === r1 (m1), r3===r2 (m2)
pii merge(int r1, int m1, int r2, int m2) {
    auto [a, b, g] = bezout(m1, m2);
    if(r1 % g != r2 % g) return {INFL, INFL};
    // We want a such that r1 + m1*a === r2 (mod m2). We can also divide everything by g.
    int step = m1 / g;
    // Solve for x: x = (r2-r1) mod div m1 ->
    // x= ((r2-r1)/g * u + step) % step
    int x = ((r2-r1)/g * a + step) % step;
    // Do some more stuff but very dumb and complicated
}
...
void solve() {
    ...
    // We can do this shorter version if all divisors d are relatively prime
    int curr = 0;
    f(i, n) {
        int x = a[i].ss;
        int y = tot / x;
