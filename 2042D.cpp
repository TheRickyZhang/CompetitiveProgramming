...
void solve() {
    ...
    sort(all(nums), [&](node a, node b) {
        if(a.x == b.x) return a.y > b.y;
        return a.x < b.x;
    });

    vi toa(all(a)); vi tob(all(b));
    map<int, int> aa, bb;
    f(i, toa.size()) aa[toa[i]] = i;
    f(i, tob.size()) bb[tob[i]] = i;
    for(auto& [x, y, i] : nums) {
        x = aa[x]; y = bb[y];
    }
    for(auto [a, b, c] : nums) {
    ...
    BIT<int> bitcount(n, 0LL, [&](int a, int b) {
        return a+b;
    });
    BIT<int> bitsum(n, 0LL, [&](int a, int b) {
        return a+b;
    });
    vi res(n, -999);

    f(i, n) {
        cout<<bitsum<<en<<bitcount<<en;
        auto [x, y, ii] = nums[i];
        res[ii] = bitsum.query(y, n-1) - (range[ii] * bitcount.query(y, n-1));
        bitsum.add(y, range[ii]);
        bitcount.add(y, 1);
    }
    cout<<bitsum<<en<<bitcount<<en;
    cout<<"Answer: ";
