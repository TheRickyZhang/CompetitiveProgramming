...
tpl_<tn_ T> struct Segtree { int n; v<T> t, nums; T z; fn<T(T, T)> c;
    ...
    Segtree(int sz, fn<T(T, T)> combine, const v<T>& init = {}, const T& zero = T()) : n(sz), t(4 * sz, zero), nums(sz, zero), z(zero), c(std::move(combine)) { if (!init.empty()) { nums = init; build(1, 0, n-1); } }
...
int32_t main() {
    ...
    // We can naturally express the heights and order processing with a stack
    int ln = log2(n) + 1;
    stack<int> s;
    vi next(n, -1);
    f(i, n) {
    ...
    vvi up = binaryJump(next);
    for(int i = 0; i < k; i++) {
        int l, r; cind>>l>>r;
        int res = 1;
        repr(j, ln-1, 0) {
            if(up[l][j] != -1 && up[l][j] <= r) {
                res += 1<<j;
                l = up[l][j];
            }
        }
    ...
    // Segtree<node> tree(n, [&](node a, node b) {
    //     auto it = lower_bound(all(b.h), a.mx + 1);
    //     while(it != b.h.end()) {
    //         a.h.pb(*it);
    //         ++it;
    //     }
    ...
    // });
    // f(i, n) {
    //     tree.update(i, node{a[i], {a[i]}});
    // }
    // f(i, k) {
    //     int l, r; cind>>l>>r;
    //     cout<<tree.query(l, r).h.size()<<en;
    // }
}