...
int32_t main() {
    ...
    // Count of with black root (don't forget +1 for trivial case!)
    vi dp(n, 0);
    autotree dfs = [&](int u, int p) {
        ...
        for(int v : adj[u]) {
            if(v==p) continue;
            dfs(v, u);
            dp[u] = mult(dp[u], dp[v]+1);
        }
    };
    dfs(0, -1);
    // cout<<dp<<en;

    // Ultimate technique: rerouting
    vi up(n, 0);
    up[0] = 1;
    vi res(n);
    autotree dfs2 = [&](int u, int p) {
        res[u] = mult(dp[u], up[u]);
        vi ch;
        for(int v : adj[u]) if(v != p) ch.pb(v);
        int sz = ch.size();
        vi pre(sz+1, 1), suf(sz+1, 1);
        f(i, sz) pre[i+1] = mult(pre[i], dp[ch[i]]+1);
        repr(i, sz-1, 0) suf[i] = mult(suf[i+1], dp[ch[i]]+1);
        auto query = [&](int x) {
            return mult(pre[x], suf[x+1]);
        };
        f(i, sz) {
            int v = ch[i];
            // dp[u] / (dp[v]+1) means that all of v's siblings now contribute like children
            // up[v] = up[u] * (dp[u] / (dp[v]+1)) + 1;
            // Since MOD isn't necessarily prime, we need to use pre/suf multiplication
            up[v] = (mult(up[u], query(i)) + 1) % MOD;
            dfs2(v, u);
        }
    };
    dfs2(0, -1);
