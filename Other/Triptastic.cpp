...
void solve() {
    ...
    f(i, n) f(j, m) pre[i+1][j+1] = pre[i][j+1] + pre[i+1][j] - pre[i][j] + a[i][j];
    // cout<<pre<<en;
    ...
    f(i, n) {
        f(j, m) {
            if(a[i][j]==0) continue;
            ckmn(res, fstTrue(0LL, mx, [&](int d) {
                return query(max(0LL, i-d), max(0LL, j-d), min(n-1, i+d), min(m-1, j+d)) >= k+1;
            }));
        }
    }
    if(res > mx) cout<<-1<<en;
