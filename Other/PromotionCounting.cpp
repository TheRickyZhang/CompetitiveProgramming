...
int32_t main() {
    ...
    // Frequency counter BIT: initialize with 0
    ...
    f(i, n) {
        int r = i;
        while (r < n && nums[i].first == nums[r].first) {
            int node = nums[r].second;
            // Query for elements in the subtree of 'node'
            res[node] = bit.query(start[node], end[node]-1);
            r++;
        }
        for (int j = i; j < r; j++) {
            int node = nums[j].second;
            bit.add(end[node]-1, 1); // Mark this node as processed
        }
        i = r;
    }
    f(i, n) cout<<res[i]<<en;
