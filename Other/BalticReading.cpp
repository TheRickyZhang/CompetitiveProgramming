...
//     Matrix a(28 * 5);
//     f(i, 27) {
//         // Note that every transition not listed in following m lines has a cost of 1
//         // We do not include j=0 because j is a fake node that just goes to all the others
//         fe(j, 26) if (i != j) a.v[i*5][j*5] = 1;
//         // In order to get info of all costs <=N we set a "draining cost" from a state to its previous
//         // So that everything is accounted for in the end
//         fe(j, 4) a.v[i*5+j][i*5+j-1] = 1;
//     }
//     a.v[0][0] = 1;
//     f(i, m) {
//         char cx, cy; int w; cin>>cx>>cy>>w; w--;
//         int x = 5 * (cx - 'a' + 1), y = 5 * (cy - 'a' + 1);
//         // Note that a transition of 1 is default, so 0 = removing old edge, then add new edge
//         a.v[x][y] = a.v[y][x] = 0;
//         a.v[x][y+w] = a.v[y][x+w] = 1;
//     }
//     a = a^(n+1);
//     int res = 0;
//     // THINK ABOUT why this is correct - since our "ideal" cost of incrementing by 1 is NOT moving,
//     // At the end we must have extra cost - number of "back edges" = 0 for each letter.
//     fe(i, 26) res = (res + a.v[0][i*5]) % MOD;
//     cout<<res<<en;
// }
