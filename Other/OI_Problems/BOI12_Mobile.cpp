...
// Solve: dist(a, c) == dist(c, b)
double meet(point& a, point& b) {
    double num = (b.x * b.x) + (b.y * b.y) - (a.x * a.x) - (a.y * a.y);
    double den = 2 * (b.x-a.x);
    return num / den;
}

// Can use clever binary search, or observation about critical points
int32_t main() {
    ios::sync_with_stdio(false); cin.tie(nullptr);
    double end; cin>>n>>end;
    v<point> p(n);
    f(i, n) {
        double x, y; cin>>x>>y;
        y = abs(y);
    ...
    f(i, n) {
        ...
        // Same x handling
        if(!s.empty() && fabs(s.back().x - x) < 1e-9) {
            if (y >= s.back().y) continue;
            s.pop_back();
        }
        // Find crosses
        while(s.size() >= 2 && meet(s[s.size()-2], s[s.size()-1]) > meet(s[s.size()-1], p[i])) {
            s.pop_back();
        }
        s.push_back(p[i]);
    }
    while(s.size() >= 2 && meet(s[0], s[1]) < 0) s.pop_front();
    while(s.size() >= 2 && meet(s[s.size()-2], s[s.size()-1]) > end) s.pop_back();
    double res = 0;
    int sz = s.size();
    f(i, sz) {
        double l = (i>0 ? meet(s[i-1], s[i]) : 0);
        double r = (i<sz-1) ? meet(s[i], s[i+1]) : end;
        point lll = {l, 0}, rrr = {r, 0};
        if(l >= 0 && l <= end) ckmx(res, dist(lll, s[i+1]));
        if(r >= 0 && r <= end) ckmx(res, dist(rrr, s[i]));
    }
    cout<<fixed<<setprecision(6)<<res<<en;
}
