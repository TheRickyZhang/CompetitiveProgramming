...
int32_t main() {
    ...
    // Read in weights into 26 different trees
    vi a(n), b(n);
    f(i, n) {
        char c; cin>>c;
        a[i] = c-'a';
    ...
    // Iterate through each of the flowers
    f(c, 26) {
        int res = INFL;
        vi cnt(n, 0);
        f(i, n) if(a[i]==c) cnt[i] = b[i];
        vi sz(n, 0), up(n, 0);
        ...
        autotree dfs2 = [&](int u, int p) {
            ckmn(res, up[u] + down[u]);
            for(auto [v, w] : adj[u]) {
                if(v==p) continue;
                // Be careful with this formula - very easy to mess something up!
                down[v] = down[u] + (up[u] - (up[v] + w*sz[v])) + w * (sz[0]-sz[v]);
                dfs2(v, u);
            }
        };
        dfs2(0, -1);
