...
int main() {
    ...
    cin>>n;
    vvll nums(n, vll(n));
    f(i, n) f(j, n) cin>>nums[i][j];

    m=1<<n;
    // Represent the STATE of girls matched -> popcount 1 = which boy to match next
    vi dp(m, 0);
    dp[0] = 1;
    f(i, m) { // alloc of females
        ll matches = __builtin_popcount(i);
        f(j, n) { // Searching for a female
            if(i & (1<<j)) continue; // Already paired
            if(!nums[matches][j]) continue;
