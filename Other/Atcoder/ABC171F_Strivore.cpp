...
int32_t main() {
    ...
    int x = n+k;
    vi fa(x+1, 0); fa[0] = 1;
    fe(i, x) fa[i] = mult(fa[i-1], i);
    vi ifa(x+1, 0); ifa[x] = fastPow(fa[x], MOD-2, MOD);
    repr(i, x-1, 0) ifa[i] = mult(ifa[i+1], i+1);
    auto choose = [&](int n, int k) {
        return mult(fa[n], mult(ifa[k], ifa[n-k]));
    };
    int res = 0;
    // How many of the k are after the last character in s
    rep(i, 0, k) {
        res += mult(mult(fastPow(25, k-i), choose(k-i + n-1, n-1)), fastPow(26, i));
        res %= MOD;
    }
    cout<<res<<en;
}
