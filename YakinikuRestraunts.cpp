...
int32_t main(){
    ...
    v<v<v<pii>>> mins(m);
    f(j,m){
        v<v<pii>> sp(n,v<pii>(ln,{0,0}));
        f(i,n) sp[i][0]={a[i][j],i};
        fe(k,ln-1){
            for(int i=0;i+(1<<k)<=n;i++){
                auto [x,xi]=sp[i][k-1];
                auto [y,yi]=sp[i+(1<<(k-1))][k-1];
                sp[i][k]=(x>y||(x==y&&xi<yi))?pii{x,xi}:pii{y,yi};
            }
        }
        mins[j]=sp;
    }
    auto query=[&](int j,int l,int r){
        int k=log2(r-l+1);
        auto [x,xi]=mins[j][l][k];
        auto [y,yi]=mins[j][r-(1<<k)+1][k];
        return (x>y||(x==y&&xi<yi))?xi:yi;
    };
    ...
    function<void(int,int,int)> solve=[&](int j,int L,int R){
        if(L>R) return;
        int p=query(j,L,R), v=a[p][j];
        add(L,p,p,R,v);
        solve(j,L,p-1); solve(j,p+1,R);
    };
    f(j,m) solve(j,0,n-1);
    ...
    int res=-INFL;
    f(i,n) f(j,n) if(j>=i) cmx(res, d[i][j]-(pref[j]-pref[i]));
    cout<<res<<en;
}
