...
void solve() {
  // vi a(n); read(a);
  // // vi b(n); // How many additions needed to delay poby by 1
  // // vi c(n); // How many reductions needed
  // vi b(n); 
  // vi pre(n+1, 0);
  // // 
  // f(i, n) {
  ...
  // f(_, k) {
  //   int l, r; cind>>l>>r;
  //   cout<<pre[r+1] - pre[l]<<en;
  // }

  function<int(int)> ceil_log2 = [&](int y) { 
    int b = msb(y);
    return ((1ll<<b)==y) ? b : b+1;
  };
  auto isPow = [&](int x){ return x && ((x&(x-1))==0); };
  ...
  f(i,n){
    w[i]=calc(a[i]);
    frag[i]=(a[i]>=3 && isPow(a[i]-1)) ? 1 : 0;
  }
  vi pw(n+1), pc(n+1);
  f(i, n) {
    pw[i+1]=pw[i]+w[i]; pc[i+1]=pc[i]+frag[i];
  }
  f(_, k) {
    int l,r; cind>>l>>r;
    int x = pc[r+1]-pc[l];
    cout<<(pw[r+1]-pw[l]) + (x/2)<<en;
  }
}
