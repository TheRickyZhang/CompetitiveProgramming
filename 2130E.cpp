...
// Pass in 0-indexed to these, convert to 1-indexed when asking
int ask(vi& v) {
    int x;
    cout<<"?"<<sp<<v.size()<<sp;
    for(int i : v) cout<<i+1<<sp;
    cout.flush();
    cin>>x; return x;
}
int ask(int to) {
...
void solve() {
    cin>>n;
    int pos = fstTrue(0LL, n-2, [&](int x) {
        return ask(x+1) > 0;
    });
    int l, r;
    l = pos, r = (pos+1) % n;
    // cout<<"l"<<sp<<l<<sp<<"r"<<r<<en;
    int it = 0;
    str res(n, '0');
    while(it < n) {
        vi v;
        int i = 0;
        while(true) {
            int x = (1<<(i/2)) - 1, y = (1<<ceil(i, 2)) - 1;
            if(v.size() + (x+y)*2 + 2 > n) {
                // cout<<"breaking at "<<i<<sp<<v.size()<<sp<<x<<sp<<y<<en;
                break;
            }
            f(_, x) v.pb(l), v.pb(r);
            v.pb(it + i);
        ...
        int tot = ask(v);
        // cout<<"getting tot="<<tot<<en;
        f(j, i) {
            if(tot & (1LL<<j)) res[it+j] = '(';
            else res[it+j] = ')';
        }
        it += i;
    }
