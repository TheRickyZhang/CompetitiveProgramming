...
int32_t main() {
    ...
    cin>>n>>m;
    vvi adj(n);
    vpii a, b; // towards each other, away from each other
    f(i, m) {
        int t, u, v; cin>>t; cind>>u>>v;
        adj[u].pb(v); adj[v].pb(u);
        if(t == 1) a.pb({u, v});
        else b.pb({u, v});
    }
    vi col(n, -1);
    function<bool(int, int, int)> bipartite = [&](int u, int p, int c) {
        col[u] = c;
        for(int v : adj[u]) {
            if(v == p) continue;
            if(col[v] == c) return false;
            if(col[v] == -1) {
                if(!bipartite(v, u, 1-c)) return false;
            }
        }
        return true;
    };
    f(i, n) {
        if(col[i] == -1 && !bipartite(i, -1, 0)) {
            cout<<"NO"<<en; return 0;
        }
    }
    // cout<<col<<en;
    vvi nadj(n);
    vi in(n, 0);
    // Assume the orientation of all 0-colored is left in 0-1 relations
    for(auto [u, v] : a) {
        if(col[u] != 0) swap(u, v);
        nadj[u].pb(v); // u is to the left of v
        in[v]++;
    }
    for(auto [u, v] : b) {
        if(col[u] != 0) swap(u, v);
