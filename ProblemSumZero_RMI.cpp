...
#define str string
    int N = 100000; int MOD=1e9+7; constexpr int INF=1e9; ll INFL=0x3f3f3f3f3f3f3f3f; constexpr auto en = "\n"; constexpr auto sp = " ";
...
int32_t main(){
    ...
    // Enforce monotonicity for the immediate jumps.
    f(i, n-1) {
        ckmx(prev[i+1], prev[i]);
    }

    // Compute chain length for each index: len[i] = 0 if no valid jump, else 1 + len[prev[i]]
    vi len(n, 0);
    f(i, n) {
        if(prev[i] != -2)
            len[i] = 1 + (prev[i] >= 0 ? len[prev[i]] : 0);
        else
            len[i] = 0;
    }

    // Build offset array for contiguous dp storage.
    vi offset(n+1, 0);
    int total = 0;
    f(i, n){
        offset[i] = total;
    ...
    // Allocate contiguous dp table.
    // For each index i, dp[ offset[i] ... offset[i]+len[i]-1 ] holds its valid jump levels.
    vector<int> dp(total, -2);
    f(i, n){
        if(len[i] > 0){
            // Level 0: immediate jump stored as prev[i]
            dp[offset[i]] = prev[i];
            for (int j = 1; j < len[i]; j++){
                int p = dp[offset[i] + j - 1];
                if(p < 0 || len[p] < j)
                    dp[offset[i] + j] = -2;
                else
                    dp[offset[i] + j] = dp[offset[p] + j - 1];
            }
        }
    }
    ...
    f(query, k) {
        ...
        int res = 0;
        int chainLen = len[r];
        // Binary lifting using only the valid levels for index r.
        repr(j, chainLen - 1, 0) {
            // dp for index r at level j is stored at dp[offset[r] + j]
            if(dp[offset[r] + j] >= l - 1) {
                r = dp[offset[r] + j];
                res += (1 << j);
            }
            if(r < 0) break;
