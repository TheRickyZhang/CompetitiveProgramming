...
int32_t main() {
    ...
    f(i, n) {
        q.push({a[i][0], i, 0});
        if(m != 1) q.push({a[i][m-1], i, m-1});
    }
    rep(j, 1, m-2) {
        q.push({a[0][j], 0, j});
        if(n != 1) q.push({a[n-1][j], n-1, j});
    }
    vi res(k+1, 0);
    res[0] = n*m;
    ...
    fe(lvl, k) {
        // auto copy = q;
        // while(!copy.empty()) {
        //     auto [a, b, c] = copy.top(); cout<<a<<sp<<b<<sp<<c<<en;
        //     copy.pop();
        // }
        // cout<<en;
        res[lvl] = res[lvl-1];
        ...
        function<void(int, int)> floodfill = [&](int x, int y) {
            ...
            for(auto [dx, dy] : dirs) {
                int nx = x+dx, ny = y+dy;
                if(check(nx, ny, n, m)) {
                    if(a[nx][ny] <= lvl) floodfill(nx, ny);
                    else {
        ...
        while (!q.empty() && get<0>(q.top()) <= lvl) {
            auto [d, xx, yy] = q.top();
            q.pop();
            floodfill(xx, yy);
        }
    }
    fe(i, k) cout<<res[i]<<en;
}
