...
cx_ int N=1000; cx_ int MOD=1e9+7; // 998244353;
...
int32_t main(){
    ...
    f(day,n){
        ...
        fill(all(dpp), INFL);
        dpp[0]=dp[0]+x;
        rep(k,1,day+1){ // can’t use >day cleanings
            cmn(dpp[k], dp[k]+x);
            cmn(dpp[k], max(dp[k-1]+x-y, 0LL));
        }
        if(it<d && day==b[it]){
            int k=0; while(k<=day+1 && dpp[k]>0) ++k;
            if(k>day+1){
              cout<<-1<<en; return 0;
            }
            res+=k;
            // new segment after a clean room
            fill(all(dp), INFL); dp[0]=0;
            ++it;
        }else{
            swap(dp,dpp);
        }
