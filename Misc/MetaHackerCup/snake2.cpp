...
void solve(int t) {
  ...
  int res=fstTrue(0LL,mx,[&](int x){
    vb good(n,0);
    function<bool(int,int)> check = [&](int i,int p)->bool{
      if(good[i]) return true;
      if(a[i]<=x){ good[i]=1; return true; }
      if(i>0 && i-1!=p && llabs(a[i]-a[i-1])<=x && check(i-1,i)){ good[i]=1; return true; }
      if(i<n-1 && i+1!=p && llabs(a[i]-a[i+1])<=x && check(i+1,i)){ good[i]=1; return true; }
      return false;
    };
    f(i,n) if(!check(i,-1)) return false;
    return true;
  });
  str resStr = "Case #" + to_string(t) + ": " + to_string(res);
  cout<<resStr<<endl; 
}
