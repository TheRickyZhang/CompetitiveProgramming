...
inline int rising(int N, int t){
  int r=1, nm=(int)(N%MOD); f(j,t) r=mult(r,(nm+j)%MOD); return r;
}
...
void solve(int tc){
    ...
    if(x>1){ p.pb(x); cnt.pb(1); }

    int m=p.size();
    vvi pw(m); vvi w(m);
    f(i,m){
        pw[i].assign(cnt[i]+1,1);
        f(k, cnt[i]) pw[i][k+1]=pw[i][k]*p[i];
        int c=cnt[i];
        w[i].assign(c+1,0);
        f(k, c+1){
            int t1=mult(rising(N,k), ifa[k]);
            int t2=mult(rising(N,c-k), ifa[c-k]);
            w[i][k]=mult(t1,t2);
        }
    }
    ...
    int ans=0;
    function<void(int,int,int)> dfs=[&](int idx, int cur, int ways){
        if(cur > A) return;
        if(idx == m){
          ans=add(ans,ways); return;
        }
        int id=ord[idx];
        f(k, pw[id].size()){
          int q = pw[id][k];
          if(q && cur > A/q) break;
          dfs(idx+1, cur*q, mult(ways, w[id][k]));
        }
    };
    dfs(0,1,1);
...
int32_t main(){
  setIO("final");
  primes=sieveList(10'000'000);
  auto facts = initFact(60);
  fa = facts.first, ifa = facts.second;
  int T; if(!(cin>>T)) return 0; f(i,T) solve(i+1);
}
