...
cx_ int N=1000000; cx_ int MOD=1e9+7; // 998244353;
...
// Same as DSU, but we maintain invariant that par[i] > i if merged, and we only merge i, i+1
struct DSUNext {
  vi p;
  int n;
  DSUNext(int n) : n(n), p(n) {
    iota(all(p), 0LL);
  }
  // smallest >= i still unused. Can return sentinel n.
  int next(int i) {
    if(i >= n) return n;
    if(p[i] == i) return i;
    return p[i] = next(p[i]);
...
void solve(int t) {
  ...
  f(i, m) {
    f(j, sz[i]) {
      int u = paths[i][j];
      nodeToPaths[u].push_back({i, j});
    }
  }

  v<DSUNext> dsus; dsus.reserve(m);
  f(i, m) {
    dsus.push_back(DSUNext(sz[i]));
  }
  ...
  while(!q.empty()) {
    ...
    for(auto [i, j] : nodeToPaths[u]) {
      auto& dsu = dsus[i];
      int it = dsu.erase(j);
      while(it <= min(j+k, sz[i]-1)) {
        int v = paths[i][it];
        if(dist[v] == INFL) {
          cmn(dist[v], d+1);
          q.push({v, d+1});
  ...
  // vvi paths(m);
  // map<int, vpii> nodeToPaths;
  // f(i, m) {
  //   int u; cind>>u;
  //   paths[i].push_back(u);
  //   nodeToPaths[u].pb({i, paths[i].size() - 1});
  // }
  // v<v<iii>> checks(n);
  // vi dist(n, INFL);
  // dist[0] = 0;
  //
  // fvii dfs = [&](int u, int ii) {
  //   int d = dist[u];
  //   for(auto [i, j] : nodeToPaths[u]) {
  //     int extra = (i != ii ? 1 : 0);
  //     int nd = d + 1 + extra;
  ...
  //       int vl = paths[i][l];
  //       if(nd <= dist[vl]) {
  //         checks[i].push_back({nd, l, j});
  //         dist[vl] = nd;
  //         dfs(vl, i);
  //       }
  //     }
  //     int r = min(ll(paths[i].size()) - 1LL, j+k);
  //     if(r != j) {
  //       checks[i].push_back({nd, j, r});
  //       int vr = paths[i][r];
  //       if(nd <= dist[vr]) {
  //         dist[vr] = nd;
  //         dfs(vr, i);
  ...
  //   }
  // };
  // for(auto [i, _] : nodeToPaths[0]) {
  //   dfs(0, i);
  // }
  // // Lazily propagate checks?
  // f(i, n) {
  //   for(auto [d, l, r] : checks[i]) {
  //     rep(j, l+1, r) {
  //       cmn(dist[paths[i][j]], d);
  //     }
  //   }
  // }
  //
