...
public:
    ...
    map<string, int> solve(int l, int r) {
        ...
        auto findNextElement = [&]() {
            str e;
            assert(i <= r && isupper(s[i]));
            e += s[i++];
            while(i <= r && islower(s[i])) {
                e += s[i++];
            }
            return e;
        };
        auto parseNumber = [&]() {
            ...
            int ans = stoll(s.substr(i, j-i));
            i = j;
            return ans;
        };
        while(i <= r) {
            if(match[i] != -1) {
                map<string, int> temp = solve(i+1, match[i]-1);
                i = match[i]+1;
                int cnt = parseNumber();
                for(auto [x, y] : temp) {
                    res[x] += y * cnt;
    ...
    string countOfAtoms(string formula) {
        ...
        map<string, int> mp = solve(0, n-1);
        str res;
        for(auto [e, cnt] : mp) {
            res += e + (cnt == 1 ? "" : to_string(cnt));
        }
        return res;
    }
};
