...
int32_t main() {
    ...
    f(i, n) sz[i] = adj[i].size();

    vb vis(n, false);
    vb evis(m, false);
    vi next(n);
    // Pay close attention to this. Note we do not need to handle complex vis state - by
    // Having next, all we need is encounter vis[u] -> we can follow and reset vis[it] to get our path
    fvi dfs = [&] (int u) {
        if(vis[u]) {
            int it = u;
            while(vis[it]) {
            ...
            cout<<en;
            // return; // Note this return is NOT necessary in this logic because we want to loop until all sz[u]
            // exhausted anyway - hence why we only make one call to dfs(0)
        }
        while(sz[u]) {
            auto [v, ei] = adj[u][--sz[u]];
            if(evis[ei]) continue; evis[ei] = true;
            vis[u] = true;
            next[u] = v;
            dfs(v);
        }
...
//             int ei = adj[u][i]; i++;
//             auto [uu, vv] = edges[ei];
//             int v = u ^ uu ^ vv; // Just get the other node
//             if(v != start && nvis[v]) continue;  // Important to check for locally visited nodes before saying that the edge is used
//             if(vis[ei]) continue; vis[ei] = true;
//             dfs(v);
//             if(found) return;
//         }
//         path.pop_back();
