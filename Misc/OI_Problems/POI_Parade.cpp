...
int32_t main() {
    ...
    autotree dfs = [&](int u, int p) {
        ...
        a[u] = x + adj[u].size() - 2;
        b[u] = x + y + adj[u].size() - 3;
    };
    int cnt = 0;
    dfs(0, -1);
    int res = 0;
    // Recurrence simple, mostly comes down to handling these edges cases correctly
    f(i, n) {
        if(adj[i].size() == 1) cnt++;
        else ckmx(res, max(a[i], b[i]) + 1); // The 1 is over-subtracted since it will not have its parent
    }
    cout<<res - (cnt==n-1)<<en;

    // vi dp(n, 0); // Path starting from u down
    // vi res(n, 0);
    // vi fir(n, -1), sec(n, -1);
    // auto updateBest = [&](int i, int x) {
    //     int f = fir[i], s = sec[i];
    //     if(x > f) {
    //         fir[i] = x, sec[i] = f;
    //     } else if(x > s) {
    ...
    // // cout<<dp<<en;
    //
    // function<void(int, int)> dfss = [&](int u, int p) {
    //     res[u] = dp[u];
    //     for(int v : adj[u]) {
    //         if(v==p) continue;
    //         dfss(v, u);
    //         ckmx(res[u], res[v]); // Use path from subtree
    //     }
    //     if(sec[u] != -1) {
    //         ckmx(res[u], fir[u] + sec[u]);
    //     }
    // };
    // dfss(0, -1);
    // // cout<<res<<en;
    ...
    // for(int v : adj[0]) {
    //     updateBest(0, dp[v]);
    //     ckmx(ans, res[v] + max(1LL, ll(adj[0].size()) - 1));
    // }
    // if(sec[0] != -1) ckmx(ans, fir[0] + sec[0]);
    // cout<<ans<<en;
}
