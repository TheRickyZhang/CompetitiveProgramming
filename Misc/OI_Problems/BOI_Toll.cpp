...
int32_t main() {
    ...
    int q;
    cin>>k>>n>>m>>q;
    int cnt = ceil(n, k); // How many blocks there are
    int ln = log2(cnt) + 1;
    int dp[cnt][k][ln][k];
    f(i, cnt) f(j, k) f(a, ln) f(b, k) dp[i][j][a][b] = INFL;
    f(i, m) {
        int u, v, w; cin>>u>>v>>w;
        ckmn(dp[u/k][u%k][0][v%k], w);
    }
    fe(j, ln-1) {
        for(int i = 0; i + (1<<j) <= cnt; ++i) {
            f(x, k) {
                f(y, k) {
                    f(z, k) {
                        ckmn(dp[i][x][j][y],
                            dp[i][x][j-1][z] + dp[i + (1<<(j-1))][z][j-1][y]);
                    }
                }
            }
        }
    }
    vi curr(k, 0), temp(k, 0);
    f(_, q) {
        curr.assign(k, INFL);
        int u, v; cin>>u>>v;
        int a = u/k, x = u % k, b = v/k, y = v % k;
        curr[x] = 0; // Handle starting location gracefully
        int rem = b-a;
        // cout<<"rem "<<rem<<sp<<"ln "<<ln<<en;
        repr(ii, ln-1, 0) {
            if(rem & (1<<ii)) {
                temp.assign(k, INFL);
                f(i, k) f(j, k) {
                    ckmn(temp[j], curr[i] + dp[a][i][ii][j]);
                }
                curr = temp;
                a += (1<<ii);
            }
