...
int32_t main() {
    ...
    stack<pii> s;
    int inv = fastPow(2, MOD - 2, MOD);
    auto calc = [&](int x) {
        return mult(mult(x, x + 1), inv);
    };
    auto count = [&](int x, int y) {
        return mult(calc(x), calc(y));
    };

    int res = 0;

    for (auto [y, x] : a) {
        if (s.empty() || y > s.top().ff) {
            s.push({y, x});
        } else if (y == s.top().ff) {
            s.top().ss = (s.top().ss + x) % MOD;
        } else {
            int curr = 0;
            while (!s.empty() && s.top().ff > y) {
                auto [h, w] = s.top();
                s.pop();
                res = (res + (count(curr + w, h) - count(curr, h) + MOD)) % MOD;
                curr += w;
            }
            x += curr;
            s.push({y, x});
    ...
    while (!s.empty()) {
        auto [y, x] = s.top();
        s.pop();
        res = (res + (count(curr + x, y) - count(curr, y) + MOD)) % MOD;
        curr = (curr + x) % MOD;
    }
    cout<<res<<en;
}
