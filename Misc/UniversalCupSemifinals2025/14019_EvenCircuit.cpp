...
//     f(i, n) {
//         int x; cin>>x;
//         a[i] = 2*x + 1; // Add parity bit for even length
//     }
...
//     }
//
//     vi bit(23, -1); // Points to index that has MSB b
//     vi basis;
//     vi basisMask;
//     int res = INFL;
//     f(j, n) {
...
//             }
//         }
//         // cout<<"done processing "<<x<<en;
//         if(x == 0) {
//             cmn(res, (ll)__builtin_popcountll(mask));
//             continue;
//         }
//         while(true) {
//             assert(x != 0);
...
template<typename A, typename B> ostream& operator<<(ostream& cout, const pair<A, B>& p) {
	return cout << "(" << p.f << ", " << p.s << ")";
}
template<typename A> ostream& operator<<(ostream& cout, const vector<A>& v) {
	cout << "[";
	forn(i, v.size()) {
		if(i) cout <<", ";
		cout << v[i];
...
bool is_larger(int n, int k, int cmp) {
	ll res = 1;
	for (int i = 1; i <= k; i++) {
		res *= n+1-i;
		res /= i;
...
void solve() {
	...
	vector<int> a(n);
	forn(i,n) cin >> a[i];
	vector<vector<pii>> layers(2);
	forn(i,n) layers[1].pb(mp(a[i], i));
	for (int k = 1; !layers[k].empty(); k++) {
		sort(layers[k].begin(), layers[k].end());
		int m = layers[k].size();
		forn(i, m-1) if (layers[k][i].first == layers[k][i+1].first) {
			cout << "Yes\n" << 2*k << '\n';
			return;
		}
		...
		if (is_larger(n, k+1, 1<<22)) {
			cout << "Yes\n" << 2*(k+1) << '\n';
			return;
		}
		layers.push_back(vector<pii>());
		for (pii p : layers[k]) {
			for (int j = p.s+1; j < n; j++) {
				layers[k+1].pb(mp(p.f ^ a[j], j));
			}
		}
	}
}
