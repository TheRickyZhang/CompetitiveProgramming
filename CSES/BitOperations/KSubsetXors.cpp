...
int32_t main(){
    ...
    vi piv;
    f(i,31) if(b[i]) piv.pb(b[i]);
    sort(all(piv));

    int m = n - d;
    int mm = 1LL<<m;
    int need = ceildiv(k, mm);

    vi cur = {0}, nxt, tmp;
    f(ii, d){
        int x = piv[ii];
        nxt.clear();
        for(int v: cur) nxt.pb(v ^ x);
        sort(all(nxt));

        tmp.clear();
        int p = 0, q = 0;
        while(tmp.size() < need && (p < cur.size() || q < nxt.size())){
            if(q == nxt.size() || (p < cur.size() && cur[p] < nxt[q])){
                tmp.pb(cur[p++]);
            }
            else if(p == cur.size() || (q < nxt.size() && nxt[q] < cur[p])){
                tmp.pb(nxt[q++]);
            }
            else {
                tmp.pb(cur[p]);
