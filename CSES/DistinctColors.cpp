...
int32_t main() {
    ...
    function<void(int, int)> dfs = [&](int u, int p) {
        ...
        tour[t++] = u; // Post order traversal
    };
    dfs(0, 0);
    BIT<int> tree(n, 0LL, [&](int a, int b) {
        return a+b;
    });
    vi res(n);
    map<int, int> mp; // Rightmost occurrence of color
    f(i, n) {
        // Think through this logic. To not over count repeated colors, we increment at first node and decrement anytime later
        if(mp.count(c[tour[i]])) {
            tree.add(mp[c[tour[i]]], -1);
        }
        mp[c[tour[i]]] = i;
        tree.add(i, 1);
        res[tour[i]] = tree.query(last[tour[i]], i);
    }
    f(i, n) cout<<res[i]<<sp;
}
