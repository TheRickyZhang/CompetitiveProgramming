...
vvi identity(int x) {vvi res(x, vi(x, 0));f(i, x) res[i][i]=1;return res;} inline vvi mult(const vvi& A, const vvi& B, int m = MOD) { int n = A.size(); vvi C(n, vi(n)); f(i, n) f(j, n) f(k, n) C[i][j] = (C[i][j] + A[i][k]*B[k][j]) % m; return C;}
inline vvi fastPow(vvi A, int p, int m = MOD) { vvi B = identity(A.size()); for(; p; p>>=1, A = mult(A, A, m)) if(p & 1) B = mult(B, A, m); return B;}
