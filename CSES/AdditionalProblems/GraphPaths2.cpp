...
inline vvi mult(const vvi &A, const vvi &B) {
    int n = A.size();
    vvi C(n, vi(n, INFL));
    f(i, n) f(k, n) f(j, n)
        if (A[i][k] < INFL && B[k][j] < INFL)  // Prevent overflow
            C[i][j] = min(C[i][j], A[i][k] + B[k][j]);
    return C;
}

inline vvi fastPow(vvi A, int64_t p) {
    int n = A.size();
    vvi r(n, vi(n, INFL));
    f(i, n) r[i][i] = 0;  // Identity matrix for min-plus multiplication
    while (p) {
        if (p & 1) r = mult(r, A);
        A = mult(A, A);
        p >>= 1;
...
int32_t main() {
    ...
    vvi res = fastPow(a, k);
    if(res[0][n-1]>=INFL) cout<<-1<<en;
    else cout<<res[0][n-1]<<en;
}
