...
int t, k, n, m;
// Note that this choice of negation doesn't matter; as long as it's consistent = convenient
int neg(int x) {
    return 2*m-x-1;
}
void solve() {
    ...
    // Read into radj lists
    f(i, n) {
        int u, v; char a, b; cin>>a>>u>>b>>v; u--; v--;
        if(a=='-') u = neg(u);
        if(b=='-') v = neg(v);
        adj[neg(u)].pb(v); adj[neg(v)].pb(u);
        radj[u].pb(neg(v)); radj[v].pb(neg(u));
    }
    vb vis(2*m, false);
    vi nodes;
