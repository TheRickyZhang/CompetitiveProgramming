...
int32_t main() {
    ...
    str s; cin>>s;
    n = s.size(); m = n/2;
    vvi choose(m+1, vi(m+1, 0));
    f(i, m+1) choose[i][0] = 1;
    fe(i, m) {
        rep(j, 1, i) choose[i][j] = (choose[i-1][j] + choose[i-1][j-1]) % MOD;
    }
    vvi dp(n+1, vi(n+1, 0));
    f(i, n) dp[i+1][i] = 1; // Note this base case
    for(int len = 2; len <= n; len += 2) {
        f(i, n-len+1) {
            int j = i+len-1;
            for(int k = i+1; k <= j; k += 2) {
                if(s[i] != s[k]) continue;
                int curr = (dp[i+1][k-1] * dp[k+1][j]) % MOD;
                curr = (curr * choose[(j-i+1) / 2][(k-i+1)/2]) % MOD;
                dp[i][j] += curr; dp[i][j] %= MOD;
            }
        }
    }
    cout<<dp[0][n-1]<<en;
