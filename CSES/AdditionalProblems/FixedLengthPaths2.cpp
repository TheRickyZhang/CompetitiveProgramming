...
int32_t main() {
    ...
    vi tot(r+1, 0);
    vb rem(n, false);
    int subtree_depth, mx_depth; // Don't forget about this to not TLE!
    int res = 0;
    ...
    function<void(int, int, int)> get_cnt = [&](int u, int p, int d) {
        if(d > r) return;
        cnt[d]++;
        ckmx(subtree_depth, d);
        for(int v : adj[u]) {
            if(v==p || rem[v]) continue;
            get_cnt(v, u, d+1);
        }
    ...
    function<void(int)> build = [&](int u) {
        ...
        for(int v : adj[c]) {
            ...
            get_cnt(v, c, 1);
            int curr = 0;
            for (int x = max(0LL, l-1); x <= r-1; x++) {
                curr += tot[x];
            }
            rep(d, 1, subtree_depth) {
                res += curr * cnt[d];
                if(r-d >= 0) curr -= tot[r-d];
                if(l-d-1 >= 0) curr += tot[l-d-1];
            }
            rep(d, 1, subtree_depth) {
                tot[d] += cnt[d];
            }
            ckmx(mx_depth, subtree_depth);
        }
        rep(i, 1, mx_depth) tot[i]=0;
        for(int v : adj[c]) {
            if(!rem[v]) build(v);
        }
    };
