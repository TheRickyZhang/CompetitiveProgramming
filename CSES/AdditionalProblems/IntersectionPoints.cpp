...
// NOTE: Onl;y runtime error since need to account for negatives with pos shift
int32_t main() {
    ...
    f(i, n) {
        ...
        // We go up from y, so put endpoints if same x
        if(a==c) {
            points.pb({b, 1, a, a});
            points.pb({d, 3, a, a});
        } else {
    ...
    sort(all(points)); // Ascending y order, with 1->new vert, 2->hori, 3->end vert
    BIT<int> bit(1e6, 0, ad);
    int res = 0;
    for(auto [a, b, c, d] : points) {
        if(b==1) {
            bit.add(c, 1);
            // cout<<bit<<en;
        } else if(b==2) {
