...
int32_t main() {
    ...
    fe(j, ln-1) {
        f(i, n) {
            ...
            ckmx(mx[i][j], max(mx[i][j-1], (par[i][j-1] == -1 ? -1 : mx[par[i][j-1]][j-1])));
        }
    }
    ...
    f(i, k) {
        ...
        if(u==v) {
            cout<<best<<en; continue;
        }
        repr(j, ln-1, 0) {
            if(par[u][j] == par[v][j]) continue;
            ckmx(best, mx[u][j]); ckmx(best, mx[v][j]);
            u = par[u][j]; v = par[v][j];
        }
        ckmx(best, max(mx[u][0], mx[v][0]));
        cout<<best<<en;
    }
}
