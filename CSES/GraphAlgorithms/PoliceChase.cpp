...
// Implementation of finding minimum cut
int32_t main() {
    setIO();
    cin>>n>>m;
    vv<edge> adj(n);
    ...
    auto ae = [&](int u, int v) {
        ...
        adj[u].pb({v, 1, vi, true});
        adj[v].pb({u, 0, ui, false});
    };
    f(i, m) {
        int u, v; cind>>u>>v;
        edges.pb({u, v});
    ...
    function<bool(void)> bfs = [&]() {
        ...
        while(!q.empty()) {
            int u = q.front(); q.pop();
            for(auto [v, c, r, _] : adj[u]) {
                if(c <= 0 || dist[v] != -1) continue;
                dist[v] = dist[u]+1;
                q.push(v);
            }
        }
    ...
    function<int(int, int)> dfs = [&](int u, int f) {
        if(u == t) return f;
        int curr = 0;
        for(auto& i = pos[u]; i<adj[u].size(); ++i) {
            auto& [v, c, r, _] = adj[u][i];
    ...
    fvi dfss = [&](int u) {
        seen[u] = true;
        for(auto [v, c, r, _] : adj[u]) { // Note that here we do not care what the original direction was
            if(c > 0 && !seen[v]) dfss(v);
        }
    };
    dfss(s);
    vpii res;
    ...
    assert(res.size() == flow);
    for(auto [u, v] : res) cout<<u+1<<sp<<v+1<<en;
}
