...
tpl_<tn_ T> struct Segtree { int n; v<T> t; T z; function<T(T, T)> c;   // All 0-indexed except for i
...
tpl_<tn_ T> struct BIT     { int n; v<T> t, nums; T z; function<T(T, T)> c;   // All 0-indexed
    BIT(int sz, T zero, function<T(T, T)> combine) : n(sz), t(sz + 1, zero), nums(sz, zero), z(zero), c(move(combine)) {}
...
int32_t main() {
    ...
    f(i, k) {
        ...
        if(tt==1) {
            ...
            int u; cin>>u; u--;
            int res = bit.query(start[u], end[u]-1); // Since end is exclusive
            cout<<res<<en;
        }
    }
}
