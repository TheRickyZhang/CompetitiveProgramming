...
// Minimal topological permutation - note that ordering != topo, but that it acts on the revers
int32_t main() {
    ios::sync_with_stdio(false); cin.tie(nullptr);
    cin>>n>>m;
    vvi adj(n); vi out(n, 0);
    f(i, m) {
        int u, v; cind>>u>>v;
        adj[v].pb(u); // Note the difference
        out[u]++;
    }
    pq<int> q;
    vi res;
    f(i, n) if(out[i]==0) q.push(i);
    while(!q.empty()) {
        int u = q.top(); q.pop();
        res.pb(u);
        for(int v : adj[u]) {
