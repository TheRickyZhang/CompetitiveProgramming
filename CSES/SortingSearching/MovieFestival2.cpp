...
int main() {
    ...
    f(i, n) {
        auto [l, r] = nums[i];
        // You may be thinking: why not lower_bound instead of upper_bound-1?
        // Upper_bound-1 guaranteed to return <= x, while lower_bound returns >=x
        auto it = ends.upper_bound(l);
        if(it==ends.begin()) continue; ends.erase(--it);
        ends.insert(r);
        res++;
    }
    cout<<res<<en;
    ...
    // f(i, n) {
    //     int l, r; cin>>l>>r;
    //     nums.pb({l, 1, i}); nums.pb({r, 0, i}); // Process ending first if same time
    // }
    // sort(all(nums));
    // for(auto [a, b, c] : nums) cout<<a<<sp<<b<<sp<<c<<en;
    // vb watching(n+1, false);
    // int res=0, free=k;
    // f(i, 2*n) {
    //     auto [a, t, pos] = nums[i];
    //     // cout<<a<<sp<<t<<sp<<pos<<en;
    //     if(t==0 && watching[pos]) {
    //         watching[pos]=false;
    //         free++;
    //     } else if(t==1 && free>0 && !watching[pos]) {
    //         watching[pos]=true;
    //         free--;
    //         res++;
    //         cout<<" "<<a<<"++, "<<free<<" free"<<en;
