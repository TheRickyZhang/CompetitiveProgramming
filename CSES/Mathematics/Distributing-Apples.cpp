...
int32_t main() {
    ...
    cin>>n>>m;
    int nn = n+m;
    vi fact(nn+1); fact[0] = 1;
    fe(i, nn) fact[i] = mult(fact[i-1], i);
    vi inv(nn+1); inv[nn] = fastPow(fact[nn], MOD-2, MOD);
    repr(i, nn-1, 0) inv[i] = mult(inv[i+1], i+1);
    auto choose = [&](int n, int k) {
        return mult(fact[n], mult(inv[k], inv[n-k]));
    };
    cout<<choose(n+m-1, n-1)<<en;
}
