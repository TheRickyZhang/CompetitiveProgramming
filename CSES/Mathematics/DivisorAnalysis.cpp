...
int32_t main() {
    ...
    for(auto [d, f] : pow) {
        cnt = (cnt * (f+1)) % MOD;
        sum = (sum * (fastPow(d, f+1) - 1 + MOD)) % MOD;
        sum = (sum * fastPow(d-1, MOD-2, MOD)) % MOD;
    }
    int prod = 1, curCnt = 1;
    for(auto [d, f] : pow){
        prod = (fastPow(prod, f+1) * fastPow(fastPow(d, f*(f+1)/2), curCnt));
        prod %= MOD;
        curCnt = (curCnt * (f+1) % (MOD-1));
    }
    cout<<cnt<<sp<<sum <<sp<< prod;
