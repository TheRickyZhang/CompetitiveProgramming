...
cx_ int fpow(int a, int b) { int res = 1; a %= MOD; while (b > 0) { if (b & 1) res = res * a % MOD; a = a * a % MOD; b >>= 1; } return res; } cx_ int inv(int x) { return fpow(x, MOD-2); }
...
int32_t main() {
    ...
    int res = 0;
    fe(i, 4) {
        res = add(res, fpow(2, ceil(n*n, 4 / gcd(i, 4))));
    }
    res = mult(res, inv(4));
    cout<<res<<en;
}
