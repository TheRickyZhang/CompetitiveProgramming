...
#define vvd vv<double>
vvd operator*(const vvd& a, const vvd& b) {
    int n = a.size();
    vvd res(n, v<double>(n));
    f(i, n) f(k, n) f(j, n) res[i][j] += a[i][k] * b[k][j];
    return res;
}
vvd operator^(vvd a, int p) {
    int n = a.size();
    vvd curr(n, v<double>(n, 0));
    f(i, n) curr[i][i] = 1;
    while(p > 0) {
        if(p & 1) {
            curr = curr * a;
...
int32_t main() {
    ...
    f(x, 8) {
        f(y, 8) {
            ...
            for(auto [dy, dx] : dirs) {
                int ny = y + dy, nx = x + dx;
                if(check(ny, nx, 8, 8)) good.pb({ny, nx});
            }
            double p = 1.0 / good.size();
            for(auto [ny, nx] : good) {
                m[to(y, x)][to(ny, nx)] += p;
            }
        }
