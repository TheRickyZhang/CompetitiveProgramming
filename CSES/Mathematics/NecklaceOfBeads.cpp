...
struct bout{tpl_<tn_ T> bout& operator<<(T x){if cx_(is_integral_v<T>){int y=x;if(y==0){cout<<'0';return *this;}if(y<0){cout<<'-';y=-y;}string s;while(y){s.pb('0'+(y&1));y>>=1;}reverse(all(s));cout<<s;}else cout<<x;return *this;}} bout;
...
tpl_<tn_ Graph> tuple<vi,vi,vi> getAdj(Graph &adj,int a=0){int n=adj.size();vi par(n),dep(n),sz(n,0);
    function<void(int,int,int)>dfs=[&](int u,int p,int d){par[u]=p,dep[u]=d,sz[u]=1;
        for(auto &x:adj[u]){ int v=[&](){if cx_(std::is_same_v<std::decay_t<decltype(x)>,int>)return x;else return x.ff;}();
...
cx_ int pow(int a, int b, int mod = MOD) { int res = 1; a %= mod; while (b > 0) { if (b & 1) res = res * a % mod; a = a * a % mod; b >>= 1; } return res; } cx_ int inv(int x, int m = MOD) { return pow(x, m-2, m); }
cx_ int add(int a, int b, int m = MOD) { return (a+b < m ? a+b : a+b-m); } cx_ int mult(int a, int b, int m = MOD) { return (a*b < m ? a*b : a*b % m); } cx_ int ceil(int num, int den) { return num >= 0 ? (num + den - 1) / den : num / den; }
...
pair<vi, vi> initFact(int n) { vi fa(n+1), ifa(n+1); fa[0] = 1; fe(i, n) fa[i] = mult(fa[i-1], i); ifa[n] = pow(fa[n], MOD-2, MOD); repr(i, n-1, 0) ifa[i] = mult(ifa[i+1], i+1); return {fa, ifa}; }
...
// Example of Burnside's Lemma (Dihedral group)
int32_t main() {
    setIO();
    while(true) {
        cin>>n;
        ...
        int res = 0;
        fe(i, n) {
            res = add(res, pow(3, gcd(i, n))); // Rotations
        }
        // Reflections
        if(n & 1) res = add(res, mult(n, pow(3, ceil(n, 2))));
        else {
            res = add(res, mult(n/2, pow(3, n/2+1)));
            res = add(res, mult(n/2, pow(3, n/2)));
        }
        res = mult(res, inv(2*n));
        cout<<res<<en;
    }
}
