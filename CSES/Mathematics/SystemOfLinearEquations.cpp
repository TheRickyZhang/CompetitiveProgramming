...
int32_t main() {
    ...
    vi cols(m);
    f(i, m) cols[i] = i;
    auto swapColumn = [&](int i, int j) {
        swap(cols[i], cols[j]);
        f(k, n) {
            swap(a[k][i], a[k][j]);
        }
    };
    // Subtract x times the first row from the second
    auto subtractRow = [&](int x, int co, int y) {
        f(j, m+1) {
            a[y][j] = ((a[y][j] - co * a[x][j]) % MOD + MOD) % MOD; // resolve negatives;
        }
    };
    f(i, n) {
        bool found = false;
        for(int j = i; j < m; j++) {
            ...
            int inverse = inv(a[i][i]);
            f(k, m+1) {
                a[i][k] = mult(a[i][k], inverse);
            }
            for(int k = i+1; k < n; k++) {
                subtractRow(i, a[k][j], k);
            }
            found = true;
            break;
        }
    ...
    repr(i, n-1, 0) {
        int it = 0;
        while (it < m && a[i][it] == 0) it++;
        if(it == m) continue;
        res[it] = a[i][m];
        for (int j = it+1; j < m; j++) {
            res[it] = (res[it] - mult(a[i][j], res[j])) % MOD;
            if (res[it] < 0) res[it] += MOD;
        }
    }
    vi orderedRes(m, 0);
    f(i, m) {
        orderedRes[cols[i]] = res[i];
    }
    cout<<orderedRes<<en;
}