...
int32_t main() {
    // Not listed here BUT the explicit grundy calculation is:
    // g(0) = g(1) = g(2) = 0;
    // g(n) = mex(g(a) xor g(b) | a+b = n);
    // We can notice that this is cyclic order 3 by observation
    setIO();
    vi dp(2001, 0);
    // n^2 dp - only empirically known for large n that g(n) never 0
    vb used(2002, false);
    rep(i, 3, 2000) {
        used.assign(2002, false);
        rep(j, 1, (i-1)/2) {
            used[dp[j] ^ dp[i-j]] = true;
        }
        rep(j, 0, 2001) {
            if(!used[j]) {
                dp[i] = j; break;
    ...
    f(i, t) {
        cin>>n;
        if(n > 2000) cout<<"first"<<en;
        else {
            cout<<(dp[n] > 0 ? "first" : "second")<<en;
        }
    }
}
