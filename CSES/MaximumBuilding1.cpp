...
int32_t main() {
    ...
    // Apply the solution of maximum building to each row
    int res = 0;
    repr(i, n-1, 0) {
        vi a(m);
        f(j, m) a[j] = i - prev[i][j];
        vi heights(m, 0);
        stack<int> s;
        f(j, m) {
            while(!s.empty() && a[s.top()] >= a[j]) s.pop();
            heights[j] += j - (s.empty() ? -1 : s.top());
            s.push(j);
        }
        s = stack<int>();
        repr(j, m-1, 0) {
            while(!s.empty() && a[s.top()] >= a[j]) s.pop();
            heights[j] += (s.empty() ? m : s.top()) - j;
            s.push(j);
        }
        f(j, m) {
            ckmx(res, a[j] * (heights[j]-1)); // Note we double count the current index
        }
    }
    cout<<res<<en;
}
