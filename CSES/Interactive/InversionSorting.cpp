...
int32_t main() {
    setIO();
    cin>>n;
    // Assume everything < i is sorted relatively,
    int prev = measure(1, 1);

    auto insert = [&](int i, int j) {
        rev(i, j);
        if(i+1 <= j) prev = rev(i+1, j);
    };
    for(int i = 2; i <= n; i++){
        // Go through math. It might seems more intuitive to use cnt(i-1) since those are already sorted but this
        // Works better, in that we know all cnt(i) states get flipped, so we cn subtract the difference and divide by 2
        int next = measure(1, i);
        int pos= (cnt(i) - (next - prev)) / 2;
        insert(i - pos, i);
    }
}