...
int32_t main() {
    ...
    repr(i, log2(n), 0) {
        int ii = 1<<i;
        str s(n, '0');
        for(int l = 0; l * ii < n; l++) {
            int r = min(n-1, l*ii + ii - 1);
            if(l % 2 == 0) {
                rep(j, l*ii, r) s[j] = '1';
            }
        }
    ...
    vi ans(n);
    // The permutation happens in reverse compared to what I thought
    for(int k = 0; k < dp.size(); ++k) {
        if(!dp[k].empty()) ans[ dp[k][0] ] = k + 1;
    }
    cout<<"!";
    f(i, n) cout<<ans[i]<<sp;
    cout<<endl;
