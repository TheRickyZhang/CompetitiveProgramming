...
int32_t main(){
    ...
    f(i,n) if(!vis[i]){
        int u=i;
        // mark the whole chain until we hit a visited node
        while(!vis[u]){
            vis[u]=true;
            u=a[u];
        }
        // if we closed a cycle back to i, count its length
        if(u==i){
            int cnt=1, v=a[i];
            while(v!=i){
                ++cnt;
            ...
            int x=cnt;
            // factor cnt into primes, record max exponent
            for(int p=2; p*(ll)p<=x; ++p) if(x%p==0){
                int c=0;
                while(x%p==0) x/=p, ++c;
                ckmx(mp[p],c);
            }
            if(x>1) ckmx(mp[x],1LL);
        }
    }
    // build result = ∏ p^e mod
    ll ans=1;
    for(auto &pr: mp){
        ll b=pr.first, pw=1;
        int e=pr.second;
