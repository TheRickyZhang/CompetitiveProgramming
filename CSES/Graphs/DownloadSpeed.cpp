...
int32_t main() {
    setIO();
    cin>>n>>m;
    int s = 0, t = n-1; // Note these could be different but hardcoded here
    vv<edge> adj(n);
    f(i, m) {
    ...
    function<int(int, int)> dfs = [&](int u, int f) {
        ...
        for(auto& i = pos[u]; i < adj[u].size(); ++i) {
            ...
            if(cap <= 0 || dist[v] != dist[u]+1) continue; // Remember this!
            int w = dfs(v, min(f, cap));
            if(w == 0) continue; // Remember this!
            f -= w, cap -= w;
            curr += w, adj[v][rev].cap += w;
            if(f == 0) break; // Remember this!
        }
        return curr;
    };
    int flow = 0;
