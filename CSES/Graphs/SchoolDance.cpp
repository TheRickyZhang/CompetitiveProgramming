...
int32_t main(){
    ...
    int n,m,k;cin>>n>>m>>k;
    vvi g(n);
    f(i,k){int u,v;cind>>u>>v;g[u].pb(n+v);}
    int N = n + m;
    vi match(N, -1), dist(n);
    int distNil;
    auto bfs = [&](){
        queue<int> q; distNil = INF;
        f(u,n)
            if(match[u] < 0) dist[u] = 0, q.push(u);
            else            dist[u] = INF;
        while(!q.empty()){
            int u = q.front(); q.pop();
            if(dist[u] >= distNil) continue;
            for(int v : g[u]){
                int w = match[v];
                if(w < 0)           distNil = dist[u] + 1;
    ...
    function<bool(int)> dfs = [&](int u){
        for(int v : g[u]){
            int w = match[v];
            if((w<0 && distNil==dist[u]+1) || (w>=0 && dist[w]==dist[u]+1))
                if(w<0 || dfs(w)){
                    match[u]=v; match[v]=u;
                    return true;
                }
