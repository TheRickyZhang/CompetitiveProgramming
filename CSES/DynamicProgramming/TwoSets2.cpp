...
#define fe(i, to) for (ll i = 1; i <= (to); ++i)
...
int main() {
    ...
    cin>>n;
    vll nums(n+1); f(i, n+1) nums[i] = i;
    ll target = accumulate(all(nums), 0);
    if(target % 2 == 1) {
        cout<<0<<en; return 0;
    }
    target /= 2;
    vll dp(target+1, 0); dp[0]=1;
    fe(x, n) {
        for(ll i=target; i>=x; --i) {
            dp[i] = add(dp[i], dp[i-x]);
        }
    }
    int res = mult(dp[target], invmod(2));
    cout<<res<<en;
}
