...
// This is the euler tour + segment tree variation
...
tpl_<tn_ T> struct BIT     { int n; v<T> t, nums; T z; function<T(T, T)> c;   // All 0-indexed
    ...
    void set(int i, T x) {T diff = x - nums[i];nums[i] = x; for (i += 1; i <= n; i += (i & -i)) t[i] += diff; }
    T query(int i) { T res = z; for (i += 1; i > 0; i -= (i & -i)) res = c(res, t[i]); return res; }
    T query(int l, int r) { return query(r) - query(l-1); }
};
template <typename T> ostream& operator<<(ostream& os, const Segtree<T>& seg) { function<void(int,int,int,int)> pt=[&](int i,int a,int b,int d){if(a>b)return;os<<string(d*2,' ')<<"["<<a<<","<<b<<"]: "<<seg.t[i]<<"\n";if(a!=b){int m=(a+b)/2;pt(2*i,a,m,d+1);pt(2*i+1,m+1,b,d+1);}};os<<"Segment Tree:\n";pt(1,0,seg.n-1,0);return os; }
template <typename T> ostream& operator<<(ostream& os, const BIT<T>& bit) { os<<"BIT:\n";for(int i=1;i<=bit.n;++i)os<<"Idx "<<(i-1)<<": "<<bit.t[i]<<" ([ "<<(i-(i&-i))<<","<<i-1<<" ])\n";return os; }
...
int32_t main() {
    ...
    // Important - we double count the presence of all non-leaf nodes in v, the euler traversal of the graph
    // Intuitively - compare it to only begin/end arrays. Then [start[u], start[v]] would not contain any potential ancestors
    vi start(n, 0), depth(n, 0), nodes(2*n - 1, 0);
    int t=0;
    ...
    function<void(int, int, int)> dfs = [&](int u, int p, int d) {
        ...
        for(int v : adj[u]) {
            if(v==p) continue;
            dfs(v, u, d+1);
            nodes[t++] = u; // NOTE: this is inside the loop!
        }
    };
    ...
    // cout<<nodes<<en;
    // cout<<start<<en<<depth<<en;

    int SENTINEL = 2*n - 1;
    depth.pb(INF); // Very nuanced - since our comparison is not a minimum "value", we need an index that stores sentinel = INF
    nodes.pb(SENTINEL);
    // Be very careful with initialization here;
    Segtree<int> tree(nodes.size(), SENTINEL, [&](int a, int b) {
    ...
    f(i, k) {
        ...
        // cout<<min(l, r)<<sp<<max(l, r)<<en;
        cout<<tree.query(min(l, r), max(l, r)) + 1<<en;
    }
}
