...
vi z(str& s) {
    ...
    int l, r=0;
    for(int i=1; i <n; ++i) {
        // If l<=i<=r, can leap back to z[i-l] or remaining r-i+1
...
int main() {
    ...
    vi res = z(s);
    f(i, n) {
        if(i+res[i]==n) cout<<i<<" "; // NOTE: May be unintuitive but since z[i] is the length to end, n-z[i] = length of prefix
    }
    cout<<n<<en;
}
