...
// Doing a DFS on time-based queries
int32_t main() {
    setIO();
    int q;
    cin>>n>>q;
    vi a(n); read(a);
    vvi adj(q);
    // Note that solutions sometimes have a sentinel 0, where last[1] = 0;
    // BUT this is the only connection to 0, as all other k > 1 must have been created with a copy -> last[k] = copy operation
    // So we can maintain 0-indexing and just ignore when last[i] = -1;
    vi last(q, -1);
    vvi queries(q);

    // Even though the k arrays grow, we also need to keep track of the cnt ourselves
    int cnt = 0; // We start with one array but remember, 0-indexed!
    f(i, q) {
        cin>>t;
        if(t == 1) {
            int k, pos, x; cind>>k>>pos; cin>>x;
            if(last[k] != -1) adj[last[k]].pb(i);
            last[k] = i;
            queries[i] = {t, k, pos, x};
        } else if(t == 2) {
            int k, l, r; cind>>k>>l>>r;
            if(last[k] != -1) adj[last[k]].pb(i);
            last[k] = i;
            queries[i] = {t, k, l, r};
        } else {
            // Perhaps unintuitively, we are NOT adding 2 edges per split
            // But instead saying that two future nodes refer to this one
            int k; cind>>k;
            if(last[k] != -1) adj[last[k]].pb(i);
            last[k] = i;        // Here
            last[++cnt] = i;    // Here
    ...
    fvii dfs = [&](int u, int p) {
        ...
        const auto& op = queries[u];
        if(op[0] == 1) {
            int pos = op[2], x = op[3];
            int prev = bit.query(pos, pos);
            // cout<<"update "<<pos<<sp<<prev<<sp<<x<<en;
            bit.update(pos, x);
            search();
            bit.update(pos, prev);
        } else if(op[0] == 2) {
            ...
            search(); // Query 3 already in structure of the tree
        }
    };
    dfs(0, -1);
    // cout<<res<<en;
    f(i, q) {
        if(res[i] != -INFL) cout<<res[i]<<en;
    }
}
