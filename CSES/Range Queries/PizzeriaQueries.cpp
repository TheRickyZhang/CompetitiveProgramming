...
int32_t main() {
    ...
    cin>>n>>k;
    Segtree<int> up(n, INF, [&](int a, int b) {
        return min(a, b);
    });
    Segtree<int> down(n, INF, [&](int a, int b) {
        return min(a, b);
    });
    vi nums(n); f(i, n) cin>>nums[i];
    f(i, n) { // Account for var contribution splitting |a-b| -> a-b, b-a
        up.add(i, nums[i] + i+1);
        down.add(i, nums[i] -i-1);
    }
    // cout<<up<<en<<down<<en;
    f(i, k) {
        cin>>t;
        if(t==1) {
            ...
            int a; cin>>a;
            int u = -a + up.query(a-1, n-1);
            int d = a + down.query(0, a-1);
            cout<<min(u, d)<<en;
        }
    }
}
