...
int32_t main() {
    ...
    Segtree<node> tree(n, [&](node a, node b) {
        return node{max({a.mx, b.mx, a.r + b.l}), a.x + b.x, max(a.l, a.x + b.l), max(b.r, b.x + a.r)};
    });
    f(i, n) tree.update(i, node{a[i], a[i], a[i], a[i]});
    f(i, k) {
        int l, r; cind>>l>>r;
        cout<<tree.query(l, r).mx<<en;
    }
}