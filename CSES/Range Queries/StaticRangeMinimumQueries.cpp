...
void print(const vi& v) { fora(x, v) {cout<<x<<" ";} cout<<"\n"; }
...
tpl_<tn_ T> struct Segtree      { int n; v<T> t, nums; fn<T(T, T)> c;
    ...
    Segtree(int sz, fn<T(T, T)> combine, const v<T>& init = {}, T zero = T()) : n(sz), t(4 * sz, zero), nums(sz, zero), c(move(combine)) { if (!init.empty()) { nums = init; build(1, 0, n-1); } }
...
// Divide and conquer static range queries

// OFFLINE (typically fastest, n log n)
// int32_t main() {
//     setIO();
//     cin>>n>>k;
//     vi nums(n); read(nums);
//     vi A(k), B(k); // For these problems, to reduce naming collisions using a, b for bounds is best
//     f(i, k) cind>>A[i]>>B[i];
//     vi ids(k); iota(all(ids), 0); // We store bound in [left, right] and dynamically update the ids
//     vi res(k, INFL);
//
//     vi ll(n), rr(n); // These store the answers
//     fn<void(int, int, vi&)> solve = [&](int l, int r, vi& v) {
//         if(v.empty()) return;
//         int m = (l+r) / 2;
//         if(l == r) {
//             fora(id, v) res[id] = nums[l];
//             return;
//         }
//         ll[m] = nums[m]; repr(i, m-1, l) ll[i] = min(nums[i], ll[i+1]);
//         rr[m+1] = nums[m+1]; rep(i, m+2, r) rr[i] = min(nums[i], rr[i-1]);
//         vi next[2];
//         for(int id : v) {
//             int a = A[id], b = B[id];
//             if(a <= m && m+1 <= b) {
//                 res[id] = min(ll[a], rr[b]);
//             } else {
//                 next[a >= m+1].pb(id);
//             }
//         }
...
// ONLINE (Clever bit manipulation. About 4x slower than offline, comparable to sparse table)
int32_t main() {
    cin>>n>>k;
    vi nums(n); read(nums);
    int ln = log2(n) + 1;
    vvi res(n, vi(ln, 0));
    vi mask(n); // mask[i] has 1<<j when right side for lvl j
    fn<void(int, int, int)> solve = [&](int l, int r, int d) {
        if(l == r) return;
        int m = (l+r) / 2;
        res[m][d] = nums[m]; repr(i, m-1, l) res[i][d] = min(nums[i], res[i+1][d]);
        res[m+1][d] = nums[m+1]; rep(i, m+2, r) res[i][d] = min(nums[i], res[i-1][d]);
        rep(i, m+1, r) mask[i] ^= 1<<d;
        solve(l, m, d+1);
        solve(m+1, r, d+1);
    };
    solve(0, n-1, 0);
    f(i, k) {
        int a, b; cind>>a>>b;
        if(a==b) cout<<nums[a]<<en;
        else {
            int d = __builtin_ctz(mask[a] ^ mask[b]);
            cout<<min(res[a][d], res[b][d])<<en;
        }
    }
}