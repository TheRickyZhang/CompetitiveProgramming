...
int32_t main() {
    ...
    repr(i, m-2, 0) {
        ckmn(next[i], next[i+1]);
    }
    // cout<<next<<en;
    vvi up = binaryJump(next, m);
    f(i, k) {
        int l, r; cind>>l>>r;
        auto lit = lower_bound(all(values), l), rit = upper_bound(all(values), r);
        if(lit == values.end() || rit == values.begin()) {
            cout<<0<<en; continue;
        }
        l = lit - values.begin(), r = rit - values.begin() - 1;
        int res = 0;
        repr(j, ln-1, 0) {
            if(up[l][j] <= r) {
                l = up[l][j];
