...
void solve() {
  ...
  vi b(a.begin(), a.begin() + m);
  vi c(a.begin() + m, a.end());
  // print(b); print(c);
  Segtree<int, decltype(_min)> pre(m, _min, b, INFL);
  Segtree<int, decltype(_min)> suf(n-m, _min, c, INFL);

  auto calc = [&](){
    if(n % 2 == 0) {
      // left can steal
      int lres = l + max(0LL, suf.query(0, n-m-1) - pre.query(0, m-1));
      int rres = l+r - lres;
      cout << lres << " " << rres << en;
    } else {
      // right can steal
      int rres = r + max(0LL, pre.query(0, m-1) - suf.query(0, n-m-1));
      int lres = l + r - rres;
      cout << lres << " " << rres << en;
    }
  };
