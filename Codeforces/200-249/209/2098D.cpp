...
void solve(){
    ...
    vvi mids(k); // Each of k midpoint possibilities (ids)
    map<pii,int> mp; // Map midpoints -> id
    int M=0;
    f(i,k){
        auto [x1,y1]=a[i];
        auto [x2,y2]=a[i+1];
    ...
    f(i,k){
        ...
        if(keep.size()<active.size()){
            ...
            f(m, dp.size()) if(dp[m]){
                int c=dp[m], nm=0;
                f(t, keep.size()) if(m&(1<<keep[t])) nm|=1<<t;
                dpp[nm]=add(dpp[nm], c);
            }
            dp=move(dpp);
            active=move(na);
        ...
        for(int md: start[i]) active.pb(md);
        if(dp.size() < (1<<active.size())) dp.resize(1<<active.size());
        ...
        f(m, dp.size()) if(dp[m]){
            int c=dp[m];
            for(int j: cand) if(! (m&(1<<j))){
                dpp[m|(1<<j)] = add(dpp[m|(1<<j)], c);
            }
        }
        dp=move(dpp);
    }
