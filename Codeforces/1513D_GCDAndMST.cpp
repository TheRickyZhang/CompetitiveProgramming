...
void solve() {
    ...
    vpii sorted;
    f(i, n) if(a[i] < k) sorted.pb({a[i], i});
    sort(all(sorted));

    DSU d(n);
    int res = 0;
    // Process using value expansion
    for (auto [x, i] : sorted) {
        for (int j = i-1; j >= 0; j--) {
            if(a[j] % x != 0 || d.same(j, i)) break;
            d.merge(i, j);
            res += x;
        }
        for (int j = i+1; j < n; j++) {
    ...
    //     while(r < n && b[r] % x == 0) r++;
    //     rep(j, l+1, r-1) {
    //         if(i==j || (abs(j-i)==1 && x >= k) || d.par(i) == d.par(j)) continue;
    //         edges.push({b[i], i, j});
    //     }
    // };
    // int cnt = 0;
    ...
    //     }
    // }
    // // Note that cnt should never be < n-1;
    // assert(cnt == n-1);
    // cout<<base+res<<en;
}
