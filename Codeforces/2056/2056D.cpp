...
void solve() {
    cin>>n;
    vvi pre(10, vi(n, 0)); // Use -1, 1 based on x values
    vvi freq(10, vi(n, 0)); // How many times x appears
    vi nums(n);
    f(j, n) {
        int x; cin>>x; x--;
        nums[j] = x;
    ...
    f(i, 10) {
        f(j, n-1) {
            pre[i][j+1] += pre[i][j]; // Accumulate diff; bad iff cnt<=x equals cnt>x, or pre[l, r] == 0
            freq[i][j+1] += freq[i][j];
        }
    }
    // cout<<pre<<en;
    ...
    // NOTE: This works theoretically, but there is a much simpler way. We do a "delayed" update, where
    // the freq map is ONLY UPDATED from pointers i...j when encountering nums[j] == x. Basically, lazily propagate the map count
    f(i, 10) {
        map<int, BIT<int>> mp;
        mp[0] = BIT<int>(n, 0, ad);
        mp[0].add(0, 1);
        ...
        for(int j = 0; j < n; j++) {
            ...
            int f = freq[i][j];
            if(mp.count(p)) {
                // if(mp[p].query(f-1)) cout<<"+"<<mp[p].query(f-1)<<"at "<<j<<en;
                bad += mp[p].query(f-1); // We want to query how many have < freq so net freq >= 1
            } else {
                mp[p] = BIT<int>(n, 0, ad);
            }
            mp[p].add(f, 1);
