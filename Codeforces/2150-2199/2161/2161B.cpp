...
void solve(){
  ...
  int mni = INF, mnj = INF, mxi = -INF, mxj = -INF;
  for(auto [i, j] : b) {
    cmx(mxi, i); cmx(mxj, j);
    cmn(mni, i); cmn(mnj, j);
  }
  if(mxi - mni == 1 && mxj - mnj == 1) {
    quit("YES");
  }

  int x = b[0].ff+b[0].ss, y = b[0].ff-b[0].ss;
  bool q, r, s, t;
  q = r = s = t = true;
  for(auto [i, j] : b) {
    int xx = i+j, yy = i-j;
    // cout<<xx<<sp<<yy<<endl;
    if(xx != x && xx != x+1) q = false; 
    if(xx != x && xx != x-1) r = false; 
    if(yy != y && yy != y+1) s = false; 
    if(yy != y && yy != y-1) t = false; 
  }
  if(q || r || s || t) {
    cout<<"YES"<<endl;
  } else {
