...
void solve() {
  ...
  vi par(n, -1), dep(n, 0);
  // sum = sum of a[u], dp = sum of formula. Sum -> how much change when moved by dep?
  vi sum(n, 0), dp(n, 0);
  // max additional gain from original dp[u], since possibility of best
  vi bestDiff(n, 0);
  vi res(n, 0);
  ...
  function<void(int,int)> dfs1 = [&](int u, int p) {
    ...
    // mx, 2nd mx pattern
    int mx1 = 0, mx2 = 0, mxu = -1;
    int bestSum1 = 0, bestSum2 = 0;
    int bestChild1 = -1;
    // max diff among children v
    int maxdiffv = 0;
    ...
    for(int v : adj[u]) {
      ...
      // best inside a subtree
      cmx(maxdiffv, bestDiff[v]);
    ...
    // best excluding mxu depth
    int bestNoU = 0;
    if(bestChild1 != -1) {
      if(bestChild1 != mxu) bestNoU = bestSum1;
      else bestNoU = bestSum2;
    }

    // cross-child moves reduce to moving an entire child subtree:
    // deepest -> mx2, non-deepest -> mx1
    int diff = 0;
    if(mxu != -1) diff = sum[mxu] * mx2;
    int diffo = bestNoU * mx1;

    bestDiff[u] = max({0LL, maxdiffv, diffo, diff});
    res[u] = dp[u] + bestDiff[u];
  };
  dfs1(0, -1);
