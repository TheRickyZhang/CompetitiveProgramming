...
auto ad = [](int a, int b) {return a+b;}; auto sub = [](int a, int b) {return a-b;}; auto sortifa = [](const pii& a,const pii& b) {if(a.ff == b.ff) return a.ss > b.ss; return a.ff < b.ff;};
...
int32_t main() {
    ...
    vi ifa(n+1, 1); ifa[n] = fastPow(fa[n], MOD-2, MOD);
    repr(i, n-1, 0) ifa[i] = mult(ifa[i+1], i+1);
    vi derange(n+1, 0);
    derange[0] = 1;
    fe(i, k) derange[i] = derange[i-1] * i + (i & 1 ? -1 : 1);
