...
#define tn_ typename
#define f(i, to) for (long long i = 0; i < (to); ++i)
...
// // cont 0...x
// ll countOnes(ll x, int i) {
//     ll sz = 1LL << (i + 1);        // 2^(i+1)
//     ll full = (x+1)/sz, rem = (x+1)%sz;
//     ll res = full * (1LL<<i);     // each full cycle has 2^i ones at bit i
//     ll extra = rem - (1LL<<i);
//     if (extra > 0) res += extra;
//     return res;
// }
//
// // Returns how many in [l..r] have i-th bit set
// ll numOnes(ll l, ll r, int i){
//     return countOnes(r, i) - countOnes(l - 1, i);
// }
//
// // Per-bit approach for maximum triple XOR mask
// void solve() {
//     ll l, r; cin>>l>>r;
//     ll n = r - l + 1;
//     ll tot=0;
...
//             q.push(a);
//         }
//         else if(ones >= 2 && zeros >= 1) {
//             ll a = q.top(); q.pop();
//             ll b = q.top(); q.pop();
//             a |= (1<<i);  b |= (1<<i);
//             q.push(a), q.push(b);
//         }
//         if((ones >= 2 && zeros >= 1) ||( ones >= 1 && zeros >= 2)) {
//             tot += 1<<(i+1);
//         }
//     }
//     vi nums;
...
//     f(i, 3) {
//         if(nums[i] < l) {
//             for(ll j = l-nums[i]; i<r-nums[i]+1; ++i) {
//                 ll x = nums[i]+j;
//                 if(x >= l && !seen.count(x)) nums[i] = x;
//             }
//         }
//         seen.insert(nums[i]);
//     }
//     cout<<nums[0]<<sp<<nums[1]<<sp<<nums[2]<<en;
//
//     ll xx=0;
//     f(i, 3) {
//         f(j, i) {
...
//         }
//     }
//     cout<<"xor: "<<xx<<sp<<tot<<en; // Compare expected vs actual
//     cout<<en;
// }
