...
void solve() {
  ...
  vpii b;
  unordered_map<vi, int, vHash> mp;
  function<int(vi&, vi&)> calc = [&](vi& used, vi& chosen) {
    cout<<"calculating"<<sp<<used<<en;
    int res = 0;
    if(all_of(all(used), [](int x) { return x == 1; })) {
      res = 1;
    }
    ...
    return mp[chosen] = res;
  };
  vi used(n, false);
  vi chosen(n, false);
  int res = calc(used, chosen);
  for(auto [k, cnt] : mp) {
    for(int x : k) cout<<x;
    cout<<en<<cnt<<en;
  }
