...
int countFactorizations(int x, int mn, const unordered_map<pii, ll, pairHash>& mp) {
    if(x==1) return 1;
    pii key = {x, mn};
    if(mp.count(key)) return mp[key];
    ll res = 0;
    for(int i = mn; i*i <= x; ++i) {
        if(x % i == 0) res += countFactorizations(x / i, i, mp);
    }
    if(x >= mn) res++;
    return mp[key] = res;
}
