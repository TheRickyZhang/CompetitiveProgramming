...
void solve() {
    cin>>n>>m;
    vi a(n), b(m);
    f(i, n) cin>>a[i]; sort(all(a));
    f(i, m) cin>>b[i]; sort(all(b));
    pq<int> q1, q2;
    f(i, a.size()/2) {
        q1.push(a[n-i-1] - a[i]);
    }
    ...
    int aa = n & 1, bb = m & 1;
    int rema = 0, remb = 0, curr=0;
    vi res;
    while (q1.size()+rema > 0 && q2.size()+remb > 0 && (q1.size() || q2.size())) {
        int x = q1.top(), y = q2.top();
        if(q2.empty() || x > y || (x == y && (2*q1.size() + rema > 2*q2.size() + remb))) {
            curr += x;
            res.push_back(curr);
            q1.pop();
            if(--bb < 0) {
