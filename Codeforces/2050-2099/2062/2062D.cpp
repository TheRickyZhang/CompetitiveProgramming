...
void solve() {
    ...
    autotree dfs = [&](int u, int p) {
        ...
        nums[u] = min(right[u], max(left[u], mx));
    };
    dfs(0, 0);

    int res = 0;
    // Count min (number passing through node 1)
    autotree dfss = [&](int u, int p) {
        for(int v : adj[u]) {
            if(v==p) continue;
            dfss(v, u);
            res += max(0LL, nums[v]-nums[u]);
        }
    };
    dfss(0, 0);
    ...
    //         if(v==p) continue;
    //         dfs(v, u);
    //         int diff = left[v]-right[u];
    //         if(diff > 0) {
    //             res += diff;
    //             // left[u] += diff; right[u] += diff;
    //         }
    //         ckmn(mn, left[v]);
    //         ckmx(mx, left[v]);
    //         left[u] = right[u] = left[v];
    //     }
    //     // if(mn != INFL || mx != 0) res += mx-mn;
    // };
    // dfs(0, 0);
    // cout<<base+res<<en;
}
