...
struct Dinic {
    ...
    int N;
    v<Edge> ed; vvi adj;  vi lvl, ptr;

    void init(int n) {
        N = n;  ed.clear(); adj.assign(n, vi());
    }
    // Add edge from u to v with capacity 'cap' and reverse capacity 'rcap'
    void ae(int u, int v, int cap, int rcap = 0) {
        assert(min(cap, rcap) >= 0);
        adj[u].pb(ed.size()); ed.pb({v, 0, cap});
        adj[v].pb(ed.size()); ed.pb({u, 0, rcap});
    }

    // BFS to build level graph; returns true if t is reachable.
    bool bfs(int s, int t) {
        lvl.assign(N, -1); ptr.assign(N, 0);
...
void solve() {
    ...
    v<string> mat(n);
    f(i, n) cin >> mat[i];
    // Compute row and column XOR (as quaternary values)
    vi rowX(n,0), colX(m,0);
    f(i, n) f(j, m) {
        int x = mat[i][j]-'0';
        rowX[i] ^= x;
        colX[j] ^= x;
    }
    // For each row/column, if XOR≠0 we need an odd number of changes.
    // So, set demand = 1 if error, 0 if ok.
    vi rowDem(n), colDem(m);
    f(i, n) rowDem[i] = (rowX[i] != 0);
    f(j, m) colDem[j] = (colX[j] != 0);

    // Build bipartite flow network.
    // Nodes: 0..n-1 = rows, n..n+m-1 = columns, S = n+m, T = n+m+1.
    int S = n + m, T = S + 1, totNodes = T + 1;
    Dinic dinic; dinic.init(totNodes);
    f(i, n) if(rowDem[i]) dinic.ae(S, i, 1);
    f(j, m) if(colDem[j]) dinic.ae(n+j, T, 1);
    f(i, n) f(j, m) {
        // Each cell (i,j) gives an edge from row i to col (n+j) with cost 1.
        dinic.ae(i, n+j, 1);
    }
    int minChanges = dinic.maxFlow(S, T);
    cout << minChanges << "\n";

    // Reconstruct the min cut.
    vb vis(totNodes, false);
    ...
    // Mark cell (i,j) as "to change" if row i is reachable but column j is not.
    vvi change(n, vi(m, 0));
    f(i, n) f(j, m) {
        if(vis[i] && !vis[n+j]) change[i][j] = 1;
    }

    // Now, we must assign new values in changed cells so that each row and column becomes 0.
    // For each row, let unchanged cells contribute their original XOR.
    f(i, n) {
        int cur = 0;
        f(j, m) if(!change[i][j]) cur ^= (mat[i][j]-'0');
        bool fixed = false;
        f(j, m) if(change[i][j]) {
            if(!fixed) {
                // Set one changed cell so that (cur XOR new) = 0 ⇒ new = cur.
                mat[i][j] = char('0' + cur);
                fixed = true;
            } else {
                // Other changed cells can be set arbitrarily (choose 0).
                mat[i][j] = '0';
            }
        }
    }
    // For each column, do a similar adjustment.
    f(j, m) {
        int cur = 0;
        f(i, n) if(!change[i][j]) cur ^= (mat[i][j]-'0');
        bool fixed = false;
        f(i, n) if(change[i][j]) {
            if(!fixed) {
                // Force column XOR to 0 by setting new value = cur.
                mat[i][j] = char('0' + cur);
                fixed = true;
            }
            // Others remain as set.
        }
    }
    f(i, n) cout << mat[i] << "\n";
}
