...
void solve() {
    ...
    vvi dp(n, vi(4, 0));
    fe(i, n-1) {
        if(s[i]=='0') dp[i][0] = dp[i][1] = 1;
        else dp[i][1] = dp[i][2] = 1;
    }
    cout<<dp<<en;
    repr(i, n-2, 0) {
        int carry = (dp[i+1][2] + dp[i+1][3]);
        int prev = (dp[i+1][0] + dp[i+1][1]);
        dp[i][3] = (dp[i][3] * prev) + (dp[i][2] * carry);
        dp[i][2] = (dp[i][2] * prev) + (dp[i][1] * carry);
        dp[i][1] = (dp[i][1] * prev) + (dp[i][0] * carry);
        dp[i][0] = (dp[i][0] * prev);
        f(j, 3) dp[i][j] %= MOD;
    }
    cout<<dp<<en;

    int base = fastPow(2, n-1, MOD);
    int num = (n-1) * base + (dp[1][2]+dp[1][3]);
    int dem = base;
    num %= MOD, dem %= MOD;
    // cout<<num<<sp<<dem<<en;
    cout<<(num * fastPow(dem, MOD-2, MOD)) % MOD<<en;
}
