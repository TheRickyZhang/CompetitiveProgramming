...
void solve(){
    ...
    function<bool(int,int,vi&,vb&)> getPath = [&](int u, int x, vi& path, vb& vis) -> bool {
        vis[u] = true;
        if(u == x){ path.pb(u); return true; }
        for(int v : adj[u])
            if(!vis[v] && getPath(v, x, path, vis)){
                path.pb(u);
                return true;
            }
        return false;
    };
    vi path; vb vis(n, false);
    getPath(a, b, path, vis);
    reverse(all(path));
 
    vb on(n, false);
    for(int v : path) on[v] = true;
 
    function<void(int,int,vb&,vi&)> dfs = [&](int u, int p, vb &on, vi &part){
        part.pb(u);
        for(int v : adj[u]){
    ...
    f(i, path.size()-1){
        ...
        vi part;
        for(int v : adj[u])
            if(!on[v]) dfs(v, u, on, part);
        reverse(all(part));
        for(auto v : part) res.pb(v);
        res.pb(u);
    }
    int u = path.back();
    vi part;
    ...
    if(res.size() != n){ cout<<-1<<en; return; }
    for(auto v : res) cout<<v+1<<sp;
    cout<<en;
}
