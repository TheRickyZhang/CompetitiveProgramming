...
void solve() {
    int lll, rrr;
    cin>>n>>lll>>rrr;
    vi a(n+1); f(i, n) cin>>a[i+1];
    vi pre(n+1, 0LL);
    f(i, n) pre[i+1] = pre[i] ^ a[i+1];
    function qu = [&](int x, int y) {
        return pre[y]-pre[x];
    };
    if(lll <= n) {
        cout<<a[lll]<<en; return;
    }
    function<int(int, int)> calc = [&](int l, int r) {
        if(r < l) {
            cout<<"HUH"<<en; return 0LL;
        }
        if(l<= n) {
            if(r <= n) return qu(l, r);
            else return qu(l, n) ^ calc(n+1, r);
        }
        int left = (l & 1) ? calc(1, l/2) : 0;
        int right = (r % 2 == 0) ? calc(1, r/2) : 0;
        // cout<<left<<sp<<right<<en;
        return left ^ right;
    };
    cout<<calc(lll, rrr)<<en;
}
