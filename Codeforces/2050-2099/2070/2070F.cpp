...
int a[21][MAX_MASK], f_arr[21][MAX_MASK];
int in_arr[MAX_MASK], b_arr[MAX_MASK], c_arr[MAX_MASK], ans_arr[MAX_MASK];
...
int32_t main(){
    ...
    // Read friend preferences and count them by bitmask.
    rep(i, 1, m) {
        char s[23];
        cin >> s;
        int len = strlen(s), mask = 0;
        f(j, len)
            mask |= (1 << (s[j] - 'A'));
        in_arr[mask]++;
    }

    // Read pizza sizes and compute b_arr and c_arr for every subset.
    f(i, n) {
        int t;
        cin >> t;
        f(mask, N) {
    ...
    // For subsets with no odd pizza, adjust answer
    f(mask, N) {
        if (c_arr[mask] == 0)
            ans_arr[b_arr[N - 1 - mask]] -= in_arr[mask];
    }

    f(mask, N) a[c_arr[mask]][mask] = in_arr[mask];
    rep(i, 0, n) FWT_OR(a[i], N);

    // Convolve: for every two groups (with odd counts i and j), combine their contributions.
    rep(i, 0, n) rep(j, 0, n - i) f(mask, N) f_arr[i + j][mask] += a[i][mask] * a[j][mask];
    rep(i, 0, n) FWT_IOR(f_arr[i], N);

    f(mask, N) ans_arr[b_arr[N - 1 - mask]] += f_arr[c_arr[mask]][mask];
    rep(k, 0, b_arr[N - 1]) cout << ans_arr[k] / 2 << " ";
    cout << en;
