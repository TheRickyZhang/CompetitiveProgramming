...
void solve() {
    ...
    f(i, m2) {
        ...
        if(adj[u][v] || adj[v][u]) good[u] = good[v] = true;
    }
    // Basically run dijkstra dynamically without constructing the whole graph
    mpq<iii> q;    q.push({0, x, y});
    vvi dist(n, vi(n, INFL));
    dist[x][y] = 0;
    while(!q.empty()) {
        auto [d, x, y] = q.top(); q.pop();
        if(d > dist[x][y]) continue;
        auto& aa = adj1[x], bb = adj2[y];
        for(int a : aa) {
            for(int b : bb) {
                int nd = d + abs(b-a);
                if(nd < dist[a][b]) {   // Note this should prevent infinite when a == b
                    dist[a][b] = nd;
                    q.push({nd, a, b});
                }
            }
