...
vector<pii> dirs={{1,0},{0,1},{0,1},{-1,0}}; map<char, int> dirMap={{'D',0},{'R',1},{'N',2},{'W',3}}; auto check=[](auto y,auto x,auto m,auto n){return y>=0&&y<m&&x>=0&&x<n;};
void solve() {
    cin>>n>>m;
    vvi grid(n, vi(m));
    vi a(n), b(m);
    ...
    f(i, n) f(j, m) {
        cin>>grid[i][j];
        a[i] += grid[i][j], b[j] += grid[i][j];
    }
    vpii visited; visited.pb({0, 0});
    f(i, s.size()) {
        visited.pb(dirs[dirMap[s[i]]]);
    }
    vi xinc(n, 0), yinc(m, 0);
    f(i, visited.size()-1) {
        visited[i+1] = visited[i+1] + visited[i];
    }
    f(i, visited.size()) {
        auto [x, y] = visited[i];
        xinc[x]++, yinc[y]++;
    }
    // cout<<xinc<<en<<yinc<<en;
