...
#define adj vector<array<array<int,2>,2>>
...
void solve() {
    ...
    f(i, n-1) {
        ...
        f(j, n) {
            ...
            if (d==0) goodr[i][0][0] = goodr[i][1][1] = 0;
            else if (d==1) goodr[i][0][1] = 0;
            else if (d==-1) goodr[i][1][0] = 0;
        }
    }
    ...
    f(i, n-1) {
        ndp = {INFL, INFL};
        f(x, 2) f(y, 2) if (goodr[i][x][y]) {
            ndp[y] = min(ndp[y], dp[x] + a[i+1] * y);
        }
        dp = ndp;
    }

    int cr = min(dp[0], dp[1]);

    adj goodc(n-1);
    f(j, n-1) {
        goodc[j] = {{{1,1},{1,1}}};
        f(i, n) {
            int d = grid[i][j] - grid[i][j+1];
            if (d==0) goodc[j][0][0] = goodc[j][1][1] = 0;
            else if (d==1)  goodc[j][0][1] = 0;
            else if (d==-1) goodc[j][1][0] = 0;
        }
    }
    ...
    f(j, n-1) {
        ndp = {INFL, INFL};
        f(x, 2) f(y, 2) if (goodc[j][x][y]) {
            ckmn(ndp[y], dp[x] + b[j+1] * y);
        }
        dp = ndp;
    }

    int cc = min(dp[0], dp[1]);
    if(cr == INFL || cc == INFL) {
        cout<<-1<<en;
    } else {
        cout<<cr + cc<<en;
