...
#define str string
    int N = 100000; int MOD=998244353; constexpr int INF=1e9; constexpr int INFL=0x3f3f3f3f3f3f3f3f; constexpr auto en = "\n"; constexpr auto sp = " ";
...
void solve() {
    ...
    f(i, n) {
        ...
        f(j, sz[i]) {
            int pos = a[i][j];
            int l = lower_bound(all(a[i]), pos-d) - a[i].begin();
            int r = upper_bound(all(a[i]), pos+d) - a[i].begin();
            if(l < j) {
                dpp[i][l] = (dpp[i][l] + dp[i][j]) % MOD;
                dpp[i][j] = (dpp[i][j] - dp[i][j] + MOD) % MOD;
            }
            if(j+1 < r) {
                dpp[i][j+1] = (dpp[i][j+1] + dp[i][j]) % MOD;
                if(r < sz[i]) dpp[i][r] = (dpp[i][r] - dp[i][j] + MOD) % MOD;
            }
        }
        f(j, sz[i]-1) dpp[i][j+1] = (dpp[i][j+1] + dpp[i][j]) % MOD;

        if(i+1 >= n) continue;
        f(j, sz[i]) {
            int pos = a[i][j];
            int l = lower_bound(all(a[i+1]), pos-d+1) - a[i+1].begin();
            int r = upper_bound(all(a[i+1]), pos+d-1) - a[i+1].begin();
            int addVal = (dp[i][j] + dpp[i][j]) % MOD;
            if(l < sz[i+1]) dp[i+1][l] = (dp[i+1][l] + addVal) % MOD;
            if(r < sz[i+1]) dp[i+1][r] = (dp[i+1][r] - addVal + MOD) % MOD;
        }
        f(j, sz[i+1]-1) dp[i+1][j+1] = (dp[i+1][j+1] + dp[i+1][j]) % MOD;
    }
    // cout<<dp<<en;
    // cout<<dpp<<en;
    int res = 0;
    f(j, sz[n-1]) res = (res + dp[n-1][j] + dpp[n-1][j] + MOD) % MOD;
    cout<<res<<en;
}
