...
//     while(cl < r) {
//         int a = cl - l;
//         if((a < x/2 && fff[cl-'a']+1 >= freqq[cl-'a']/2) || (a >= x/2 && s[cl] != s[l + x - a - 1])) {
//             ckmn(res, r - cl + 1);
//             break;
//            }
//         seen.insert(s[cl]);
...
//     while(cr > l) {
//         int a = r - cr;
//         if((a < x/2 && fff[cl-'a']+1 >= freqq[cl-'a']/2) ||
//            (a >= x/2 && s[cr] != s[l + a])) {
//             ckmn(res, cr - l + 1);
//             break;
//            }
...
int32_t main(){
 ...
 while(t--){
  ...
  int m=n/2; // left: [0, m-1], right: [m, n-1]
  vi tot(26,0); f(i,n) tot[s[i]-'a']++;
  vi need(26,0); f(c,26) need[c]=tot[c]/2;
  // Build prefix frequency array.
  vvi freq(n+1,vi(26,0));
  f(i,n){freq[i+1]=freq[i]; freq[i+1][s[i]-'a']++;}
  auto query=[&](int l,int r)->vi{
    if(l>r)return vi(26,0);
    vi res(26,0);
    f(c,26) res[c]=freq[r+1][c]-freq[l][c];
    return res;
  };
  // Fixed counts in left half (indices 0..m-1) after removing [l,r]
  auto leftFixed=[&](int l,int r)->vi{
    int L=0,R=m-1;
    int rl=max(l,L), rr=min(r,R);
    vi totL=query(L,R), rem=query(rl,rr);
    return totL - rem;
  };
  // Fixed counts in right half (indices m..n-1)
  auto rightFixed=[&](int l,int r)->vi{
    int L=m,R=n-1;
    int rl=max(l,L), rr=min(r,R);
    vi totR=query(L,R), rem=query(rl,rr);
    return totR - rem;
  };
  // Check mirror consistency: for i in [0, m-1] if both i and n-1-i are fixed, they must agree.
  auto mirrorConsistent=[&](int l,int r)->bool{
    for(int i=0;i<m;i++){
      bool fixedL = (i<l || i>r);
      int j = n-1-i;
      bool fixedR = (j<l || j>r);
      if(fixedL && fixedR && s[i]!=s[j]) return false;
    }
    return true;
  };
  // Try candidate removal of length L.
  auto canFix=[&](int L)->bool{
    for(int l=0;l<=n-L;l++){
      int r = l+L-1;
      vi lf = leftFixed(l,r), rf = rightFixed(l,r);
      bool ok=true;
      f(c,26) { if(lf[c]>need[c] || rf[c]>need[c]) {ok=false; break; } }
      if(!ok) continue;
      if(!mirrorConsistent(l,r)) continue;
      return true;
    }
    return false;
  };
