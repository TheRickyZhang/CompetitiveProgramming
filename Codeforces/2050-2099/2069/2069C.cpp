...
struct node {
    int l, r, a, b, tot;
    node(int l, int r, int a, int b, int tot) : l(l), r(r), a(a), b(b), tot(tot) {}
};
void solve() {
    int n; cin>>n;
    ...
    for (int x : nums) {
        if (x==1) o++;
        else if (x==2) dp = (2*dp+o) % MOD;
        else if (x==3) res = (res + dp) % MOD;
    }
    cout<<res<<en;
    ...
    //     return node(a.l, b.r, a.a + b.a, a.b + b.b, a.tot + b.tot + a.l * b.r * (pre[b.r]-pre[a.l + 1]));
    // }
    // int l=0, r=n-1;
    // while(l < r+1) {
    //     if(nums[l]==2) l++;
    ...
    //         if(nums[l]==1 && nums[r]==3) {
    //             res = (res + fastPow(2, cnt) - 1) % MOD;
    //             cout<<l<<sp<<r<<sp<<cnt<<en;
    //         }
    //         l++;
    //     }
    // }
