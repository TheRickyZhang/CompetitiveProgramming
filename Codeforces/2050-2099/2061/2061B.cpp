...
void solve() {
    ...
    f(i, n-1) {
        if(nums[i] == nums[i+1]) {
            ...
            if(i > 0 && i < n-2 && 2 * x > nums[i+2] - nums[i-1]) {
                cout<<x<<sp<<x<<sp<<nums[i-1]<<sp<<nums[i+2]<<en; return;
            }
            if(!q.empty()) {
                auto [dd, aa, bb] = q.top();
                if(2 * x > dd) {
                    cout<<x<<sp<<x<<sp<<nums[aa]<<sp<<nums[bb]<<en; return;
                }
            }
