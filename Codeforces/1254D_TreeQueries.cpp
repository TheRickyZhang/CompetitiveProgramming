...
struct LazySegtree {            // NOTE this "apply" passes in [l, r] by default! ap and cmp use {old, new op ... }
    ...
    LazySegtree(int sz, C cmb, Ap _ap, Cmp _cmp, const v<T>& i={}, const T& z=T()) :n(sz), t(4*sz), nums(sz), ops(4*sz), c(move(cmb)),
...
int32_t main(){
  ...
  auto ap  = [](int x,int p,int l,int r){return x + p*(r-l+1);};

  vi dep(n,0), par(n,-1), sz(n,1);
  vi levelpar(n,-1);
  levelpar[0]=0;
  for(int v:adj[0]) levelpar[v]=v;

  auto dfs = [&](auto&& self,int u,int p)->void{
    par[u]=p;
    auto& lu=adj[u];
    if(p!=-1) lu.erase(find(all(lu),p));
    if(levelpar[u]==-1) levelpar[u]=levelpar[p];
    f(i,lu.size()){
      int v=lu[i];
      dep[v]=dep[u]+1;
      self(self,v,u);
      sz[u]+=sz[v];
      if(sz[v]>sz[lu[0]]) swap(lu[0],lu[i]);
    }
  };
  dfs(dfs,0,-1);
  ...
  // heavy child size mapped to position of parent: next_sz[pos[u]] = sz[heavy(u)]
  vi next_sz(n,0);
  f(u,n){
    if(!adj[u].empty()){
      int hc=adj[u][0];
  ...
  auto ad = [](int x, int y){ return add(x, y); };
  BIT<int, decltype(ad)> FX(n, ad), FXSZ(n, ad), FXNSZ(n, ad); // ΣX, ΣX*sz, ΣX*sz[heavy]
  vi X(n,0);

  auto suma = [&](int u, auto& F)->int{
    int r=0;
    while(root[u]!=root[0]){
      int h=root[u];
      r=add(r, F.query(pos[h], pos[u]));
      u=par[h];
    }
    r=add(r, F.query(pos[0], pos[u]));
    return r;
  };

  auto pathb = [&](int u)->int{
    int r=0;
    while(root[u]!=root[0]){
      int h=root[u];
      r=add(r, FXNSZ.query(pos[h], pos[u]-1));
      int p=par[h];
      r=add(r, mult(FX.query(pos[p], pos[p]), sz[h]));
      u=p;
    }
    r=add(r, FXNSZ.query(pos[0], pos[u]-1));
    return r;
  };
   ...
   * Every update (u, x):
   * a[u] += x
   * if p in (u, root) path, a[p] += x * (sz[r] - sz[p] + 1) / n 
   * else if u is root, handle separately (behaves weirdly?)
   * else any other v node, a[v] += x * (sz[v]) / n
   * Let's factor out 1/n to apply to the end, and use units of sz[u] to also compute at end.
   *  - u -> subtract 1 unit
   *  - p -> subtract 2 units, add 1 (sz[r]+1) unit
   */
  int cnt=0;            // sum X_v*sz[v]
  int rootres=0;        // equals cnt
  ...
  f(_,k){
    ...
    if(type==1){
      ...
      X[u]=add(X[u],x);
      cnt=add(cnt, mult(x, sz[u]));
      rootres=cnt;

      FX.add(pos[u], x);
      FXSZ.add(pos[u], mult(x, sz[u]));
      FXNSZ.add(pos[u], mult(x, next_sz[pos[u]]));
    }else{
      int u; cin>>u; u--;
      if(u==0){
        int num=rootres;
        // cout<<num<<en;
        cout<<mult(num, inv(n))<<en;
      }else{
        int S1=suma(u, FX);
        int S2=suma(u, FXSZ);
        int H =pathb(u);
        int num=cnt;
        num=add(num, mult(n, S1));
        num=sub(num, S2);
        num=sub(num, H);
        // cout<<num<<en;
        cout<<mult(num, inv(n))<<en;
