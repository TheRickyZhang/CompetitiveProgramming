...
int t, k, n, m;
vi primes = sieveList(1e6);
void solve() {
    cin>>n;
    int m = fstTrue(1LL, ll(primes.size()), [&](int x) {
        // Don't forget that n vertices = # edges + 1!
        if(x & 1) return (x * (x+1) / 2) + 1 >= n;
        else return (x*x/2 + 1) + 1 >= n;
    });
    // cout<<m<<en;
    vvpii adj(m);
    vpii edges;
    ...
    f(i, m) {
        f(j, i) {
            if(m % 2 == 0 && (j & 1) && i==j+1) {
                continue; // Remove edges {1, 2}, {3, 4} for even deg
            }
            adj[i].pb({j, id});
            adj[j].pb({i, id});
            id++;
    ...
    fvi dfs = [&](int u) {
        while(!adj[u].empty()) {
            auto [v, ei] = adj[u].back(); adj[u].pop_back();
            if(evis[ei]) continue; evis[ei] = true;
            dfs(v);
        }
        path.pb(u);
    };
    dfs(0); // Since we have 0 as only one with more deg in the even case
    reverse(all(path));
    assert(path.size() >= n);
    f(i, n) cout<<primes[path[i]]<<sp;
    cout<<en;
}
