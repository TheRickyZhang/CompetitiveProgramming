...
int getLCA(const vvi& up,const vi& dep, int u, int v, const vi& tin = {}, const vi& tout = {}) {
    if (!tin.empty()) {if (tin[u] <= tin[v] && tout[v] <= tout[u]) return u; if (tin[v] <= tin[u] && tout[u] <= tout[v]) return v;}
...
// 0-init default, [i] = value with greatest pivot at 1<<i
using basis = array<int, 32>;
...
int32_t main() {
    ...
    // Returns true if a[i] is linearly independent to the basis, and inserts it
    auto add = [&](basis& b, int i) {
        int x = reduce(b, a[i]);
        if(x == 0) return false;
        b[31 - __builtin_clz(x)] = x;
        return true;
    };
    fn<void(int, int)> dfs = [&](int u, int p) {
        basis b{};
        // The crucial part here is that pivotIndices is constructed from u->root (NOT root->u), so we init a new basis and add the new u first.
        if(add(b, u)) pivotIndices[u].pb(u);
        for(int i : pivotIndices[p]) {
            if(add(b, i)) pivotIndices[u].pb(i);
        }
        for(int v : adj[u]) {
            if(v == p) continue;
            dfs(v, u);
    ...
    int k; cin>>k;
    cout<<pivotIndices<<en;
    f(_, k) {
        int u, v, x; cind>>u>>v; cin>>x;
        int w = getLCA(up, dep, u, v, tin, tout);
        basis b{};
        // Handle w exactly once
        for(int i : pivotIndices[u]) {
            if(dep[i] < dep[w]) break;
            add(b, i);
        }
        for(int i : pivotIndices[v]) {
            if(dep[i] <= dep[w]) break;
            add(b, i);
        }
        // If x is linearly independent it should get added
        cout<<(reduce(b, x) == 0 ? "YES" : "NO")<<en;
    }
}