...
freq getCnt(const str& s) {
  freq cnt{};
  fora(c, s) cnt[c - '0']++;
  return cnt;
}

bool sub(freq& a, const freq& b) {
  fd(d) if(a[d] < b[d]) return false;
  fd(d) a[d] -= b[d];
  return true;
}
...
void solve() {
  ...
  fe(y, 9 * n) {
    str suf = calc(y);
    if(suf.size() >= n) continue;

    freq rem = all;
    if(!sub(rem, getCnt(suf))) continue;
