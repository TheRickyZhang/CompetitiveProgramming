...
void solve() {
  ...
  // classic pattern, best together (asme subtree) or apart (diff subtrees)
  vi best1(m), best2(m);

  fvii dfs = [&](int u, int mx) {
    int w = (tr[u].r - tr[u].l + 1) * (mx - tr[u].mx);

    int mx1 = 0, mx2 = 0, same = 0;
    for(int v : tr[u].ch) {
      dfs(v, tr[u].mx);
      same = max(same, best2[v]);
