...
int k, n, m;
void solve() {
  // n as the color count makes more sense
  cin >> m >> n;
  int sz = n*m;

  v<pii> cards(sz);
  f(i, sz) cin >> cards[i].ff;
  f(i, sz) cind >> cards[i].ss;
  ...
  // fixes[c] = ranks that are forced to be rank-clued for color c
  // plus sentinel 0 at the front
  vvi fixes(n);
  vb bad(m+1, false); // bad[r] => rank r cannot be finished color-wise globally
  ...
  f(c, n) {
    ...
    fixes[c].insert(fixes[c].begin(), 0);
  }

  // dp[i] = min length after ranks [1...i] (= res + 1)
  vi dp(m+1, INFL);
  dp[0] = 0;

  Segtree<int, decltype(_min)> st(m+1, _min, {}, INFL);
  st.update(0, 0);

  // ptr[c] = index of largest fix < current i
  vi ptr(n, 0);
  ...
  for(int i = 1; i <= m; i++) {
    ...
    vi bounds;
    f(c, n) {
      while(ptr[c]+1 < fixes[c].size() && fixes[c][ptr[c]+1] < i) {
        ptr[c]++;
      }
      bounds.pb(fixes[c][ptr[c]]);
    }
    sort(all(bounds));

    int prev = 0;

    // If previous color milestone j lies in a bucket where exactly (n-k)
    f(k, n) {
      int r = min<int>(bounds[k], i-1);
      if(prev <= r) {
        int best = st.query(prev, r);
        cmn(dp[i], best + (i-1) + (n-k));
      }
      prev = max(prev, r+1);
    }

    if(prev <= i-1) {
      int best = st.query(prev, i-1);
      cmn(dp[i], best + (i-1));
    }
