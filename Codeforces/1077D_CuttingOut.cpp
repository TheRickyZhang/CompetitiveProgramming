...
int main() {
    ...
    // Input: How many copies we are making
    // Output: Whether we have enough to fill arrays of at least size k
    auto check = [&](int x) {
        int cnt = 0;
        for(auto [a, b] : freq) {
            cnt += b/x; // How many copies
    ...
    int res = l-1; // Max copies
    int appended = 0;
    for (auto it = freq.rbegin(); it != freq.rend() && appended < k; ++it) {
        auto [val, count] = *it;
        int canUse = count / res;
        canUse = min(canUse, k - appended);
