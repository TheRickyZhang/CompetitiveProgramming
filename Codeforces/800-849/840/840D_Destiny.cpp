...
int32_t main() {
    ...
    int q; cin>>n>>q;
    vi a(n); // Values
    vvi b(n); // {values, positions}
    f(i, n) {
        cind>>a[i];
        b[a[i]].pb(i);
    }
    vi c(n); f(i, n) c[i] = i; // Most frequent values
    sort(all(c), [&](int x, int y) {
        return b[x].size() > b[y].size();
    });
    vi freq(n+1, 0);
    f(i, q) {
        int l, r, k; cind>>l>>r; cin>>k;
        int x = (r-l+1) / k; // Must have > x times
        int res = INFL;
        if(r-l < 1000) {
            rep(i, l, r) {
                if(++freq[a[i]] > x) ckmn(res, a[i]);
            }
            rep(i, l, r) freq[a[i]]--;
        } else {
            f(i, n) {
                int val = c[i];
                if(b[val].size() <= x) break;
                int cnt = upper_bound(all(b[val]), r) - lower_bound(all(b[val]), l);
                if(cnt > x) ckmn(res, val);
            }
        }
