...
int32_t main() {
    ...
    cin>>n>>k;
    v<iii> edges;
    vi a(n), b(n); // Distance from 1->u, u->1;
    vvpii adj(n);
    f(i, n-1) {
    ...
    vi in(n), out(n), dep(n);
    auto anc = [&](int u, int v) { // True if u is ancestor of v
        return in[u] <= in[v] && out[v] <= out[u];
    };
    int t = 0;
    function<void(int, int, int, int)> dfs = [&](int u, int p, int d, int lvl) {
        dep[u] = lvl, in[u] = t++; a[u] = d;
        for(auto [v, w] : adj[u]) {
            if(v != p) dfs(v, u, d + w, lvl+1);
        }
        out[u] = t - 1; // Inclusive time interval
    };
    dfs(0, -1, 0, 0);
    f(i, n-1){
        auto& [u,v,w] = edges[i];
    ...
    // NOTE that we store a[i] + b[i] for all in[], out[] here.
    // We update b[] in the array and then in the tree by doing add(in[i], in[i], diff)
    // Then to get a[i] see geta. It may seem easier to store a[i], b[i] separately but this saves on unnecessary information
    v<node> init(n);
    f(u, n) init[ in[u] ] = node{a[u] + b[u]};
    LazySegtree<node, int> tree(n,
        [&](node a, node b){ return node{min(a.x, b.x)}; },
        [&](node a, int o, int l, int r) {
            return node{a.x + o};
        },
        ad,
        init
    ...
    // cout<<tree.t<<en;
    auto geta = [&](int u) {
        return tree.query(in[u], in[u]).x - b[u]; // Since we store
    };
    f(_, k) {
        cin>>t;
        if(t == 1) {
            int i, nw; cind>>i; cin>>nw;
            auto& [u, v, w] = edges[i];
            if(i >= n-1) {
                tree.add(in[u], in[u], nw - b[u]);
                b[u] = nw;
            } else {
                tree.add(in[v], out[v], nw-w);
                w = nw;
            }
        } else {
            int u, v; cind>>u>>v;
            if(anc(u, v)) {
                // cout<<"anc"<<sp<<u<<sp<<v<<sp;
                cout<<geta(v) - geta(u)<<en;
            } else {
                cout<<geta(v) - geta(u) + tree.query(in[u], out[u]).x <<en;
            }
        }
    }
}
