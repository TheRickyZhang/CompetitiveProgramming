...
struct Node {
    int a, b, x; // leftover 0, leftover 1, total 01 cnt
...
Node combine(Node a, Node b) {
    int mn = min(a.a, b.b);
    return Node(a.a + b.a - mn, a.b + b.b - mn, a.x + b.x + mn);
}
ostream& operator<<(ostream& os, const Node& x) {  os<<x.a<<sp<<x.b<<sp<<x.x<<en; return os; }
...
int32_t main() {
    ...
    n = s.size();
    vector<Node> nodes;
    f(i, n) nodes.pb(Node(s[i]));

    Segtree<Node> tree(n, Node(), combine, nodes);
    ...
    f(_, k) {
        ...
        if(t==1) {
            ...
            if(nodes[i].a == 1) {
                ...
                nodes[i].a = 1, nodes[i].b = 0;
            }
            tree.update(i, nodes[i]);
            // cout<<tree<<en;
        } else {
            int l, r; cin>>l>>r; l--; r--;
            int matches = tree.query(l, r).x;
            cout<<r-l+1 - 2*matches<<en;
        }
    }
}
