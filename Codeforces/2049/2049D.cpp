...
void solve() {
    ...
    vvll dp(n+1, vll(m, INFL));
    dp[0][0] = 0;
    f(i, n) {
        f(shift, m) {
            vll temp(m, INFL);
            f(j, m) temp[j] = dp[i][j] + grid[i][(j+shift) % m] + k * shift; // Account for the shift
            f(j, m) ckmn(temp[j], temp[(j+m-1) % m] + grid[i][(j+shift) % m]);
            // f(j, m) ckmn(temp[j], temp[(j+m-1) % m] + grid[i][(j+shift) % m]);
            f(j, m) ckmn(dp[i+1][j], temp[j]);
        }
    }
    cout<<dp[n][m-1]<<en;
    ...
    //     vi pre(2*m+1, 0);
    //     f(j, 2*m) {
    //         pre[j+1] = grid[i][j % m] + pre[j];
    //     }
    //     cout<<pre<<en;
    //
    //     fe(x, m) { // Lengths
    //         dp[i][x-1] = pre[x]-pre[0];
    //         rep(j, x+1, 2*m) {
    //             // cout<<"j "<<j<<sp<<"x "<<x<<sp<<pre[j]-pre[j-x] + k *(j-x)<<en;
    //             ckmn(dp[i][x-1], pre[j]-pre[j-x] + k *(j-x));
    //         }
    //     }
    // }
    // cout<<dp<<en;
    // // Figure out best combination of horizontal movements;
    // vi cost(m, INF); cost[0] = 0; //
    // vi ccost(m, INF);
    // f(i, n) { // Row
    //     f(j, m) {
    //         for(int k = m-1; k >= j; --k) {
    //             ckmn(ccost[k], cost[k-j] + dp[i][j]);
    //         }
    //     }
    //     cout<<"cost for row "<<i<<": "<<ccost<<en;
    //     cost = ccost;
    //     ccost.assign(m, INF);
    // }
    // cout<<cost<<en;
