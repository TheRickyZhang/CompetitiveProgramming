...
int32_t main() {
    ...
    vi ifa(n+1); ifa[n] = fastPow(fa[n], MOD-2, MOD);
    ...
    // How many players left, hp lost for everyone
    vvi dp(n+1, vi(x+1, 0));
    dp[n][0] = 1;
    repr(i, n, 2) {
        f(j, x) {
            ...
            // Note how this min is necessary and correct. It is only possible to take <= x damage, and since our
            // fastPow also uses nj it does not over count
            int nj = min(x, j + i-1);
            int power = 1;
            repr(k, i, 0) {
                dp[k][nj] += mult(dp[i][j], mult(choose(i, k), power));
                dp[k][nj] %= MOD;
                power = mult(power, nj-j);
            }
        }
    }
    int res = 0;
    rep(i, 0, x) res = (res + dp[0][i]) % MOD;
    cout<<res<<en;
    ...
    //     return 0;
    // }
    // // How much the "winner" has if resulting in win
    // rep(i, n, x) {
    //     res += mult(n, fastPow(i-1, n-1));
    //     res %= MOD;
    // }
    // cout<<(fastPow(x, n) - res + MOD) % MOD<<en;
}
