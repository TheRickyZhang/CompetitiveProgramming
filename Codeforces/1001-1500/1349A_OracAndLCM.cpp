...
int ceil(int num, int den) { return num >= 0 ? (num + den - 1) / den : num / den; } int fastPow(int a, int b, int mod = MOD) { int res = 1; while (b > 0) { if (b & 1) res = res * a; a = a * a; b >>= 1; } return res; } int fastLog(int a, int b) {int res = 0; int p = 1; while (p <= b / a) { p *= a; res++;} return res; }
...
int32_t main() {
    ...
    int x = a[0], y = a[1];
    int g = gcd(x, y);
    tie(x, y) = make_pair(g, x/g*y);
    rep(i, 2, n-1) {
        y = gcd(y, a[i]);
        g = gcd(x, y);
        tie(x, y) = make_pair(g, x/g*y); // If the new y is smaller than the x, swap
    }
    cout<<y<<en;

    // Can do 2nd minimum exponent per p much faster!
    // int res = 1;
    // vi primes = sieveList(n+1);
    // for(int p : primes) {
    //     vi b(n, 0);
    //     f(i, n) {
    //         int x = a[i];
