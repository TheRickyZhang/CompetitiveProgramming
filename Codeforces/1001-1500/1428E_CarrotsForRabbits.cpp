...
tpl_<tn_ T>        using v = vector<T>;  using ll=long long;  using pii=pair<int,int>;  using pll=pair<ll,ll>;  using t3=tuple<ll,ll,ll>;                using t4=tuple<int,int,int,int>;
...
int main() {
    ...
    auto val = [&](ll len, ll cuts) {
        ll a = len / cuts;
        ll b = len - a*cuts;
        return (cuts-b)*a*a + b*(a+1)*(a+1);
    };
    f(i, n) {
        ll x; cin>>x; res += x*x;
        q.push({val(x, 1)-val(x, 2), x, 2});
    }
    f(i, k-n) {
        auto [c, a, b] = q.top(); q.pop();
        res -= c;
        q.push({val(a, b)-val(a, b+1), a, b+1});
    }
    cout<<res<<en;
}
...
//         ll tot = 0;
//         for(ll x : nums) tot += ((x/i)*i*i + (x%i) * (x%i));
//         cout<<i<<sp<<tot<<en;
//         res = min(res, tot);
//     }
//     cout<<res<<en;
