...
int32_t main() {
    ...
    vi p(n); f(i, n) cind>>p[i];
    map<int, int> mp;
    f(i, n) mp[p[i]] = i;
    vi a(m); f(i, m) cind>>a[i];
    vi pos(*max_element(all(a))+1);
    f(i, m) {
        pos[a[i]] = mp[a[i]];
    }
    auto prev = [&](int x) {
    ...
    vi b(m); // Largest index l < i where a[l] = p[j-1]
    vi lastPos(m, -1);
    f(i, m) {
        // cout<<"last pos"<<i<<sp<<pos[a[i]]<<sp<<prev(pos[a[i]])<<en;
        b[i] = lastPos[p[prev(pos[a[i]])]];
        lastPos[a[i]] = i;
    }
    // cout<<b<<en;
