...
int32_t main() {
    ...
    // l, r, 0/1 for which color
    int dp[n][n][2];
    f(i, n) f(j, n) dp[i][j][0] = dp[i][j][1] = INF;
    f(i, n) dp[i][i][0] = dp[i][i][1] = 0;

    // Need to use BFS-like propagation with lr -> l-1, r+1. CANNOT use x, l, r=l+x

    f(r, n) {
        for(int l = r; l >= 0; --l) {
            f(k, 2) {
                int c = k==0 ? nums[l] : nums[r];
                if(l > 0) ckmn(dp[l-1][r][0], dp[l][r][k] + (c != nums[l-1]));   // Expanding to include l-1 in range
                if(r < n-1) ckmn(dp[l][r+1][1], dp[l][r][k] + (c != nums[r+1])); // Expanding to include r+1 in range
            }
        }
    }
