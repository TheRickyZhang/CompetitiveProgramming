...
void solve() {
    ...
    if(cnt==0 || cnt==n) cout<<*max_element(all(a))<<en;
    else cout<<res-cnt+1<<en;
    // vi b(n+2, 0), c(n+2, 0);
    // int curr = 0;
    // f(i, n) {
    //     b[i+1] = b[i];
    ...
    //     if(a[i] & 1) c[i+1]++;
    // }
    // auto lcnt = [&](int l, int r) {
    //     return b[r+1]-b[l];
    // };
    // auto rcnt = [&](int l, int r) {
    //     return c[r+1]-c[l];
    // };
    // int res = *max_element(all(a));
    // int tot = accumulate(all(a), 0LL);
    //
    // // cout<<b<<en<<c<<en;
    // f(i, n-1) {
    //     if((a[i]+a[i+1]) % 2 == 1) {
    //         // cout<<i<<sp<<i+1<<sp<<tot - rcnt(0, i-1) - lcnt(i+2, n-1)<<en;
    //         ckmx(res, tot - rcnt(0, i-1) - lcnt(i+2, n-1));
    //     }
    // }
    // cout<<res<<en;
    ...
    // int res = *max_element(all(a));
    // int i = 0;
    // while(i+1 < n && (i > 0 || (a[i+1] + a[i]) % 2 == 1)) {
    //     a[i+1] += a[i]; ckmx(res, a[i+1]);
    //     if(a[i+1] % 2 == 0) a[i+1]--;
    //     i++;
    // }
    // ckmx(res, a[i]);
    // i = n-1;
    // while(i-1 >= 0 && (i < n-1 || (b[i-1]+b[i]) % 2 == 1)) {
    //     b[i-1] += b[i]; ckmx(res, b[i-1]);
    //     if(b[i-1] % 2 == 0) b[i-1]--;
    //     i--;
    // }
    // ckmx(res, b[i]);
