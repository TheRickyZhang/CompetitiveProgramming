...
tpl_<tn_ T, tn_ U>
struct LazySegtree {            // NOTE "apply" passes in [l, r] by default! ap and cmp use {old, new op ... }
...
void solve() {
    ...
    int res = 0;
    rep(a, 1, min(n-2, x-2)) {
        rep(b, 1, min(n/a - 1, x-a-1)) {
            int mx = lstTrue(0LL, 1000000LL, [&](int c) {
                return a*b + b*c + a*c <= n && a+b+c <= x;
            });
            assert(mx >= 0 && mx <= 1e6);
            res += mx;
        }
