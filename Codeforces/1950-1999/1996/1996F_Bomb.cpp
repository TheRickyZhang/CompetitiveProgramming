...
//     auto dfs = [&](auto&& self, int i, int L, int R, int d) -> void { if (L>R || rows>=maxR || d>maxD) return; cout<<str(d*2,' ')<<"["<<L<<","<<R<<"]: "<<t[i]<<'\n';
//         ++rows; if (L < R) {int M = (L+R)/2;self(self, 2*i,   L,  M, d+1); self(self, 2*i+1, M+1,R, d+1); } }; cout<<"Segtree\n"; dfs(1, 0, n-1, 0);}
...
//         return ceil(a.ff, a.ss) > ceil(b.ff, b.ss);
//     });
//     vi a(n), b(n);
//     f(i, n) {
//         a[i] = nums[i].ff;
//         b[i] = nums[i].ss;
//     }
//     int taken = lstTrue(0LL, ceil(a[0], b[0]), [&](int m) {
//         int x = max(0LL, a[0] - m * b[0]); // Low bound
//         int rem = k - m;
//         rep(i, 1, n-1) {
//             rem -= max(0LL, (a[i] - x) /  b[i] + 1);
...
//     pq<pii> q;
//     int res = 0;
//     int x = max(0LL, a[0] - taken * b[0]);
//     f(i, n) {
//         int cnt = max(ceil(a[i]-x, b[i]), 0LL);
//         k -= cnt;
//         int rem = a[i] - cnt*b[i];
//         res += a[i]*cnt - b[i]*calc(cnt-1); // Does a[i] + (a[i]-b[i]) + (a[i]-2*b[i]) ...
//         q.push({max(0LL, rem), b[i]});
//     }
//     while(k>0){
//         auto [v,w] = q.top(); q.pop();
//         if(v==0) break;
//         res += v; --k;
//         q.push({ max(0LL, v - w), w });
//     }
//     cout<<res<<en;
// }
//
