...
int t, k, n, m;
struct node {
    int cnt = 0, mn = INFL; // This works since we should never have negative values
};
...
void solve() {
    ...
    LazySegtree<node, int> tree(n,
        [&](node a, node b) {
            // This is the crucial logic - we DON't actively maintain the number of zeroes, instead
            // maintain mn and cnt, so we can combine easily.
            node c;
            c.mn = min(a.mn, b.mn);
            c.cnt = 0;
    ...
    auto get = [&]() {
        auto x = tree.query(0, n-1);
        return (x.mn > 0 ? 0 : x.cnt);
    };
    int res = get();
    // Iterate over splits (unwrap circle to line = unique path), determine rest from there.
    f(i, n-1) {
        for(int j : a[i]) {
            tree.add(0, n-1, 1);
            tree.add(i, j-1, -2);
