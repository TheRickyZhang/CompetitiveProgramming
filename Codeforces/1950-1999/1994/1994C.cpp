...
void solve() {
    ...
    vi dp(n+2, 0);
    repr(i, n-1, 0) {
        int j = upper_bound(all(pre), pre[i] + k) - pre.begin();
        dp[i] = dp[j] + (j-i-1);
    }
    ...
    //     }
    //     else {
    //         int j = lstTrue(prev+1, i, [&](int m){ return query(m,i)>k; });
    //         res[i] = (j >= prev+1 ? j - prev : 0);
    //     }
    //     // cout<<i<<sp<<res<<en;
    // }
    // // cout<<res<<en;
    // cout<<n * (n+1) / 2 - accumulate(all(res), 0LL) <<en;
