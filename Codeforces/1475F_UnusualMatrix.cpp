...
inline vvi mult(const vvi &A, const vvi &B) {int n=A.size(); vvi C(n,vi(n,0)); f(i,n) f(k,n) f(j,n) C[i][j]=(C[i][j]+A[i][k]*B[k][j])%MOD; return C;}
inline vvi fastPow(vvi A, int64_t p) {int n=A.size(); vvi r(n,vi(n,0)); f(i,n) r[i][i]=1; while(p){if(p&1)r=mult(r,A); A=mult(A,A); p>>=1;} return r;}
void read(vi &v) { for (auto &x : v) cin >> x; } struct cind { tpl_ <tn_ T> cind& operator>>(T &x) { cin >> x; --x; return *this; }} cind;


// Note that a greedy method is possible but has caveats. We use systematic 2SAT instead
void solve() {
    int n; cin>>n;
    vs a(n); f(i, n) cin>>a[i];
    vs b(n); f(i, n) cin>>b[i];

    auto neg = [&](int x) {
        return 4*n - x - 1;
    };
    vvi adj(4*n), radj(4*n);
    f(i, n) {
        f(j, n) {
            int x=i, y=n+j;
            // Note that this might seem redundant but ri == cj can ONLY be expressed as
            // ri OR ~cj, or ~ri OR cj. Don't worry about adj and radj being equal; trust the process
            if (a[i][j]==b[i][j]) {
                adj[neg(x)].push_back(neg(y)); adj[y].push_back(x);
                radj[neg(y)].push_back(neg(x)); radj[x].push_back(y);

                adj[x].push_back(y); adj[neg(y)].push_back(neg(x));
                radj[y].push_back(x); radj[neg(x)].push_back(neg(y));
            } else {
                adj[neg(x)].push_back(y);adj[neg(y)].push_back(x);
                radj[y].push_back(neg(x));radj[x].push_back(neg(y));

                adj[x].push_back(neg(y)); adj[y].push_back(neg(x));
                radj[neg(y)].push_back(x);radj[neg(x)].push_back(y);
            }
        }
    }
    // cout<<adj<<radj<<en;
    ...
    function<void(int)> dfss = [&](int u) {
        if(vis[u]) return; vis[u] = true;
        for(int v : radj[u]) dfss(v);
        id[u] = t;
    };
    f(i, 4*n) if(!vis[i]) dfs(i);
