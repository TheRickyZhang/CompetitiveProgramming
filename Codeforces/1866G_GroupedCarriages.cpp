...
int32_t main() {
    ...
    f(i, n) {
        ...
        int l = max(0LL, i-b[i]);
        int r = min(n-1, i+b[i]);
        v[l].pb({r, a[i]}); // This makes it easy to only apply pairs when at index l
    }
    int res = fstTrue(0LL, *max_element(all(a)), [&](int m) {
        mpq<pii> q; // {end, x}
        f(i, n) {
            // We were not able to get rid of some capacity
            if(!q.empty() && q.top().ff < i) return false;
            for(pii p : v[i]) q.push(p);
            int need = m;
            while(need > 0 && !q.empty()) {
                auto [r, x] = q.top(); q.pop();
                int use = min(x, need); // Handle cases gracefully by -= min remain
                need -= use, x -= use;
                if(x > 0) q.push({r, x});
            }
        }
