...
struct node {
    int x, a, b;
    node() : x(0), a(0), b(0) {}
    node(int i, int j, int k) : x(i), a(j), b(k) {}
    friend ostream& operator<<(ostream& os, const node& n) {
        return os<<n.x<<sp<<n.a<<sp<<n.b<<en;
    }
};
...
int32_t main() {
    ...
    str s; cin>>s;
    n = s.size();
    Segtree<node> tree(n, node(), [&](node x, node y) {
        int match = min(x.a, y.b);
        int nx = x.x + y.x + match;
        int na = x.a + y.a - match;
        int nb = x.b + y.b - match;
        return node(nx, na, nb);
    });
    f(i, n) {
        str c = string(1, s[i]);
        if(c=="(") tree.add(i, node(0, 1, 0));
        else tree.add(i, node(0, 0, 1));
    }
    // cout<<tree<<en;
