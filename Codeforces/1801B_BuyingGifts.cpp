...
void solve() {
    ...
    sort(all(bb)); multiset<int>s(all(bb)); // For keeping track of which left can adjust
    sort(all(nums), [&](pii x, pii y) {
    ...
    // KEY: lock in max price for A as a[i]. Thus, choose bj for j < i, and for all k > i ONLY maybe choosing b[i] can provide smaller diff
    int res = INF;
    f(i, n) {
        int x = nums[i].ff, y = pre[i];
        // cout<<i<<sp<<"check "<<x<<sp<<y<<en;
        // Need this special case since requirement of choosing 1
        if(i==0) {
            auto lit = leq_bound(all(s), x);
            if(lit != s.end()) ckmn(res, x - *lit);
            auto it = s.lower_bound(x);
            if(it != s.end() && *it > y) ckmn(res, *it - x);
        } else {
            ckmn(res, abs(y-x));
            if(x > y) {
                auto it = s.lower_bound(x);
