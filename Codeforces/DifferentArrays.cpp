...
template<class T> bool       ckmn(T& a, const T& b) {return b < a ? a = b, 1 : 0;}  template<class T> bool ckmx(T& a, const T& b) {return a < b ? a = b, 1 : 0;}
    int N = 10000; int MOD=998244353; constexpr int INF=1e9; constexpr int INFL=0x3f3f3f3f3f3f3f3f; constexpr auto en = "\n"; constexpr auto sp = " ";
...
//     int dpp[2*Z] = {};
//     dp[Z] = 1;
//     fe(i, n-2) { // Processing ith index
//         f(x, 2*Z) {
//             int nx = nums[i]+x;
//             if(dp[x] == 0 || nx < 0 || nx >= 2*Z) continue;
//             dpp[nx] = add(dpp[nx], dp[x]); // Add current value
//             if(nx != Z) dpp[2*Z-nx] = add(dpp[2*Z-nx], dp[x]); // Add negative by leveraging Zero offset
//         }
//         f(i, 2*Z) {
//             dp[i] = dpp[i];
//             dpp[i] = 0;
