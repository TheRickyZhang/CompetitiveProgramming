...
#define str string
    int N = 200000; int MOD=998244353; constexpr int INF=1e9; constexpr int INFL=0x3f3f3f3f3f3f3f3f; constexpr auto en = "\n"; constexpr auto sp = " ";
...
int32_t main() {
    ...
    f(_, t) {
        ...
        fe(i, n) {
            int l = i-1, r=n-i;
            if(l==0 || l > r) cout<<0<<sp;
            else {
                // Pay very close attention to values!
                // choose elements in subarray (med r reserved), ways to arrange subarray, ways to arrange whole array (counting subarray as 1 element)
                cout<<mult(choose(n-l-2, l-1), mult(fa[2*l], fa[n-2*l+1]))<<sp;
            }
        }
        cout<<en;
    }
