...
ll N = 5000; ll MOD=1000000007; constexpr int INF=1000000000; constexpr ll INFL=0x3f3f3f3f3f3f3f3f; constexpr auto en = "\n"; constexpr auto sp = " ";
...
// (UNIQUE: NEED to set -1 for INVALID)
// Still does not work - Not sufficient to just use audience for indexing, since might miss out on better team score later on
// (Use extra dimension)
// int main() {
//     int a; cin>>n>>t>>a;
//     vll aud(n);
...
//
//     vvll team(n, vll(t, 0));
//     f(i, n) f(j, t) cin>>team[i][j];
//
//     vpll best(n);
//     f(i, n) best[i] = {aud[i], i};
//     sort(rall(best)); // Has best audience contribution
//
//     vll dp(1<<t, -1); // Dp over team state (UNIQUE: NEED to set -1 for INVALID)
//     dp[0] = 0;
//     auto dpp = dp;
//
//     ll res = 0;
...
//             ll cnt = i - __builtin_popcount(mask); // 1s in mask = chosen for team
//             if(cnt < a) ckmx(dpp[mask], dp[mask] + best[i].ff); // Choose for audience
//             f(j, t) {
//                 if(mask & (1<<j)) continue;
//                 ckmx(dpp[mask | (1<<j)], dp[mask] + team[best[i].ss][j]);
//             }
//         }
//         dp = dpp;
//         fill(all(dpp), -1);
