...
set<arr> get(int r){
    ...
    set<arr> st;
    arr cnt{};
    function<void(int,bool,bool)> dfs=[&](int i,bool ti,bool stt){
        if(i==m){ if(stt) st.insert(cnt); return; }
        int mx=ti? s[i]-'0' : 9;
        f(d,mx+1){
            bool nti=ti&&d==mx;
            bool nst=stt||d;
...
int32_t main(){
    ...
    while(T--){
        ...
        f(k,11){
            if(pows[k]>n) break;
            int lim=(n - (pows[k]-1)) / pows[k];
            auto A=get(lim);
            set<arr> Braw = get((lim-9)/10);

            // build B by copying + incrementing the ’9’‐count
            set<arr> B;
            for(auto p : Braw){
                p[9]++;
                B.insert(p);
            ...
            set<arr> C;
            for(auto &p:A) if(!B.count(p)) C.insert(p);
            if(k==0){ arr z{}; z[0]=1; z[1]=1; C.insert(z); }
            for(auto p:C){ p[0]+=k; p[9]+=k; tot.insert(p); }
        }
        cout<<tot.size()<<en;
    }
    return 0;
