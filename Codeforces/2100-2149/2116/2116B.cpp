...
cx_ int N = 100000; cx_ int MOD=998244353; cx_ int INF=1e9; cx_ ll INFL=0x3f3f3f3f3f3f3f3f; cx_ auto en = "\n"; cx_ auto sp = " ";
...
void solve() {
    ...
    int apos = 0, bpos = 0;
    auto calca = [&](int i) {
        // cout<<"a"<<sp<<i<<sp<<apos<<sp<<i-apos<<sp;
        return (1LL<<a[apos]) + (1LL<<b[i-apos]);
    };
    auto calcb = [&](int i) {
        // cout<<"b"<<sp<<i<<sp<<bpos<<sp<<i-bpos<<sp;
        return (1LL<<b[bpos]) + (1LL<<a[i-bpos]);
    };
    f(i, n) {
        if(a[i] > a[apos]) apos = i;
        if(b[i] > b[bpos]) bpos = i;
        if(a[apos] > b[bpos]) {
            cout<<add(fpow(2, a[apos]), fpow(2, b[i-apos]));
        } else if(b[bpos] > a[apos]) {
            cout<<add(fpow(2, b[bpos]), fpow(2, a[i-bpos]));
        } else {
            cout<<add(fpow(2, a[apos]), fpow(2, max(b[i-apos], a[i-bpos])));
        }
        cout<<sp;
    }
    cout<<en;
