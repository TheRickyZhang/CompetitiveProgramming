...
void solve() {
    ...
    auto calc = [&](int mx) {
        int i = lower_bound(all(a), mx) - a.begin();
        // Sum of lower values
        int curr = pre[i];
        // How much extra our upper values can give
        int mov =  (pre[n] - pre[i]) - (n-i) * mx;
        // If we evenly distribute, how many each lower value has
        int full = (curr + mov) / (n-i-1);
        int part = (curr + mov) % (n-i-1);
        int temp = mov*k + (n-i-1)*tri(mx) + i*tri(full) + part*(full+1);
        ckmn(res, temp);
        cout<<res<<sp<<temp<<en;
        return temp;
    };
    int _ = fstTrue(a.front(), a.back(), [&](int x) {
