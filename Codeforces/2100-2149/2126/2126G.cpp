...
void solve() {
    ...
    // cout<<l<<en<<r<<en;
    auto comb = [](node a, node b) {
        return node{a.x + b.x, max({a.mx, b.mx, a.r+b.l}), max(a.l, a.x+b.l), max(b.r, b.x+a.r)};
    };
    Segtree t(n, comb, v<node>(n, node{1, 1, 1, 1}), node{-INFL, -INFL, -INFL, -INFL});
    vvi pos(n+1);
    f(i, n) pos[a[i]].pb(i);
    int res = -INFL;
    int med = 1;
    rep(mn, 1, n) {
        ...
        while(med <= n) {
            bool ok = false;
            repr(j, pos[mn].size()-1, 0) {
                int i = pos[mn][j];
                int x = (a[i] >= med ? 1 : -1) + t.query(l[i]+1, i-1).r + t.query(i+1, r[i]-1).l;
                // cout<<i<<sp<<med<<sp<<t.query(l[i]+1, i-1).r<<sp<<(a[i]>=med ? 1 : -1)<<sp<<t.query(i+1, r[i]-1).l<<en;
                if(x < 0) {
                    // Important for pruning ones that don't work
                    pos[mn].pop_back();
                    continue;
                }
                cmx(res, med-mn);
            ...
            if(!ok) break;
            for(int i : pos[med]) t.update(i, node{-1, -1, -1, -1});
            med++;
        }
    }
    cout<<res<<en;
