...
tpl_<tn_ T, tn_ C = decltype(_add)> struct BIT     { int n; v<T> t, nums; C c;
    static_assert(is_invocable_r_v<T,C,T,T>,"Combine must be T(T,T)");
...
void solve() {
    ...
    vi a(n); read(a);
    vpii b;
    f(i, n) b.emplace_back(a[i], i);
    sort(all(b));
    BIT<int> bit(n, _add);
    int res = 0;
    for(auto [_, i] : b) {
        res += min(i - bit.query(0, i-1), n-i-1 - bit.query(i+1, n-1));
        bit.add(i, 1);
    }
    cout<<res<<en;
    // int res = INFL;
    // BIT<int> bit(2*n+1, _add);
    // vi pre(n+1, 0), suf(n+1, 0);
    // f(i, n) {
    //     pre[i+1] = pre[i] + bit.query(a[i]+1, n);
    //     bit.add(a[i], 1);
    // }
    // fe(i, n) bit.update(i, 0);
    // repr(i, n-1, 0) {
    //     suf[i] = suf[i+1] + bit.query(a[i]+1, n);
    //     bit.add(a[i], 1);
    // }
    // cout<<pre<<en<<suf<<en;
    // f(i, n+1) cmn(res, pre[i]+suf[i]);
    // cout<<res<<en;
}
