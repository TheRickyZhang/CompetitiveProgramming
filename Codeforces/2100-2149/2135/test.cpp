...
void solve(){
    ...
    auto pw=[&](ll x,ll e){ ll r=1%MOD; x%=MOD;
        while(e){ if(e&1) r=r*x%MOD; x=x*x%MOD; e>>=1; }
        return (int)r;
    };

    vi deg(n); f(i,n) deg[i]=adj[i].size();
    vb in(n,true);
    queue<int> q;
    f(i,n) if(deg[i]==1) q.push(i);
    while(!q.empty()){
        int u=q.front(); q.pop();
        if(!in[u]) continue;
        in[u]=false;
        for(int v:adj[u]) if(in[v]) if(--deg[v]==1) q.push(v);
    }

    int core=0; f(i,n) core+=in[i];
    if(core==0){
        int freeCnt=0; f(i,n) if(a[i]==-1) freeCnt++;
        cout<<pw(k, freeCnt)<<en; return;
    }

    // core may have multiple components; handle each independently and multiply
    vi vis(n,0), col(n,-1);
    int ans=1;
    f(start,n) if(in[start] && !vis[start]){
        // BFS this core-component
        vector<int> comp; comp.reserve(64);
        queue<int> qq; qq.push(start); vis[start]=1; col[start]=0;
        bool odd=false;
        while(!qq.empty()){
            int u=qq.front(); qq.pop();
            comp.pb(u);
            for(int v:adj[u]) if(in[v]){
                if(col[v]==-1){ col[v]=col[u]^1; vis[v]=1; qq.push(v); }
                else if(col[v]==col[u]) odd=true;
            }
        }
        // pin/component count
        int pinned=-1; // -1 = none, else the required constant
        if(odd){
            for(int u:comp){
                if(a[u]!=-1 && a[u]!=0){ cout<<0<<en; return; }
            }
            pinned=0;
        }else{
            for(int u:comp){
                if(a[u]!=-1){
                    if(pinned==-1) pinned=a[u];
                    else if(pinned!=a[u]){ cout<<0<<en; return; }
                }
            }
        }
        if(!odd && pinned==-1) ans = (ll)ans * (k%MOD) % MOD; // choose c freely
        // if odd or pinned, contributes ×1
    }

    // vertices outside core are unconstrained, each unknown adds a factor k
    int freeTree=0; f(i,n) if(!in[i] && a[i]==-1) freeTree++;
    ans = (ll)ans * pw(k, freeTree) % MOD;
