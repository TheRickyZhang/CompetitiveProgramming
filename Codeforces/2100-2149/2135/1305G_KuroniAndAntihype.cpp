...
//     vi a(n); read(a);
//
//     int ln = log2(*max_element(all(a))) + 1;
//     int full = (1<<ln)-1;
//     // cout<<full<<en;
//     DSU d(n);
//     vvi best(full+1);
...
//             if(i & (1<<j)) {
//                 int k = i ^ (1<<j);
//                 curr.insert(curr.end(), all(best[k]));
//                 sort(all(curr), [&](int x, int y) {
//                     return a[x] > a[y];
//                 });
//                 while(curr.size() >= 2 && d.same(curr[0], curr[1])) {
//                     curr.erase(curr.begin() + 1);
//                 }
//                 if (curr.size() > 2) curr.resize(2);
//             }
//         }
//     }
//     // cout<<best<<en;
...
struct dsu {
    ...
    int connect(int u, int v) {
        ...
        int ans = (d[u] == -1 ? cnt[u] : 1) + (d[v] == -1 ? cnt[v] : 1) - 1;
        d[u] += d[v];
        d[v] = u;
        return ans;
    }
