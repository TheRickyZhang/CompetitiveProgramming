...
tpl_<tn_ It, tn_ T>
auto leq_bound(It l,It r,T x) {
    auto it=upper_bound(l,r,x);
    return it!=l ? prev(it) : r;
}

tpl_<tn_ It, tn_ T>
auto less_bound(It l,It r,T x) {
...
tpl_<tn_ T>
bool cmn(T& a,T b) { return b<a ? a=b,1 : 0; }

tpl_<tn_ T>
bool cmx(T& a,T b) { return a<b ? a=b,1 : 0; }
...
tpl_<tn_ A,tn_ B>
ostream& op_<<(ostream& os,const pair<A,B>& p) { return os<<"("<<p.ff<<", "<<p.ss<<")"; }

tpl_<tn_ A>
ostream& op_<<(ostream& o,const v<v<A>>& m) {
    for(auto& r:m) {
        o<<"{";
        for(auto& e:r)o<<e<<" ";
        o<<"}\n";
...
tpl_<tn_ K,tn_ T>
ostream& op_<<(ostream& o,const map<K,T>& m) {
    o<<"{";
    for(auto& p:m)o<<p.ff<<":"<<p.ss<<",";
    return o<<"}";
}

tpl_<tn_ C,tn_ T=enable_if_t<!is_same_v<C,str>,tn_ C::value_type>>
ostream& op_<<(ostream& os,const C& v) {
...
private:
    ...
    void modify(int i, int a, int b, int p,T x,bool upd) {
        ...
        int m=(a+b)/2;
        p<=m ? modify(2*i,a,m,p,x,upd) : modify(2*i+1,m+1,b,p,x,upd);
        t[i]=c(t[2*i],t[2*i+1]);
    }
...
tpl_<tn_ T, tn_ C>
void printBIT(const BIT<T,C>& b,int l=16) {
    cout<<"BIT:\n";
    int lv=0;
    while(1<<lv<=min(b.n,l))lv++;
    int c=min(b.n,l);
    v<vs> g(lv,vs(c,str(4,' ')));
    fe(i,c) {
        int r=__builtin_ctz(i);
        if(r<lv) g[r][i-1]=format("{:4}",b.t[i]);
    }
    f(r,lv) {
        f(c2,c)cout<<g[r][c2];
        cout<<en;
...
struct Line {
    ...
    friend long double intersect(Line x,Line y) {
        return static_cast<long double>(y.b-x.b)/static_cast<long double>(x.a-y.a);
    }
};
...
struct CHT:multiset<Line,less<>> {
    ...
    bool isect(iterator it1,iterator it2) {
        ...
        if(it1->a==it2->a) it1->p=it1->b>it2->b ? INFL : -INFL;
        else it1->p=floor(it2->b-it1->b,it1->a-it2->a);
        return it1->p>=it2->p;
    }
    ...
    int query(int x) {
        ...
        return l.a*x+l.b;
    } // min -> negate inserted a, b, x
};

tpl_<bool upperHull=true>
struct MonotonicCHT {
    deque<Line> h;
    ...
    void add(const Line& ln) {
        // upperHull -> insert increasing slope, else decreasing slope
        if(!upperHull) {
            ln.a=-ln.a;
            ln.b=-ln.b;
        }
        while(h.size()>=2&&badUpper(h[h.size()-2],h.back(),ln)) h.pop_back();
        h.pb(ln);
    }
    ...
    int query(int x) {
        ...
        while(h.size()>=2) {
            if(h[1].at(x)>=h[0].at(x)) h.pop_front();
            else break;
        }
...
tpl_<class G,class S>
pair<vi,vi> _bfs(G& adj,const S& src=0,const bool zo=false) {
    int n=adj.size();
    deque<int> q;
    vi d(n,INFL),p(n,-1);
    auto add=[&](const int u) {
        d[u]=0;
        q.pb(u);
    };
    if constexpr(is_integral_v<decay_t<S>>) add(src);
    else for(int u:src) add(u);
    while(!q.empty()) {
        int u=q.front();
        q.pop_front();
        for(auto e:adj[u]) {
            int v,w;
            if constexpr(is_same_v<decay_t<decltype(e)>,int>) v=e,w=1;
            else v=e.ff,w=e.ss;
            int nd=d[u]+w;
            if(nd>=d[v]) continue;
            d[v]=nd;
            p[v]=u;
            if(zo&&w==0) q.push_front(v);
            else q.push_back(v);
        }
    }
    return {d, p};
...
void _dijkstra(vi& d,vvpii& adj, int a=0) {
    mpq<pii> q;
    d[a]=0,q.push({0, a});
    while(!q.empty()) {
        auto [w, u]=q.top();
...
tpl_<tn_ F>
pair<vvi,vvi> _jumpW(vi& par, int out,vi& wt,F mrg) {
...
int _lca(int u, int v,const vvi& up,const vi& d) {
    ...
    if(d[u]<d[v]) swap(u,v);
    rep(j,0,ln-1) { if(d[u]-d[v]&(1<<j)) u=up[u][j]; }
    if(u==v) return u;
...
vi _virtualTree(vvi& vadj,const vi& nodes,const vi& tin,const vvi& up,const vi& d,bool dir=true) {
    vi lu=nodes;
...
struct pairHash {
    tpl_<tn_ A,tn_ B>
    size_t op_()(const pair<A,B>& p) const { return hash<A>{}(p.ff)^(hash<B>{}(p.ss)<<1); }
};

struct vHash {
    tpl_<tn_ T>
    size_t op_()(const v<T>& x) const {
        size_t h=0;
        for(auto& i:x)h^=hash<T>{}(i)+0x9e3779b9+(h<<6)+(h>>2);
        return h;
    }
};
...
cx_ ll msb(ll x) { return 63-__builtin_clzll(x); }
cx_ ll lsb(ll x) { return __builtin_ctz(x); }
...
void solve() {
    ...
    vi rem(n);
    f(i,n) rem[i]=v[i];
    auto [vals, mp]=_compress(rem);
    SparseSegtree tree(a, 0);
    set<int> s;
    for(int r : rem) {
        tree.range_set(r, r, tree.range_sum(r, r) + 1);
        s.insert(r);
    }
    int d = (a+b) / 2;
    for(int r : rem) {
