...
// void solve() {
//     cin>>n;
//     vpii points(n); read(points);
//
//     int tx = 0, ty = 0;
//     auto get = [&](str dir, int k) {
//         int res;
//         if(dir == "U") ty += k;
//         else if(dir == "D") ty -= k;
//         else if(dir == "R") tx += k;
...
//         return res;
//     };
//     MonotonicCHT<true> upper; MonotonicCHT<false> lower;
//     sort(all(points));
//     // Very clever application of line-point duality, which allows us to use CHT processing for points as well
//     for(auto [x, y] : points) {
//         upper.add(Line(x, -y));
//     }
//     sort(rall(points));
...
//     }
//     vpii hull;
//     for(Line l : upper.h) { hull.pb({l.a, -l.b}); }
//     hull.pop_back();
//     // reverse(all(lower.h));
//     for(Line l : lower.h) { hull.pb({-l.a, l.b}); }
//     hull.pop_back();
//     // hull now contains (l0, l1, ... ln-1, ln = l0)
//     // cout<<hull<<en;
//
//     int pos1 = 0, pos2 = -1;
//     n = hull.size();
//     f(i, n) {
//         auto [x1, y1] = hull[(i+n-1) % n];
//         auto [x2, y2] = hull[i];
//         auto [x3, y3] = hull[(i+1) % n];
//         if(x2 >= x1 && y2 - y1 >= x2 - x1 && y3 - y2 < x3 - x1) pos1 = i;
//         if(x2 >= x1 && y2 - y1 <= x2 - x1 && y3 - y2 > x3 - x1) pos2 = i;
//     }
//     if(pos2 == -1) {
//         assert(n <= 2);
//         pos2 = pos1;
//     }
//     // cout<<"pos"<<sp<<pos1<<sp<<pos2<<en;
//     auto [x1, y1] = hull[pos1];
//     auto [x2, y2] = hull[pos2];
//     // cout<<x1<<sp<<y1<<en;
//     // cout<<x2<<sp<<y2<<en;
//
//     get("U", 2*INF);
//     int dist = get("L", 2*INF + 1);
//     int val1 = dist - abs(tx) - abs(ty) + x1 - y1;
//     dist = get("R", 4*INF + 2);
//     // cout<<"tx "<<tx<<sp<<"ty "<<ty<<en;
//     int val2 = dist - abs(tx) - abs(ty) + x2 + y2;
//
//     // cout<<"vals"<<sp<<val1<<sp<<val2<<en;
//     // Find intersection of x + val1 = -x + val2
//     int tot = val2-val1;
//     // assert(tot % 2 == 0);
//     cout<<'!'<<sp<<tot/2<<sp<<(tot/2 + val1)<<endl;
//     cout.flush();
// }
