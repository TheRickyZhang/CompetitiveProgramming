...
// Very clever solution for least total cost of traversing and merging operation on graph
// Suppose all nodes get merged at r. Then, we form a directed tree to root r, where each node except r merged at some parent
// Total cost = sum(edge + a[p[u]] if u != r)
// sum a[p[u]] = sum a[u] + a[v] for all edges {u, v} - sum a[i] + sum a[r] (we overcount by precisely each starting, except for root)
// So cost = sum(w(u, v)+a[u]+a[v]) - sum(a[i]) + a[r] which can be solved with a MST!

// Only caveat is that we can't generate all n^2 edges
// So we can only find and merge the "best" values for all nodes.
// For some u, we want to minimize a[u] + a[v] + dist(u, v). If v != u, then we can add a (u, v) edge. In this case we denote v as p[u], parent of u.
// Otherwise, we complete the graph by using existing edges as bridges. Note that EVERY edge cost must represent a[u] + a[v] + dist(u, v) for some u, v
// So take edge (u, v). If p[u] != u, then we have determined it is "cheaper" to actually merge at p[u] than u.
// So our total cost is a[p[u]] + a[p[v]] + dist(u, p[u]) + dist(v, p[v]) + dist(u, v)
// So based on this, it is convenient to store b[u] = a[p[u]] + dist(u, p[u]) in our implementationi
...
void solve() {
    ...
    // To efficiently calculate b for all nodes, run a multi-source dfs with starting costs of a[i];
    vi b(n, INFL), par(n);
    mpq<iii> q;
    ...
    for(auto [w, u, v] : e) {
        int x = par[u], y = par[v];
        if(u == y || v == x) continue; // We can't merge with each other?
        if(x != y) ne.pb({b[u] + b[v] + w, x, y});
    }
    sort(all(ne));
