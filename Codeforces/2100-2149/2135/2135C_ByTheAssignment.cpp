...
// This solution is conceptually correct, to not do n node walks -> up to n^2, we could use the difference method to
// Push cycle c in a node range [a, b] and then do a dfs to record, only merging new things with the top element of the stack.
int k, n, m;
//
// // Literally the most scuffed, creative but kind of terrible problem solution
// void solve() { cin>>n>>m>>k;
//     vi a(n); read(a);
//     vvi adj(n); read(adj, m);
//     vvi b(n), c(n);
//     vi tin(n, -1), dep(n, 0);
//     int t = 0; int m = 0;
//     bool good = true;
...
//
//     vvi nadj(n);
//     vpii vals; // Holds {a[u], index to previous different}
//     vi inCycle(n+1, 0);
//     fvii dfs = [&](int u, int p) {
//         if(!good) return;
//         tin[u] = t++;
//         int prev = -1;
//         assert(dep[u] == vals.size());
//         if(!vals.empty()) {
//             auto [x, pos] = vals.back();
//             if(a[u] == x) prev = pos;
//             else prev = dep[u] - 1;
//         }
//         vals.push_back({a[u], prev});
//         for(int v : adj[u]) {
//             if(v == p) continue;
//             if(tin[v] == -1) {
//                 dep[v] = dep[u] + 1;
...
//                 b[v].pb(m); c[u].pb(m);
//                 int d = dep[u] - dep[v] + 1;
//                 int fst = (a[u]==-1) ? prev : dep[u];
//                 int x = vals[fst].ff;
//                 if(d & 1) {
//                     while(fst >= dep[v] && (vals[fst].ff == 0 || vals[fst].ff == -1)) fst = vals[fst].ss;
//                     if(fst < dep[v]) {
//                         flex.pb(0);
//                     } else {
//                         good = false; flex.pb(-111);
//                     }
//                 } else {
//                     if(fst < dep[v]) {
//                         flex.pb(INFL);
//                     } else {
//                         while(fst >= dep[v] && (vals[fst].ff == x || vals[fst].ff == -1)) fst = vals[fst].ss;
//                         if(fst < dep[v]) {
//                             flex.pb(x);
//                         } else {
//                             good = false; flex.pb(-222);
//                         }
//                     }
//                 }
//                 m++;
...
//     dfs(0, -1);
//     if(!good) quit(0);
//     // cout<<"flex"<<sp<<flex<<en;
//     // cout<<"dep"<<sp<<dep<<en;
//     // cout<<"incycle"<<sp<<inCycle<<en;
//
//     vi par(m); iota(all(par), 0);
...
//         if(x == y) return;
//         if(flex[x] == INFL && flex[y] != INFL) swap(x, y);
//         par[y] = x;
//         if(flex[x] != INFL && flex[y] != INFL && flex[x] != flex[y]) {
//             // cout<<"cannot merge "<<u<<sp<<v<<en;
//             good = false; return;
//         }
//         if(flex[x] == INFL) flex[x] = flex[y];
//     };
//     // cout<<"cycles"<<sp<<cycles<<en;
//     // cout<<"flex"<<sp<<flex<<en;
//
//     int res = 1;
//     f(i, n-1) inCycle[i+1] += inCycle[i];
//     f(i, n) if(inCycle[i] == 0 && a[i] == -1) {
//         res = mult(res, k);
//     }
//     vb vis(m, false);
//     stack<int> s;
//
//     // Just for ensuring efficient merging. There likely is a much better way to do this.
//     fvii dfs2 = [&](int u, int p) {
//         // cout<<"vis "<<u<<en;
//         while(!s.empty() && vis[s.top()]) {
//             s.pop();
//         }
//         for(int j : b[u]) {
//             if(!s.empty()) merge(j, s.top());
//             s.push(j);
//         }
//         for(int j : c[u]) vis[j] = true;
//         for(int v : nadj[u]) {
//             if(v == p) continue;
//             dfs2(v, u);
//         }
...
//
//     // cout<<p<<en;
//     // Important that we don't overcount cycles here. If a -> p, b -> p, still only one extra k factor
//     set<int> st;
//     f(i, m) {
//         // cout<<i<<sp<<getPar(i)<<en;
//         st.insert(getPar(i));
//     }
//     // cout<<"cycle i's:";
//     for(int i : st) {
//         // cout<<i<<sp;
//         if(flex[i] == INFL) res = mult(res, k);
//     }
//     // cout<<en;
//     cout<<res<<en;
// }
...
int32_t main(){
  ...
  while(T--){
    ...
    for(int i=0;i<m;i++){
      int u,v; cin>>u>>v; eu[i]=u; ev[i]=v;
      adj[u].push_back({v,i});
      adj[v].push_back({u,i});
    }

    // Pass 1: find bridges via diff-DFS (no Tarjan lowlink)
    vector<int> tin(n+1,0), par(n+1,-1), d(n+1,0);
    vector<char> br(m,0);
    int t=0;
    ...
    function<void(int,int)> dfs = [&](int u,int pe){
      ...
      for(auto [v,id]:adj[u]){
        ...
        if(!tin[v]){
          ...
          dfs(v,id);
          d[u]+=d[v];
          if(d[v]==0) br[id]=1; // edge (u,v) is a bridge
        }else if(tin[v]<tin[u]){
    ...
    // Pass 2: build 2ECCs by ignoring bridges; check bipartite & equal values
    vector<int> comp(n+1,0), col(n+1,-1);
    long long ans=1;
    int cid=0;
    for(int s=1;s<=n && ans; s++){
      if(comp[s]) continue;
      cid++;
      queue<int> q; q.push(s);
      comp[s]=cid; col[s]=0;

      int x=-1; bool odd=false;
      ...
      while(!q.empty() && ans){
        int u=q.front(); q.pop();
        for(auto [v,id]:adj[u]){
          if(br[id]) continue; // ignore bridges
          if(!comp[v]){
            comp[v]=cid; col[v]=col[u]^1; q.push(v);
            touch(v);
          }else{
            if(col[v]==col[u]) odd=true; // odd cycle found
          }
        }
      }
      ...
      if(odd){
        if(x>0) ans=0;
        // x==-1 or x==0 → exactly 1 way, no multiply
      }else{
        if(x<0) ans = (ans*V)%MOD; // all free inside bipartite block
      }
    }
