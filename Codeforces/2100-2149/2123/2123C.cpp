...
void solve() {
    cin>>n;
    vi a(n); read(a);
    vi premx(n), sufmx(n), premn(n), sufmn(n);
    premx[0] = premn[0] = a[0];
    sufmx[n-1] = sufmn[n-1] = a[n-1];
    f(i, n-1) {
        premx[i+1] = max(premx[i], a[i+1]);
        premn[i+1] = min(premn[i], a[i+1]);
    }
    repr(i, n-2, 0) {
    ...
    cout<<1;
    rep(i, 1, n-2) {
        if((premn[i] > a[i] && sufmx[i] < a[i]) || premn[i] == a[i] || sufmx[i] == a[i]) {
            cout<<1;
        } else {
            cout<<0;
        }
