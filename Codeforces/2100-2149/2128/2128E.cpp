...
void solve() {
    ...
    // cout<<"a "<<a<<en;
    int l, r;
    int res = lstTrue(0LL, sz-1, [&](int x) {
        vi b(n);
        f(i, n) b[i] = a[i] >= x ? 1 : -1;
        vi best(n+1, 0);
        vi left(n+1, 0);
        ...
        f(i, n) {
            ...
            if(best[i] <= curr) {
                ...
                best[i+1] = curr;
                left[i+1] = i+1; // Note this +1, since the left end is excluded when doing a[r] - a[l-1]
            }
        }
        return false;
    });
    cout<<val[res]<<sp<<l+1<<sp<<r+1<<en;
}
