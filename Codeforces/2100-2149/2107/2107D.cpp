...
vvi adj;
vi par, seen;
vb vis; // vis = apple taken, seen = temporary for this bfs
int t;

iii getDiam(int a, vi &roots){
    queue<pii> q;
    q.push({a,0});
    seen[a] = t; par[a] = -1;
    int best = a, bd=0; // Since we may have multiple paths
    while(!q.empty()){
        auto [u,d]=q.front(); q.pop();
        if(d>bd || (d==bd && u>best)) bd=d, best=u;
        for(int v: adj[u]) if(!vis[v] && seen[v]!=t){
            seen[v]=t; par[v]=u;
            q.push({v,d+1});
        }
    }
    ...
    // Only need to record parents here
    // Marking with t -> don't need to reset everything
    q.push({x,0});
    seen[x] = t; par[x] = -1;
    best=x; bd=0;
    while(!q.empty()){
    ...
    // {x, y} is our path
    // Since roots is LOCAL we can erase and push neighbors of path
    vi diam;
    int it = y;
    while(it != x) {
        diam.pb(it); it = par[it];
    } diam.pb(x);

    for(int v: diam) vis[v]=true;
    roots.clear();
    for(int u: diam) {
        for(int v: adj[u]) {
            if(!vis[v]) roots.pb(v);
...
void solve(){
    ...
    pq.push(getDiam(0, roots));
    while(!pq.empty()){
        auto [d,u,v] = pq.top(); pq.pop();
        res.pb(d); res.pb(u); res.pb(v);
        next.clear();
        for(int r: roots) {
            vi cr;
            pq.push(getDiam(r, cr));
            for(int c: cr) next.pb(c);
        }
        roots.swap(next);
    }
