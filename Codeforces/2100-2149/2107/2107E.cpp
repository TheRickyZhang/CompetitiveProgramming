...
int t, k, n, m;
/*
 * We can model each subtree as (u, p, x) - size, number of pairs with LCA root, weight of tree
 * To combine:
 * u = u1 + u2 + 1
 * p = u1 + u2 + u1*u2 (can choose root and node from sub, or one from each sub)
 * x = x1 + x2 + p1 + p2 + (p)
...
void solve() {
    ...
    assert(llabs(rem) <= 1);
    rep(i, 1, n-1) cout<<p[i]+1<<sp<<i+1<<en;
}
