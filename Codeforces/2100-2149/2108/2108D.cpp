...
void solve(){
    int n,k; cin>>n>>k;
    if(n == 2*k) {
        cout<<"! "<<k<<sp<<k<<endl; cout.flush(); return;
    }
    vi a(k), b(k);
    f(i,k) a[i] = ask(i);
    // Need to still read the last k values for b BUT cyclically shifted
    f(i,k) b[(n%k + i) % k] = ask(n-k + i);

    int diff = -1;
    f(i,k) if(a[i] != b[i]){ diff = i; break; }
    if(diff < 0){
        cout<<"! -1"<<endl; cout.flush();
        return;
    }
    // First different with mod diff
    int l = k, r = n-k;
    while(l < r) {
        int m = (l+r)/2;
        int pos = -1;
        rep(i, max(l, m-k), min(m+k, r)) {
            if(a[i % k] == b[i % k]) continue;
            pos = i; break;
        }
        if(pos == -1) break;
        if(ask(m) == a[m % k]) l = m+1;
        else r = m;
    }
    if(l == r) {
        cout<<l<<sp<<n-l<<endl;
