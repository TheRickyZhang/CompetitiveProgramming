...
void solve(){
    ...
    f(i, it+1){
        ...
        f(j, m){
            int l = v[j], r = v[j];
            while(j+1 < m && v[j+1]==r+1) r = v[++j]; // we can skip the adjacent indices here
            if(!pressed.count(l-1) && !pressed.count(r+1)) ++cnt;
        }
        for(int x : v) pressed.insert(x);
    }
    cout<<cnt<<en;
}
