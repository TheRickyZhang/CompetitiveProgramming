...
void solve() {
    ...
    vi a(n), b(n);
    read(a); read(b);
    if(a.back() != b.back()) quit("NO");
    vb change(n, false);
    f(i, n-1) {
        if(a[i] != b[i] && (a[i] ^ a[i+1]) != b[i] && (a[i] ^ b[i+1]) != b[i]) quit("NO");
    }
    // f(i, n-1) {
    //     if(change[i] && a[i+1]==b[i+1] && (a[i] ^ a[i+1]) != b[i]) {
    //         cout<<"NO"<<sp<<i<<sp<<(a[i]^a[i+1])<<en;
    //         return;
    //     }
    // }
    cout<<"YES"<<en;
    // f(j, 30) {
    //     f(i, n-1) {
    //         int x = (a[i]>>j) & 1, y = (a[i+1]>>j) & 1, z = (b[i]>>j) & 1;
    //         if(x != z && (x ^ y) == z) {
    //             a[i] ^= a[i+1];
    //         }
    //     }
    ...
    // cout<<a<<en;
    // f(i, n-1) {
    //     if(a[i] != b[i] && (a[i] ^ a[i+1] != b[i])) quit("NO");
    // }
    // cout<<"YES"<<en;
}
