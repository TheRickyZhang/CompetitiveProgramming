...
cx_ int N = 50000; cx_ int MOD=1e9+7; cx_ int INF=1e9; cx_ ll INFL=0x3f3f3f3f3f3f3f3f; cx_ int B = 31;
...
void solve() {
    ...
    // [time, index] -> time moved
    vi t(n, -INF);
    t[0] = 0;
    rep(i, 1, 3*n-1) {
        vi tt = t;
        f(j, n) {
            int k = adj[j][(i-1) % deg[j]];
            cmx(tt[k], t[j]+1);
        }
        if(tt[n-1] > 0) {
            cout<<i<<" "<<i-tt[n-1]<<en;
    ...
    //             vis[u] = true;
    //         }
    //         int nt = ceildiv((t-i), d) * d + i + 1;
    //         int v = adj[u][i];
    //         if(!vis[v] && nt < dist[v]+deg[v]) {
    //             q.push({v, nt, x+(nt-t-1)});
    //             cmn(dist[v], nt);
    //         }
    //     }
    // }
    // cout<<dist[n-1]<<sp<<res<<en;
}
