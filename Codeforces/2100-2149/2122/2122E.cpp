...
cx_ int N = 100000; cx_ int MOD=998244353; cx_ int INF=1e9; cx_ ll INFL=0x3f3f3f3f3f3f3f3f; cx_ int B = 31;
...
void solve(){
  ...
  read(a),read(b);
  vpii cross;
  rep(i,-k+1,k-1) cross.pb({i,k-abs(i)});
  v<vvi> d(n, vvi(k+2, vi(2,0)));
  d[0][k+1][0] = 1;
  f(i,n-1){
    ll tot=0;
    rep(j,1,k) tot=(tot + d[i][j][1])%MOD;
    auto &cur=d[i], &nxt=d[i+1];
    nxt.assign(k+2, vi(2,0));
    vpii diff;
    if(a[i+1]==-1 && b[i]==-1) diff=cross;
    else if(a[i+1]==-1)  fe(j,k) diff.pb({j-b[i],1});
    else if(b[i]==-1)    fe(j,k) diff.pb({a[i+1]-j,1});
    else                  diff.pb({a[i+1]-b[i],1});
    for(auto [x,f]:diff){
      if(x>=0){
        nxt[k+1][0] = (nxt[k+1][0] + mult(f,cur[k+1][0]))%MOD;
        rep(j,0,k){
          int nj=j+x;
          if(nj<=k) nxt[nj][1] = (nxt[nj][1] + mult(f,cur[j][1]))%MOD;
        }
      } else {
        int idx = x + k;
        ll carry = (cur[k+1][0] + tot)%MOD;
        nxt[idx][1] = (nxt[idx][1] + mult(f,carry))%MOD;
      }
    }
  }
  int res=0;
  rep(i,0,k+1) rep(t,0,1) res=add(res,d[n-1][i][t]);
  if(a[0]==-1) res=mult(res,k);
  if(b[n-1]==-1) res=mult(res,k);
  cout<<res<<en;
}
