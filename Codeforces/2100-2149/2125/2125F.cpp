...
tpl_<tn_ C, tn_ T = enable_if_t<!is_same_v<C,str>,typename C::value_type>> ostream& operator<<(ostream& os,const C& v){ str sep; for(const T& x:v) os<<sep<<x, sep=sp; return os;}
...
tpl_<tn_ F> pair<vvi,vvi> binaryJumpW(const vi &par, int out, const vi &wt, F merge){int n=par.size(),ln=log2(n)+1; vvi up(n,vi(ln,0)), cost(n,vi(ln,0));
    f(i,n){up[i][0]=par[i]; cost[i][0]=(par[i]==out ? 0:wt[i]);} rep(j,1,ln-1){f(i,n){int p=up[i][j-1]; if(p==out) {up[i][j]=out; cost[i][j]=cost[i][j-1];} else{up[i][j]=up[p][j-1];
...
void solve() {
    ...
    fe(i, mx) {
        cnt[i] += cnt[i-1];
        if(cnt[i] > mxfreq && cnt[i] > 0) {
            mxfreq = cnt[i];
            want = {i};
        } else if(cnt[i] == mxfreq) {
            want.pb(i);
        }
    }
    if(want.empty()) quit(0);
    // Want should be 2 here
    // cout<<"want "<<want<<en;
    ...
    int res = INFL;
    auto lit = leq_bound(all(want), curr);
    if(lit != want.end()) cmn(res, curr - *lit);
    auto rit = upper_bound(all(want), curr);
    if(rit == want.end()) quit(res);
    int x = *rit;
    n = s.size();
    if(x > mx) quit(res);

    // We now need to compute d[n][x] -> {min cost of x dockers, first n characters}
    // This is the spirit, but we optimize space and time complexity
    // vvi d(n+1, vi(x+1, INFL)), d[i][j] = min(d[i-1][k-1] + f(k, j))

    vi cost(n-5, 0);
    f(i, n-5) {
        int c = 0;
        f(j, 6) if(s[i+j] != DOCKER[j]) c++;
        cost[i] = c;
    }
    int ln = log2(n-5) + 1;
    vvi sp(n-5, vi(ln, INF));
    f(i, n-5) sp[i][0] = cost[i];
    fe(j, ln-1) {
        for(int i = 0; i + (1<<j) <= n-5; i++) {
            sp[i][j] = min(sp[i][j-1], sp[i + (1<<(j-1))][j-1]);
        }
    }
    // Cost should be {6, 6, 6, 6, 5, 6, 6, 6, 5} here
    // cout<<"cost "<<cost<<en;
    // cout<<"sp min "<<sp<<en;
    ...
    auto func = [&](int i, int j) {
        if(j - i + 1 < 6) return INFL;
        return query(i, j-5);
    };
    vi d(n, INFL), nd(n, INFL);
    // we start d with 1 freq - this is to get accurate preprocessing
    rep(i, 5, n-1) {
        d[i] = min(d[i-1], func(i-5, i));
    }
    // cout<<"initial d "<<d<<en;
    function<void(int, int, int, int)> dsolve = [&](int l, int r, int a, int b) {
        if(r < l) return;
        int m = (l+r)/2;
        int best = INFL, pos = a;
        rep(k, a, min(m, b)) {
            int temp = (k ? d[k-1] : INFL) + func(k, m);
            if(temp < best) best = temp, pos = k;
        }
        nd[m] = best;
        dsolve(l, m-1, a, pos);
        dsolve(m+1, r, pos, b);
    };
    // Since we started with 1 docker, repeat x-1 more times
    f(_, x-1) {
        // Note that here since all costs < 5 will be INF anyways (cannot make a single docker sting)
        // We can just put that as our lower bound
        dsolve(0, n-1, 0, n-1);
        d = nd;
        // cout<<d<<en;
    }
    cout<<min(res, d[n-1])<<en;
}
