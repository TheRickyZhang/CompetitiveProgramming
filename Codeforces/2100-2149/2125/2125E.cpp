...
void solve() {
    int x; cin>>n>>x;
    if(n == 1) quit(x);
    // We always need: (n-1) * sum(diff) <= x. And we only need strict ==, just prefix sum for <
    int sum = n * (n+1) / 2; // Of the differences
    int need = sum - (n+1);  // to get our sum to satisfy equality (by adding 1 to everything)
    if(n + need > x) quit(0);
    // How many times we increase the true maximum
    // d[0] is ways for max = n + need
    vi d(x+1);
    d[0] = 1;
    // Note that this is different than adding 2 * d[i] - propogate cases where can add 0, -1
    f(_, 2) f(i, x) d[i+1] = add(d[i+1], d[i]);
    fe(i, n-2) {
        f(j, x) {
            if(j + i+1 <= x) {
                d[j + i+1] = add(d[j + i+1], d[j]);
            }
        }
    }
    int res = 0;
