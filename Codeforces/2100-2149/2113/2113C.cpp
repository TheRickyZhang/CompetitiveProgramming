...
void solve() {
    ...
    f(i, n) {
        f(j, m) {
            pre[i+1][j+1] = pre[i+1][j] + pre[i][j+1] - pre[i][j] + (grid[i][j] == 'g');
        }
    }
    auto query = [&](int lx, int ly, int rx, int ry) {
        ckmx(lx, 0LL); ckmx(ly, 0LL);
        ckmn(rx, n-1); ckmn(ry, m-1);
        return pre[rx+1][ry+1] - pre[rx+1][ly] - pre[lx][ry+1] + pre[lx][ly];
    };
    ...
    f(i, n) {
        f(j, m) {
            ...
            ckmn(best, query(i-k, j-k, i+k, j+k));
        }
    }
    cout<<cnt-best<<en;
