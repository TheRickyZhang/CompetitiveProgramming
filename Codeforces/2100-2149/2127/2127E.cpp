...
void solve(){
    ...
    f(col,k){
        ...
        int m = lu.size();
        f(i,m-1) lu.pb(_lca(lu[i], lu[i+1], up, dep));
        sort(all(lu), [&](int u,int v){ return tin[u]<tin[v]; });
        lu.erase(unique(all(lu)), lu.end());
        for(int u:lu) nodeIn[u].pb(col);
        f(i,(int)lu.size()-1){
            int a = _lca(lu[i], lu[i+1], up, dep), b = lu[i+1];
            if(c[a]!=-1 && c[a]!=col) force[a]=true;
            if(a!=b) vadjs[col][a].pb(b);
        }
    }

    vi c0=c, cc(n,-1);
    fvii findChild = [&](int u,int p){
        if(c[u]!=-1) cc[u]=c[u];
        for(int v:adj[u]){
            if(v==p) continue;
            findChild(v,u);
            if(cc[u]==-1 && cc[v]!=-1) cc[u]=cc[v];
        }
    };
    findChild(0,-1);

    f(i,n) if(c0[i]==-1 && nodeIn[i].size()>=1) c[i]=cc[i];

    int res=0;
    fvii dfs = [&](int u,int p){
        if(force[u] || nodeIn[u].size()>=2) res+=w[u];
        if(c[u]==-1) c[u] = (cc[u]!=-1 ? cc[u] : (p==-1 ? 0 : c[p]));
        for(int v:adj[u]){
            if(v==p) continue;
            dfs(v,u);
        }
