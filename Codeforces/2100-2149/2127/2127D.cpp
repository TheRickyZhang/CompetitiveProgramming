...
cx_ int N=100000; cx_ int MOD=1e9+7;
...
void solve() {
    ...
    if(mult == -1) {
        quit(mint(fa[n/2]) * mint(fa[(n+1)/2]) * 2);
    }
    fvii dfs = [&](int u, int p) {
        ...
        if(u == mult) {
            ...
            if(x == 1) d[u] = a[0] * (cnt > 0 ? 2 : 1);
            else if(x == 2) d[u] = a[0] * a[1] * 2;
        } else {
            if(x > 1) {
                good = false; return;
            }
