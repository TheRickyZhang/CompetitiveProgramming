...
void solve(){
    ...
    vi pre(mxr + 1, 0);
    rep(x, 1, mxr) pre[x] = (cnt[x] == 0 ? 1 : 0);
    rep(x, 1, mxr) pre[x] += pre[x-1];

    // i is unique if forced[i]=1 or forced[pref]=0
    string res(n,'0');
    f(i,n){
        auto [l,r] = range[i];
        if(forced[i] != 0){
            if(cnt[forced[i]]==1) res[i] = '1';
        } else {
            if(pre[r] - (l>1?pre[l-1]:0) > 0) res[i] = '1';
        }
    }
    cout<<res<<en;
}
