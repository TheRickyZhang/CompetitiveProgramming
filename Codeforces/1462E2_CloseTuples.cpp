...
void solve() {
    ...
    vi ifa(n+1, 0); ifa[n] = fastPow(fa[n], MOD-2, MOD);
    ...
    int res = 0;
    f(i, n) {
        int l = upper_bound(all(a), a[i]-k-1) - a.begin();
        int x = i-l+1;
        if(x < m) continue;
        res = (res + choose(x-1, m-1)) % MOD;
    }
    cout<<res<<en;
    ...
    // }
    // right.pb({n-1, cnt});
    // auto unique = [&](int l, int r) {
    //     return dp[r+1]-dp[l];
    // };
    // int res = 0;
    // for(auto [r, rcnt] : right) {
    //     int l = upper_bound(all(a), a[r]-m-1) - a.begin();
    //     int x = r-l+1;
    //     if(x < m) continue;
    //     fe(i, rcnt) {
    //         res +=
