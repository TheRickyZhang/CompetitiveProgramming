...
void solve() {
    ...
    // In 0, 1 case you always round up with picking opposite and ?
    if(s[0]=='0') {
        cout<<y+(z+1)/2<<en;
    } else if(s[0]=='1'){
        cout<<x+(z+1)/2<<en;
    } else {
        // You can lock in all favorable leafs, then choose z + special case where don't fix root first, if x==y.
        int res = max(x, y) + (z + (x==y ? w % 2 : 0))/2;
        cout<<res<<en;
    }
}
