...
// struct node {
//     int a, last, len, g;
//     node() : a(-1), last(-1), len(0), g(0) {}
//     explicit node(int x) : a(x), last(x), len(1), g(0) {}
//     node(int a, int last, int len, int g) : a(a), last(last), len(len), g(g) {}
// };
//
// node combine(node x, node y) {
//     if(x.len == 0) return y;
//     if(y.len == 0) return x;
//     int diff = y.a - x.last; // difference between the two intervals.
//     int newG = diff; // if x was a single element, then its g==0, so newG is diff.
//     if(x.len > 1) newG = std::gcd(x.g, diff);
//     if(y.len > 1) newG = std::gcd(newG, y.g);
//     return node(x.a, y.last, x.len + y.len, newG);
// }
//
// void solve() {
//     cin>>n;
...
//     int ln = log2(n) + 1;
//     v<v<node>> st(n, v<node>(ln, node()));
//     f(i, n) st[i][0] = node(a[i]);
//
//     // Custom sparse table
//     fe(j, ln-1) {
//         // Note that we iterate until sum <= n since the actual range covered is [i, i+(1<<j)-1]
...
//     int res = 0;
//     f(r, n) {
//         int l = fstTrue(0LL, r, [&](int i) {
//             node cur = query(i, r);
//             return (cur.len == 1) || (cur.len > 1 && cur.g != 0 && __builtin_popcountll(cur.g) == 1);
//         });
//         cout<<l<<sp;
//         if(l <= r) res += r-l+1;
//     }
