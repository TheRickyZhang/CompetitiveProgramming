...
struct node {
    ll mn, mx, val, mna, mnb, mxa, mxb;
    node(ll x = 0, int i = 0) : mn(x), mx(x), val(0), mna(i), mnb(i), mxa(i), mxb(i) {}
    node(ll mn, ll mx, ll val, ll a, ll b, ll c, ll d)
        : mn(mn), mx(mx), val(val), mna(a), mnb(b), mxa(c), mxb(d) {}
};

node combine(const node& l, const node& r) {
    if (l.mn == 0) return r;
    if (r.mn == 0) return l;
    ll mmn = min(l.mn, r.mn);
    ll mmx = max(l.mx, r.mx);
    ll mna, mnb, mxa, mxb;
    if(l.mn < r.mn) mna = l.mna, mnb = l.mnb;
    else if(r.mn < l.mn) mna = r.mna, mnb = r.mnb;
    else mna = l.mna, mnb = r.mnb;

    if(l.mx > r.mx) mxa = l.mxa, mxb = l.mxb;
    else if(r.mx > l.mx) mxa = r.mxa, mxb = r.mxb;
    else mxa = l.mxa, mxb = r.mxb;

    ll llen = max(mxb-mna, mnb-mxa) + 1;
    ll vval = max({l.val, r.val, mmx - mmn - (llen - 1)});
    return node(mmn, mmx, vval, mna, mnb, mxa, mxb);
}
...
void solve() {
    ...
    Segtree<node> st(n, node(0), combine);
    f(i, n) {
        st.update(1, 0, n-1, i, node(nums[i], i));
    }

    cout<<st.query(0, n - 1).val<<en;
    f(_, q) {
        ll i, x; cin>>i>>x; i--;
        st.update(i, node(x, i));
        cout<<st.query(0, n-1).val<<en;
    }
}
int main(){
