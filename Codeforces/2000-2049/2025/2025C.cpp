...
void solve() {
    ...
    fe(i, n-1) {
        ...
        if(mp.count(a[i-1]-k+1)) ckmx(res, i-mp[a[i-1]-k+1]);
        else ckmx(res, i - mp.begifn()->ss);
        if(a[i] != a[i-1] + 1) mp = map<int, int>();
        mp[a[i]] = i;
    }
    if(mp.count(a[n-1]-k+1)) ckmx(res, n - mp[a[n-1]-k+1]);
    else ckmx(res, n - mp.begin()->ss);
    ...
    //         if(a[i] - k >= l && starts.count(a[i]-k)) ckmx(res, i - starts[a[i]-k]);
    //     } else {
    //         l = a[i+1];
    //     }
    //     if(!starts.count(a[i])) starts[a[i]] = i;
    // }
    // if(a[n-1]-k >= l && starts.count(a[n-1]-k)) ckmx(res, n-1 - starts[a[n-1]-k]);
}
