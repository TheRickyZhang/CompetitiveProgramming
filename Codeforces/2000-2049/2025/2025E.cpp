...
inline int mult(int a, int b, int m = MOD) {return (a * b) % m;} inline int add(int a, int b, int m = MOD) {return (a +b) % m;}
...
int32_t main() {
    ...
    f(i, m) {
        f(j, i+1) {
            ...
            if(j>0) ways[i+1][j-1] = add(ways[i+1][j-1], ways[i][j]);
        }
    }
    vvi dp(n+1, vi(m+1));
    dp[0][0] = 1;
    f(i, n) {
        f(j, m+1) {
            f(k, m+1) {
                int nj = (i>0) ? j-k : j+k;
                // Note that we only consider [0, m] as possible nj values since there are up to m 1-cards;
                // Handling here is the easiest
                if(0 <= nj && nj <= m) {
                    dp[i+1][nj] = add(dp[i+1][nj], mult(dp[i][j], ways[m][k]));
                }
            }
        }
    }
