...
void solve() {
    ...
    vi a(n); // Current order
    vi pos(n);
    v<set<int>> ch(n);
    f(i, n) {
        cind>>a[i];
        pos[a[i]] = i;
        if(p[a[i]] != -1) ch[p[a[i]]].insert(i);
    }
    function<int(int)> check = [&](int u) {
        if(ch[u].empty()) return 1;
        int mn = *ch[u].begin(), mx = *ch[u].rbegin();
        return (pos[u] < mn && pos[u]+sz[u] >= mx + sz[a[mx]]) ? 1 : 0;
    };
    int cnt = 0;
    f(i, n) cnt += check(i);
    f(_, k) {
        int i, j; cind>>i>>j;
        int u = a[i], v = a[j];
        set<int> s = {u, v, p[u], p[v]};
        for(int ii : s) {
            if(ii != -1) cnt -= check(ii);
        }
        if (p[u] != -1) ch[p[u]].erase(i);
        if (p[v] != -1) ch[p[v]].erase(j);
        swap(a[i], a[j]);
        swap(pos[u], pos[v]);
        if (p[u] != -1) ch[p[u]].insert(j);
        if (p[v] != -1) ch[p[v]].insert(i);
        for(int ii : s) {
            if(ii != -1) cnt += check(ii);
        }
