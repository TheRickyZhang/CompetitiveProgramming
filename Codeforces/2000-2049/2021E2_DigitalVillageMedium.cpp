...
void solve() {
    ...
    sort(all(edges));
    DSU d(n);
    // root i, solution with k=0
    vvi dp(n);
    f(i, n) {
        if(wifi[i]) dp[i] = {0, 0};
        else dp[i] = {0};
    }
    // By going through edges in order we ensure the largest weight connecting groups is this current edge
    for(auto [w, u, v] : edges) {
        if(d.same(u, v)) continue;
        u = d.par(u), v = d.par(v);
        int a = dp[u].size(), b = dp[v].size();
        vi dpp(a+b-1, INFL);
        f(i, dp[u].size()) {
            f(j, dp[v].size()) {
                if(i==0 && j==0) dpp[0] = 0;
                // All servers in one -> carry weight of w * #servers
                if(i > 0 && j == 0) ckmn(dpp[i], dp[u][i] + w * (b-1));
                if(i == 0 && j > 0) ckmn(dpp[j], dp[v][j] + w * (a-1));
                // Both have servers -> independent sum
                if(i > 0 && j > 0) ckmn(dpp[i+j], dp[u][i] + dp[v][j]);
            }
        }
        d.merge(u, v);
        dp[d.par(u)] = dpp;
