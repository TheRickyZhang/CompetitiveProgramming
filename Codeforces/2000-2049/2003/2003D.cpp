...
void solve() {
    ...
    f(i, n) {
        ...
        f(j, x) cin>>nums[i][j];
        sort(all(nums[i]));
        nums[i].erase(unique(all(nums[i])), nums[i].end());
    }
    vi a(n), b(n);
    f(i, n) {
        int aa = 0;
    ...
    // Lol I'm actually overcomplicating it, graph-like traversal not necessary
    // map<int, int> best;
    // function<void(int)> dfs = [&](int u) {
    //     best[u] = max(u, *max_element(all(a)));
    //     for(int v : adj[u]) {
    //         if(u==v) continue;
    //         dfs(v);
    //         ckmx(best[u], best[v]);
    ...
    // for(int x : a) dfs(x);
    // cout<<best<<en;
    // int mx = best.rbegin()->ff;
    // for(int i = 0; i <= mx; ++i) {
    //     if(best.count(i)) res += best[i];
    //     else res += max(best[i], mx);
    // }
    auto calculate = [&](int x) {
        return x*(x+1)/2;
    };
    int best = *max_element(all(b));
    int res = 0;
    res += best * (min(best, k)+1);
    res += calculate(k) - calculate(min(best, k));
    cout<<res<<en;
}
