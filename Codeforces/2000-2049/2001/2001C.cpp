...
void solve() {
    cin>>n;
    set<int> remain;
    fe(i, n) remain.insert(i);
    vpii res;
    unordered_set<pii, pairHash> vis;
    // Start with u, v;
    function<void(int, int)> query = [&](int l, int r) {
        if(vis.count({l, r})) return;
        vis.insert({l, r});
        remain.erase(l); remain.erase(r);
        int x = ask(l, r);
        if(x == l) res.pb({l, r});
        else {
            query(l, x);
    ...
    query(1, n);
    // Assume that r is already known.
    function<void(int, int)> query2 = [&](int l, int r) {
        if(vis.count({l, r})) return;
        vis.insert({l, r});
        remain.erase(l); remain.erase(r);
        int x = ask(l, r);
