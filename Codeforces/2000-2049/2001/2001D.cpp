...
//         avail.push({last[i], i});
//     }
//     priority_queue<pii, vpii, cmp> mx;
//     mpq<pii> mn;
//     vb used(n, false);
//     vi res;
//     int l=0, it=0;
//     while(!avail.empty()) {
//         while(!avail.empty() && avail.top().ff < l) avail.pop();
//         int r = avail.top().ff;
//         while(it <= r && it < n) {
//             mx.push({a[it], it});
//             mn.push({a[it], it});
//             it++;
//         }
//         while(!mx.empty() && mx.top().ss < l) mx.pop();
//         while(!mn.empty() && mn.top().ss < l) mn.pop();
//         int val, pos;
//         if(res.size() % 2 == 0) {
//             while(!mx.empty() && used[mx.top().ff]) mx.pop();
//             if(mx.empty()) break;
//             auto [x, i] = mx.top(); mx.pop();
//             val=x, pos=i;
//         } else {
//             while(!mn.empty() && used[mn.top().ff]) mn.pop();
//             if(mn.empty()) break;
//             auto [x, i] = mn.top(); mn.pop();
//             val=x, pos=i;
//         }
//         res.pb(val);
//         used[val]=true;
//         while(!avail.empty() && used[avail.top().ss]) avail.pop();
//         l=pos+1;
//     }
//     cout<<res.size()<<en;
//     for(int x : res) cout<<x+1<<sp;
//     cout<<en;
//     // Segtree<mxnode> mx(n, mxnode(0, n+1), mxcombine);
//     // Segtree<mnnode> mn(n, mnnode(INFL, n+1), mncombine);
//     // f(i, n) {
//     //     mx.update(i, mxnode(a[i], i));
//     //     mn.update(i, mnnode(a[i], i));
//     // }
//     //
//     // vi last(n, -1);
//     // f(i, n) last[a[i]]=i;
...
//     //     int pos = *ends.begin();
//     //     if(i % 2 == 0) {
//     //         auto node = mx.query(curr, pos);
//     //         res.pb(node.x);
//     //         curr = node.i+1;
//     //         cout<<node.x<<sp<<curr<<en;
//     //         ends.erase(last[node.x]);
//     //     } else {
//     //         auto node = mn.query(curr, pos);
//     //         res.pb(node.x);
//     //         curr = node.i+1;
//     //         cout<<node.x<<sp<<curr<<en;
//     //         ends.erase(last[node.x]);
