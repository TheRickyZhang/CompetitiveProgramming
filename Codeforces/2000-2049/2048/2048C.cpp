...
struct trie {
    ...
    trie() : root(new tn()) {}
    ~trie() { delete root;}
    void insert(str s, int l, int r) {
        auto it = root;
        for(char c : s) {
            int i = c-'0';
            if(!it->ch[i]) it->ch[i] = new tn();
            it = it->ch[i];
        }
        it->range = {l, r};
    }
...
void solve() {
    ...
    str mx = "", l = 0, r = 0;
    f(i, n-m) {
        str candidate = s.substr(i, m);
        cout<<"BEFORE "<<sxor(fs, candidate)<<en;
        str val = sxor(fs, candidate);
        if(val> mx) {
            mx = val; l = i; r = i+m-1;
        }
    }
    cout<<1<<sp<<n<<sp<<l<<sp<<r<<en;
}
