...
template<typename A, typename B> ostream& operator<<(ostream &os, const pair<A, B> &p) { return os<<"("<<p.first<<", "<<p.second<<")"; } constexpr char sp = ' ';
...
void solve(int x) {
    if(x % 2 == 1) {
        if(x > 7) {
            int a = c++, b = c++;
            cout<<a<<sp<<a<<sp<<b<<sp<<c<<sp<<c<<sp<<a<<sp<<b<<sp; c++;
            solve(x-7);
        } else {
            cout<<-1<<en; return;
        }
