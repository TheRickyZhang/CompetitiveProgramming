...
void solve() {
    ...
    f(x, min(n, m)/2) {
        str curr;
        for(int j = x; j < m-x; ++j) curr += grid[x][j];
        for(int i = x+1; i < n-x-1; ++i) curr += grid[i][m-x-1];
        for(int j = m-x-1; j >= x; --j) curr += grid[n-x-1][j];
        for(int i = n-x-2; i>=x+1; --i) curr += grid[i][x];
        strips[x] = curr;
    }
    // cout<<strips<<en;
    int cnt = 0;
    for(str& s : strips) {
        s += s.substr(0, 3); // Accounts for cycling
        n = s.size();
        f(i, n-3) {
