...
void solve() {
    ...
    vvi a(m, vi(n, 0));
    f(j, m) a[j][0] = grid[0][j];
    f(i, n-1) f(j, m) a[j][i+1] = a[j][i] | grid[i+1][j];
    // cout<<a<<en;
    f(i, k) {
        int q; cin>>q;
        int l=0, r=n-1;
        f(_, q) {
            ...
            if(l > r) continue;
            if(c=='<') {
                auto it = less_bound(all(a[x]), y);
                if(it==a[x].end()) r=-INFL;
                else ckmn(r, it-a[x].begin());
            } else {
                auto it = upper_bound(all(a[x]), y);
                if(it==a[x].end()) l=INFL;
                else ckmx(l, it-a[x].begin());
            }
        }
        if(l > r) cout<<-1<<en;
        else cout<<l+1<<en;
