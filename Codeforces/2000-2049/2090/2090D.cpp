...
tpl_<class It, class T>     auto leq_bound  (It first, It last, T val) { auto it = r_bound(first, last, val); return it != first ? prev(it) : last;} tpl_<class C, class T>auto leq_bound(C& c, T val) {auto it = c.r_bound(val);return it != c.begin() ? prev(it) : c.end();}
tpl_<class It, class T>     auto less_bound (It first, It last, T val) {auto it = l_bound(first, last, val);return it != first ? prev(it) : last; }tpl_<class C, class T>auto less_bound(C& c, T val) {auto it = c.l_bound(val);return it != c.begin() ? prev(it) : c.end();}
tpl_<class C, class T>      auto l_bound(C& c, T val) {return c.l_bound(val);} tpl_<class C, class T>auto r_bound(C& c, T val) {return c.r_bound(val);}
...
void solve(){
    ...
    int p;
    int diff = INFL;
    auto rit = lower_bound(all(primes), m);
    auto lit = upper_bound(all(primes), m);
    if(rit == primes.begin()) rit = primes.end();
    rit = prev(rit);
    if(lit != primes.end()) {
        if(*lit - m < diff) {
            p = *lit;
