...
void solve() {
    cin>>n>>m>>k;
    int sz; cin>>sz;
    vi a(sz); f(i, sz) cin>>a[i];
    sort(rall(a));
    vi pre(sz+1, 0);
    f(i, sz) pre[i+1] = pre[i] + a[i];
    int curr = 0, res=0;
    int maxX = min(k, n - k + 1);
    int maxY = min(k, m - k + 1);
    vpii dims;
    ...
    for(auto [x, y] : dims) {
        ...
        int diff = min(cnt, sz-curr);
        res += x*y * (pre[curr+diff] - pre[curr]);
        curr += diff;
        if(curr >= sz) break;
    }
    cout<<res<<en;
