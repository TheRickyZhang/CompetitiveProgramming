...
int t, k, n, m;
// Not sure what is incorrect with this
// void solve() {
//     cin>>n>>m;
//     v<v<iii>> adj(n);
//     int c, a, b; cin>>c>>a>>b;
...
//     pq<pii> q;
//     vi end(n, -INFL);
//     end[n-1] = c; q.push({c, n-1});
//
//     auto dfs = [&](int u, int x) {
//         stack<pii> s;
//         s.push({x, u}); ckmx(end[u], x);
//         while(!s.empty()) {
//             auto [t, u] = s.top(); s.pop();
//             if(t <= a) continue;
//             for(auto [v, x, y] : adj[u]) {
//                 int nt = t-y;
//                 if(nt < end[v]) continue;
//                 end[v] = nt;
//                 s.emplace(nt, v);
//             }
//         }
...
//         for(auto [v, x, y] : adj[u]) {
//             if(t-x <= end[v]) continue;
//             if(t-x < b) {
//                 dfs(v, t-y);
//             } else {
//                 end[v] = t-x;
//                 q.emplace(t-x, v);
//                 cout<<"emplace "<<t-x<<sp<<v<<en;
//             }
//         }
//     }
//     cout<<end<<en;
...
//     qq.push({0, 0}); vis[0] = 0;
//     while(!qq.empty()) {
//         auto [t, u] = qq.top(); qq.pop();
//         if(end[u] != -INFL) {
//             ckmx(res, end[u] - t);
//             // cout<<"HERE "<<end[u]<<sp<<t<<en;
//         }
//         for(auto [v, x, y] : adj[u]) {
//             int nt = t+x;
//             if(nt > a || nt > vis[v]) continue;
//             qq.push({nt, v});
//             vis[v] = nt;
//         }
//     }
