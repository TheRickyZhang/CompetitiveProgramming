...
void solve() {
    cin >> n >> k;
    vpii rects(n);
    f(i, n) cin >> rects[i].ff >> rects[i].ss;
    vvi dp(n);
    f(pos, n) {
        auto [xx, yy] = rects[pos];
        dp[pos].assign(xx+yy+1, INFL);
        dp[pos][0] = 0;
        fe(c, xx+yy) {
            for(int i = 0; i <= min(xx, c); ++i) {
                for(int j = 0; i+j <= c; ++j) {
                    ckmn(dp[pos][i+j], i*yy + j*xx - i*j);
                }
            }
        }
    }
    ...
    f(i, n) {
        ...
        fe(i, min(sz-1, k)) {
            for(int x = k; x-i >= 0; x--) {
                ckmn(temp[x], res[x-i] + dpp[i]);
            }
        }
        res = temp;
    }
    if(res[k] == INFL) cout << -1 << en;
    else cout << res[k] << en;
}
