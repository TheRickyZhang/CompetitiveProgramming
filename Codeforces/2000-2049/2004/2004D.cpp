...
void solve() {
    ...
    f(i, n) {
        cin>>a[i];
        sort(all(a[i])); // Sort lexi
    }
    vs cities = {"BG", "BR", "BY", "GR", "GY", "RY"};
    // cout<<a<<en;
    // 01, 02, 03, 12, 13, 23
    v<set<int>> pairs(6);
    f(i, n) {
        f(j, 6) {
            if(a[i] == cities[j]) pairs[j].insert(i);
        }
    }
    // cout<<pairs<<en;

    auto shares = [&](int x, int y) {
        const str& xx = a[x], yy = a[y];
        return (xx[0]==yy[0] || xx[1]==yy[1] || xx[1]==yy[0] || xx[0]==yy[1]);
    };
    ...
    f(_, k) {
        ...
        if(shares(x, y)) {
            ...
            int curr = INFL;
            f(j, 6) {
                if(a[x] != cities[j] && a[y] != cities[j]) {
                    auto rit = pairs[j].lower_bound((x+y)/2);
                    auto lit = pairs[j].upper_bound((x+y)/2);
                    if(lit==pairs[j].begin()) lit = pairs[j].end();
                    else lit = prev(lit);
                    if(rit != pairs[j].end()) ckmn(curr, abs(x-*rit) + abs(y-*rit));
                    if(lit != pairs[j].end()) ckmn(curr, abs(x-*lit) + abs(y-*lit));
                }
            }
            if(curr == INFL) cout<<-1<<en;
            else cout<<curr<<en;
        }
    }
}
