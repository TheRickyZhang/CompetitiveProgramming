...
//     }
//     vi diff(n, 0);
//     f(i, n-1) diff[i] = nums[i+1]-nums[i];
//     diff[n-1] = INF;
//
//     // NOTE: Only odd positions will make a difference
//     vi pre(n+1, 0);
//     f(i, n) pre[i+1] = pre[i] + diff[i];
//     vi pretot(n+1, 0);
//     f(i, n) pretot[i+1] = pretot[i] + diff[i] * ((i+2)/2);
//     cout<<pre<<en<<pretot<<en;
//
//     int x = fstTrue(0LL, n-1, [&](int m) {
//         if(m==n-1) return true;
//         return pretot[m+1] > k;
//     });
//     cout<<x<<sp<<score<<en;
//
//     // Now simulate count manually
//     int res = pre[x];
//     k -= pretot[x];
//     res += k/((x+2)/2);
//     cout<<score-res<<en;
