...
void solve() {
    ...
    vi a(n, 0); // prefix for how many creatures satisfy
    vi b(n, 0);
    int x = 0;
    f(i, n) {
        x += nums[i];
    ...
    // cout<<pre<<en<<a<<en<<b<<en;
    // Handle all cases separately
    int res = -1;
    int i = fstTrue(0LL, n-1, [&](int mid) {
        return a[mid] >= m;
    });
    int j = lstTrue(0LL, n-1, [&](int mid) {
        return b[mid] >= m;
    });
    if(i >= 0 && i <= n-1) ckmx(res, pre[n] - pre[i+1]);
    if(j >= 0 && j <= n-1) ckmx(res, pre[j]);

    // cout<<"After checking begin and end "<<res<<en;
    f(i, n-1) {
        int need = m-a[i];
        int ii = lstTrue(i+1, n-1, [&](int mid) {
            if(mid <= i || mid >= n) {
                cout<<"HUH "<<i+1<<sp<<n-1<<en;
            }
            return b[mid] >= need;
        });
        // cout<<i<<sp<<ii<<en;
        if(ii <= n-1) {
            ckmx(res, pre[ii] - pre[i+1]);
            // cout<<"for i="<<i<<sp<<res<<sp<<ii<<sp<<i+1<<en;
        }
    }
    cout<<res<<en;
}
