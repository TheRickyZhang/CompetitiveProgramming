...
struct PersSegtree {
    ...
    {
        pool.reserve(n + n * static_cast<int>(ceil(log2(n))));
    }
    ...
    int _modify(int i, int l, int r, int p, T x, bool upd) {
        int u = nodes(z);
        if (l == r) {
            T prev = (i<0 ? z : pool[i].x);
            pool[u].x = upd ? c(prev, x) : x;
            return u;
        }
        int m = (l + r) >> 1;
        if (p <= m) {
            int nl = _modify(i < 0 ? -1 : pool[i].l, l, m, p, x, upd);
            pool[u].l = nl;
            pool[u].r = i < 0 ? -1 : pool[i].r;
        } else {
            int nr = _modify(i < 0 ? -1 : pool[i].r, m+1, r, p, x, upd);
            pool[u].r = nr;
            pool[u].l = i < 0 ? -1 : pool[i].l;
        }
        T vl = pool[u].l < 0 ? z : pool[pool[u].l].x;
        T vr = pool[u].r < 0 ? z : pool[pool[u].r].x;
        pool[u].x = c(vl, vr);
        return u;
    }
    T _query(int i, int l, int r, int a, int b) {
        if (i < 0 || a > r || b < l) return z;
        if (a <= l && r <= b) return pool[i].x;
        int m = (l + r) >> 1;
        return c(_query(pool[i].l, l, m, a, b), _query(pool[i].r, m+1, r, a, b));
    }
    int update(int rt, int p, T x) {
        return _modify(rt, 0, n-1, p, x, true);
    }
    int add(int rt, int p, T x) {
        return _modify(rt, 0, n-1, p, x, false);
    }
    T query(int rt, int a, int b) {
        return _query(rt, 0, n-1, a, b);
    }
};
