...
void solve() {
    ...
    // sz has # components if delete + 1
    vpii nums(n);
    f(i, n) nums[i] = {deg[i], i};
    sort(rall(nums));

    // After sorting nums and setting mx as before
    int mx=nums[0].ff; int cnt=0;
    for (auto &p : nums) {
        if (p.ff == mx) cnt++;
    }
    if(cnt >= 3) {
    ...
    if (cnt == 2) {
        int u = nums[0].ss, v = nums[1].ss;
        int temp = adj[u].count(v);
        cout<<2*mx - 1 - temp<<en;
        return;
    }

    mx = nums[0].ff + nums[1].ff;
    rep(j, 1, nums.size()-1) {
        auto [a, u] = nums[0]; auto [b, v] = nums[j];
        int sum = a+b; if(sum < mx) break;
        int temp = adj[u].count(v);
        if(temp==0) {
            cout<<mx-1<<en; return;
        }
