...
void solve() {
    ...
    vi left, right, mid;
    f(i, n) {
        if(i < l) left.pb(nums[i]);
        else if(i > r) right.pb(nums[i]);
        else mid.pb(nums[i]);
    }
    sort(all(left)); sort(all(right)); sort(rall(mid));
    int k = mid.size();
    int a=0, b=0, c=0;
    int res = 0;
    // cout<<left<<en<<right<<en<<mid<<en;
    f(i, k) {
        c += mid[i];
        if(i < left.size()) {
            a += left[i];
    ...
    cout<<accumulate(all(mid), 0LL) + res<<en;
}
