...
void solve() {
    ...
    // [0] = odd, [1] = even
    v<stack<int>> par(2);
    // Note that par assumes ->; use mirror trick
    for(int i : pos) {
        int x = nums[i];
        int p = x % 2;
        if(dirs[i] == -1) {
            if(par[p].empty()) {
                par[p].push(i); // We push NEGATIVE if still travet
            } else {
                int j = par[p].top(); par[p].pop();
                int actual = dirs[j]==-1 ? -nums[j] : nums[j];
                res[i] = res[j] = (nums[i]-actual)/2;
            }
        } else {
            par[p].push(i);
        }
    ...
    f(p, 2) {
        while(par[p].size() >= 2) {
            int i = par[p].top(); par[p].pop();
            int j = par[p].top(); par[p].pop();
            int actual = dirs[j]==-1 ? -nums[j] : nums[j];
            res[i] = res[j] = (2*m - nums[i] - actual) / 2; // Mirror, considering a bounces
        }
    }
