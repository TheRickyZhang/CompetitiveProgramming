...
/*
 * Greedy approach doesn't account for dependencies between interactions. If we intend to swap a[i][j] and a[j][i] we also swap every other pair, whicn affects the state of other swaps
 *
 * Given all pairs [i][j], [j][i], we can determine wheterh optimal is to even swaps (uncahged) or odd swaps (changed)
 *
 * This dependency is resolved with parity DSU - ie -par[u] is inverted state
 * KEY IMPLEMENTATION: It is NOT guaranteed that [i][j] < [j][i] for all i<j. BUT, by processing nodes in
 * f(i, n) f(j, n) order, we can lock in some values earlier on that prevent later adjustments to nums[j][i]
 */
void solve() {
    cin>>n;
    vvi grid(n+1, vi(n+1, 0));
    f(i, n) f(j, n) cin>>grid[i+1][j+1];
    vi p(n+1, 0);
    f(i, n+1) p[i] = i;
    ...
    auto merge = [&](int u, int v) {
        u = par(u), v = par(v);
        if(abs(u)==abs(v)) return;
        if(u > 0) p[u] = v;
        else p[-u] = -v;
    };
    fe(i, n) {
        fe(j, n) {
            // Note we are performing on grid[i][j].
            // So if it is already minimized, even parity since no change
            if(grid[i][j] < grid[j][i]) merge(i, j);
            if(grid[i][j] > grid[j][i]) merge(i, -j);
        }
    }
    ...
    fe(i, n) {
        if(par(i) < 0) continue; // Don't swap this
        fe(j, n) {
