...
constexpr ll MOD=998244353; constexpr int INF=1000000000; constexpr ll INFL=0x3f3f3f3f3f3f3f3f; constexpr auto en = "\n"; constexpr auto sp = " ";
...
int main() {
    ...
    repr(i, k-1, 0) {
        ll curr=0;
        vll freq = {1, 0}, sum = {-1, 0}; // 0-indexed, where pre = -1
        f(j, n) {
            ll b = (nums[j]>>i) & 1;
            curr ^= b;
            res = add(res, ((freq[1-curr]*j - sum[1-curr]) % MOD)<<i);
            freq[curr]++; sum[curr] += j;
        }
    }
    cout<<res<<en;
}
