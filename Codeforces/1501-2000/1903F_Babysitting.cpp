...
int32_t main(){
    ...
    f(i, n){
        ...
        int aa = neg(a), bb = neg(b);
        if(nums[i] == 0){
            // Clause: (a or b) becomes: a -> ¬b, b -> ¬a, etc.
            adj[a].push_back(bb); adj[b].push_back(aa);
            adj[bb].push_back(a); adj[aa].push_back(b);
            radj[a].push_back(bb); radj[b].push_back(aa);
            radj[bb].push_back(a); radj[aa].push_back(b);
        } else {
            // Clause: (a xor b) is encoded as: a -> b, b -> a and ¬a -> ¬b, ¬b -> ¬a.
            adj[a].push_back(b); adj[b].push_back(a);
            adj[aa].push_back(bb); adj[bb].push_back(aa);
            radj[a].push_back(b); radj[b].push_back(a);
            radj[aa].push_back(bb); radj[bb].push_back(aa);
        }
    }

    vi order; vb vis(2*m, false);
