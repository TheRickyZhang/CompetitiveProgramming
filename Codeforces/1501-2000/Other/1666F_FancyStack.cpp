...
void solve() {
    ...
    // Blocks considered, even spots filled
    vvi dp(n+1, vi(n/2 + 1));
    dp[0][0] = 1;
    int i = 0;
    for(auto [x, freq] : mp) {
        // cout<<"processing "<<x<<sp<<freq<<en;
        auto avail = [&](int j) {
            return max(j-1, 0LL) + (j==n/2) - (i-j);
        };
        if(freq == 1) {
            rep(j, 0, n/2) {
                dp[i+1][j] = add(dp[i+1][j], mult(dp[i][j], avail(j))); // Place in odd position
                if(j+1 <= n/2) dp[i+1][j+1] = add(dp[i+1][j+1], dp[i][j]);      // Place in even position
            }
            i++;
        } else {
            // Place all in odd positions
            rep(j, 0, n/2) {
                dp[i+freq][j] = add(dp[i+freq][j], mult(dp[i][j], choose(avail(j), freq)));
            }
            // Place 1 in even position, rest in odd
            rep(j, 0, n/2 - 1) {
                // cout<<"trying "<<avail(j)+1<<sp<<freq-1<<endl;
                if(j+1 <= n/2) dp[i+freq][j+1] = add(dp[i+freq][j+1], mult(dp[i][j], choose(avail(j), freq-1)));
            }
            i += freq;
        }
    }
