...
void solve() {
    ...
    f(i, n) {
        cin>>nums[i];
        if(s.empty() || nums[i] < nums[*s.rbegin()]) s.insert(i);
    }
    f(i, m) {
        int j, d; cin>>j>>d; j--;
        nums[j] -= d;
        // Get the current train begin - if j is slower also a begin
        auto it = prev(s.upper_bound(j));
        if(nums[*it] > nums[j]) s.insert(j);  // Note since 0 is in the set should never be invalid s.end()

        while(true) {
            it = s.upper_bound(j);
            if(it != s.end() && nums[*it] >= nums[j]) s.erase(it);
            else break;
        }
        cout<<s.size()<<sp;
    }
