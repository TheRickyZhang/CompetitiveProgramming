...
// Combining prev mapping with binary lifting.
// Tight time limit = use smallest prime factor for factorizing!
int t, k, n, m;
...
int32_t main() {
    ...
    vi mp(n+1, INF);
    vi next(n); // Next that is relatively composite
    repr(i, n-1, 0) {
        int x = nums[i];
        int curr = n;
        while (x > 1) {
        ...
        if(i != n-1) ckmn(curr, next[i+1]);
        next[i] = curr;
    }
    // cout<<next<<en;

    int ln = log2(100000) + 1;
    vvi lift(n, vi(ln, 0));
    f(i, n) lift[i][0] = next[i];
    fe(j, ln-1) {
        f(i, n) {
            int prev = lift[i][j-1];
            lift[i][j] = (prev == n ? n : lift[prev][j-1]);
        }
    }
    // cout<<lift<<en;
