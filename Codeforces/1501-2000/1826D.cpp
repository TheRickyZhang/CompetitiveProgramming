...
void solve() {
    ...
    vi left(n), right(n);
    f(i, n) {
        if(i==0) left[i] = nums[i]+i;
        else {
            left[i] = max(nums[i]+i, left[i-1]);
        }
    }
    repr(i, n, 0) {
        if(i==n-1) right[i] = nums[i]-i;
        else {
            right[i] = max(nums[i]-i, right[i+1]);
        }
    }
    ...
    int res=0;
    f(i, n-2) {
        // cout<<left[i]+nums[i+1]+right[i+2]<<en;
        res = max(res, left[i]+nums[i+1]+right[i+2]);
    }
    cout<<res<<en;
}
