...
template<class T> bool       ckmn(T& a, const T& b) {return b < a ? a = b, 1 : 0;}  template<class T> bool ckmx(T& a, const T& b) {return a < b ? a = b, 1 : 0;}
    int N = 20000; int MOD=1e9+7; constexpr int INF=1e9; constexpr int INFL=0x3f3f3f3f3f3f3f3f; constexpr auto en = "\n"; constexpr auto sp = " ";
...
void solve() {
    ...
    vi prev(n+1, INF);
    f(i, n) {
        dp[i+1] = min(dp[i]+1, prev[nums[i]]);
        ckmn(prev[nums[i]], dp[i]); // We can mark the previous bests as we go along
    }
    // cout<<dp<<en;
    cout<<n-dp[n]<<en;
}
