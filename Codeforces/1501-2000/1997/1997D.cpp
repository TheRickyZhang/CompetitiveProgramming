...
void solve() {
    ...
    f(i, n-1) {
        int p; cind>>p;
        adj[i+1].pb(p), adj[p].pb(i+1);
    }

    int res = lstTrue(a[0], a[0] + INFL, [&](int m) {
        // Accumulated extra cost
        vi b(n, 0);
        int x = m - a[0];
        b[0] = x;
    ...
    // int res = lstTrue(a[0], a[0] + 1000, [&](int m) {
    //     vi b = a;
    //     int val = m - a[0];
    //     cout<<val<<sp;
    //     function<bool(int, int, int)> lift = [&](int u, int p, int x) {
    //         if(u != 0 && adj[u].size() == 1) {
    //             return b[u] >= x;
    //         }
    //         for(int v : adj[u]) {
    //             if(v==p) continue;
    //             if(b[v] < x) {
    //                 if(!lift(v, u, 2*x - b[v])) return false;
    //             } else {
    //                 if(!lift(v, u, x)) return false;
    //             }
    //         }
    //         return true;
    //     };
