...
bool good(str& s) {
    int r = 0;
    f(i, s.size()) {
        if(i % 2 == 0 && (s[i]-'0' != dd)) return false;
        if(i % 2 == 1 && (s[i]-'0' == dd)) return false;
        r = (r * 10 + s[i]-'0') % m;
    }
    return r == 0;
}
...
int solve(str& s) {
    int n = s.size();
    f(i, n+1) f(j, m) f(k, 2) dp[i][j][k] = 0; // Important - this is n+1!
    dp[0][0][1] = 1;
    f(i, n) {
        int x = s[i]-'0';
        f(j, m) {
            f(k, 2) {
                for(int d = 0; d <= (k ? x : 9); ++d) {
                    if((i%2 == 0) && d == dd) continue;
                    if((i%2 == 1) && d != dd) continue;
                    if(!i && !d) continue; // Leading 0s
...
int main() {
    ...
    cin>>m>>dd;
    str a, b; cin>>a>>b;
    int res = solve(b) - solve(a) + good(a);
    cout<<res<<en;
}
