...
int32_t main() {
    ...
    // {node, dist in subtree from node} -> cnt
    vvi dp(n, vi(k+1, 0));
    f(i, n) dp[i][0] = 1; // Every node is 0 to itself
    int res = 0;
    autotree dfs = [&](int u, int p) {
        for(int v : adj[u]) {
            ...
            // This is the smart way to accumulate pairwise children in O(n) rather than O(n^2); Note we do it first!
            // Idea is take 1/2 of the curr*(other-curr) matching === count first, update after per child; Instead of 1+...n, 0+...n-1 is proper pairwise sum
            f(i, k) res += dp[u][i] * dp[v][k-i-1];

            f(i, k) dp[u][i+1] += dp[v][i];
        }
    };
    dfs(0, 0);
    cout<<res<<en;
