...
template <typename T, typename C> /* Usage: SegTree<int, plus<int>> t(n, 0, plus<int>()) | Segtree<Node, Combine> t(n, node(), Combine()) */
struct Segtree { ll n; v<T> t; T z; C c;
        Segtree(ll sz, T zero, C combine) : n(sz), t(4 * sz, zero), z(zero), c(combine) {}
    void update(ll i,ll a,ll b,ll p,T x){if(a==b){t[i]=x;return;}ll m=(a+b)/2;if(p<=m)update(2*i,a,m,p,x);else update(2*i+1,m+1,b,p,x);t[i]=c(t[2*i],t[2*i+1]);}                     void update(ll p, T x) { update(1, 0, n - 1, p, x); }
...
int main() {
    ...
    sort(all(dist)); // Get min dists first;
    ...
    avail[0].pb({dist[0].ss, k});
    for(int i=1; i<n; ++i) {
        auto [d, u] = dist[i];
        if(d-1 < 0 || d-1 >= n || avail[d-1].empty() || (k==1 && i<n-1)) {
            cout<<-1; return 0;
        }
        auto& [v, kk] = avail[d-1].back();
        if(--kk == 0) avail[d-1].pop_back();
        avail[d].pb({u, k-1});
        res.pb({u, v});
    }
    cout<<res.size()<<en;
    for(auto [a, b] : res) cout<<a+1<<sp<<b+1<<en;
}
