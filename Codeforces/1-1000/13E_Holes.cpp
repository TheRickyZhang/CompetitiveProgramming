...
int32_t main() {
    ...
    // We partition n into blocks. Each item in block stores:
    // Pointer to next block, how many steps
    vpii dp(n);

    repr(i, n-1, 0) {
        int r = min(n, i/m * m + m); // Floor mod m, add m
        if(i + nums[i] >= r) {
            dp[i] = {i+nums[i], 1};
        } else {
            dp[i] = dp[i+nums[i]]; dp[i].ss++;
        }
    }
    // cout<<dp<<en;
    f(i, k) {
        ...
        if(t==0) {
            int a, x; cin>>a>>x; a--;
            nums[a] = x;
            int l = a/m * m; int r = min(n, l+m);
            repr(i, r-1, l) { // Need to recompute each block
                if(i + nums[i] >= r) dp[i] = {i+nums[i], 1};
                else {
                    dp[i] = dp[i+nums[i]];
                    dp[i].ss++;
                }
            ...
            while(a < n) {
                auto [next, cnt] = dp[a];
                if(next >= n) { // Manually calculate to get last
                    while(a < n) {
                        res++; last=a; a += nums[a];
                    }
                } else {
                    res += cnt; last=a; a = next;
                }
            }
            cout<<last+1<<sp<<res<<en;
        }
    }
}
