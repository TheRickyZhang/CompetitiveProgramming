...
int32_t main() {
    ...
    f(i, n - 1) {
        cin >> cur;
        int leng = min(prev.size(), cur.size()), same = 0;
        while (same < leng && prev[same] == cur[same])
            same++;
        if (same < leng) {
            int u = prev[same] - 'a', v = cur[same] - 'a';
