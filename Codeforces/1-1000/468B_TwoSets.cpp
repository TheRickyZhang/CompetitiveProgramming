...
int32_t main() {
    ...
    vi nums(n);
    f(i, n) cin >> nums[i];
    // Remove the sort(all(nums)); line
    map<int, vector<int>> mp;
    f(i, n) mp[nums[i]].push_back(i);
    DSU d(n);
    f(i, n) {
        if(mp.count(a - nums[i])) {
            for(auto j : mp[a - nums[i]])
                d.merge(i, j);
        }
        if(mp.count(b - nums[i])) {
            for(auto j : mp[b - nums[i]])
                d.merge(i, j);
        }
    }
    vb musta(n, false), mustb(n, false);
    f(i, n) {
        bool aa = mp.count(a - nums[i]), bb = mp.count(b - nums[i]);
        if(aa && !bb) musta[d.par(i)] = true;
        if(!aa && bb) mustb[d.par(i)] = true;
        if(!aa && !bb) {
            cout << "NO" << en;
            return 0;
        }
