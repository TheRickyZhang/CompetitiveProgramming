...
constexpr int N = 500; constexpr ll MOD=1000000007; constexpr int INF=1000000000; constexpr ll INFL=0x3f3f3f3f3f3f3f3f; constexpr auto en = "\n"; constexpr auto sp = " ";
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        int x = c[i];
        for (int price = k; price >= x; --price) {       // Go through reverse
            for (int sum = price - x; sum >= 0; --sum) { // If we could make this before, update doubly
                if (dp[price - x][sum]) {
                    dp[price][sum + x] = true;
                    dp[price][sum] = true;
                }
