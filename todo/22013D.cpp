...
int k, n, m;
static inline int bounded_lcm(int a,int b,int cap){
  int g = gcd(a,b);
  int t = a/g;
  if(t > cap/b) return cap+1; // avoid overflow
  return t*b;
}
...
void solve(){
  ...
  int lim=1;
  for(int x : xs){
    lim = bounded_lcm(lim,x,nm);
    if(lim > nm) break;
  }

  vb div(nm+1, false);
  for(int x:xs) {
    for(int k=x; k<=nm; k+=x) div[k]=true;
  }
