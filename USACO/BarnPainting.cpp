...
int32_t main() {
    ...
    f(i, k) {
        int u, x; cin>>u>>x; u--; x--;
        f(j, 3) dp[u][j] = 0;
        dp[u][x] = 1; // All others are 0
    }
    function<void(int, int)> dfs = [&](int u, int p) {
        for(int v : adj[u]) {
            if(v==p) continue;
            dfs(v, u);
            f(i, 3) { // Loop through options for u
                int temp = 0;     for(int j : inv[i]) temp = add(temp, dp[v][j]);
                dp[u][i] = mult(dp[u][i], temp);
            }
        }
    };
    dfs(0, -1);
