...
int32_t main() {
    ...
    vvpii par(n, vpii(m, {-1, -1})); // Assign parents, also tracks vis
    function<void(int, int, pii)> dfs = [&](int x, int y,const pii& p) {
        if(par[x][y].ff >= 0) return; par[x][y] = p;
        for(auto [dx, dy] : dirs) {
            int nx = x+dx, ny=y+dy;
            if(check(nx, ny, n, m) && grid[nx][ny] == 'X') {
                dfs(nx, ny, p);
            }
        }
    };
    ...
    // cout<<"Par "<<par<<en;
    // cout<<"Shallow: "<<shallow<<en;

    auto minDist = [&](int a, int b, const pii& dest) {
        ...
        while(!q.empty()) {
            ...
            f(i, sz) {
                ...
                // cout<<"searching"<<x<<sp<<y<<en;
                if(vis[x][y]) continue; vis[x][y] = true;
                for(auto [dx, dy] : dirs) {
                    int ny=y+dy, nx = x+dx;
                    if(check(nx,  ny, n, m) && !vis[nx][ny]) {
                        if(par[nx][ny] == dest) {
                            // cout<<"RETURNED "<<cnt<<" For "<<a<<sp<<b<<en;
                            return cnt;
                        }
                        if(grid[nx][ny] == 'S') q.emplace(nx, ny);
                    }
                }
            }
        }
    ...
    int x = parList.size();
    mpq<pair<int, pii>> q; // Min with {cost, pairing}
    f(i, x) {
        f(j, i) {
            int res = INF;
            pii a = parList[i], b = parList[j];
            for(auto [x, y] : shallow) {
                int cnt = 0;
                for(auto [dy, dx] : dirs) {
                    int ny = y+dy, nx=x+dx;
                    if(check(nx, ny, n, m) && par[ny][nx] == a) cnt++;
                }
                if(cnt >= 1) {
                    // cout<<"Trying to match "<<a<<sp<<b<<en;
                    ckmn(res, minDist(x, y, b));
                }
            }
            q.emplace(res, make_pair(i, j));
        }
    }
    // cout<<"components"<<parList<<en;
    // while(!q.empty()) {
    //     auto [a, b] = q.top(); q.pop();
    //     cout<<"cost "<<a<<sp<<"connecting"<<b<<en;
    // }
    DSU dsu(x);
    int merged = 0;
    int res = 0;
    while(!q.empty()) {
        auto [cnt, islands] = q.top(); q.pop();
        auto [i, j] = islands;
        if(dsu.par(i) == dsu.par(j)) continue;
        res += cnt;
        dsu.merge(i, j);
        if(++merged == x-1) break;
    }
