...
int main() {
    ...
    sort(all(nums), [&](cow& a, cow& b) {
        if(a.s == b.s) return a.w > b.w; // Need to have heavier first
        return a.s > b.s;
    ...
    f(i, 1<<n) {
        ...
        f(j, n) {
            if(i & (1<<j)) {
                auto [h, w, s] = nums[j];
                cs = min(cs - w, s); // Current strength able to support if cannot carry
                if(cs < 0) break;
                ch += h;
                if(ch >= hh) {
                    // if(cs > res) cout<<"Maxed at mask"<<i<<" idx"<<j<<en;
                    ckmx(res, cs);
                }
            }
        }
    }
    if(res == -1) cout<<"Mark is too tall"<<en;
    else cout<<res<<en;
}
