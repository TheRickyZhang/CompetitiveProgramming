...
#define cntz __builtin_popcount
...
int32_t main() {
    ...
    // Sort bonuses and convert to prefix sum for faster lookup
    vector<pair<vi, vi>> formattedBonuses(n);
    f(i, n) {
        vpii& bb = bonuses[i];
        sort(all(bb));
        int x=0, y=0;
        for(auto [xx, yy] : bb) {
            x += xx; y+= yy;
            formattedBonuses[i].ff.pb(x);
            formattedBonuses[i].ss.pb(y);
        }
    }
    ...
    f(i, n) {
        f(j, n) {
            cin>>v[j][i]; // Note that this is flipped so that [i] = event index
        }
    }
    // At pos i, choose
    vi dp(1<<n, 0);
    // We don't need outer loop because we can infer index from binary representation
    fe(i, (1<<n)-1) {
        int o = cntz(i)-1;
        f(j, n) {
            if(i & (1<<j)) {
                int prev = i ^ (1<<j);
                ckmx(dp[i], dp[prev] + v[j][o]);
            }
        }
        auto it = leq_bound(all(formattedBonuses[o].ff), dp[i]);
        if(it != formattedBonuses[o].ff.end()) {
            int pos = it-formattedBonuses[o].ff.begin();
            dp[i] += formattedBonuses[o].ss[pos];
        }
    }
    cout<<dp[(1<<n)-1]<<en;
    ...
    //                 if(j & (1<<k)) {
    //                     int prev = j ^ (1<<k);
    //                     ckmx(dpp[j], dp[prev] + v[i][k]);
    //                 }
    //             }
    //             // Add on the bonus
    //             auto it = leq_bound(all(formattedBonuses[i].ff), dpp[j]);
    //             if(it != formattedBonuses[i].ff.end()) {
    //                 int pos = it-formattedBonuses[i].ff.begin();
    //                 dpp[j] += formattedBonuses[i].ss[pos];
    //             }
    //         }
    //     }
    //     dp = dpp;
