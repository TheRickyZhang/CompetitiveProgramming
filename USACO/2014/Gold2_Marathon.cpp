...
struct Node {
    int total, best; // total distance and max saving in this segment
    pair<int,int> first, second, penultimate, last; // endpoints and adjacent values
    int len; // number of checkpoints in the segment
};
...
// Build a leaf node from a single checkpoint.
Node make_node(const pair<int,int>& cp) {
    Node node;
    node.len = 1;
    node.total = 0;
    node.best = 0;
    node.first = node.last = cp;
    node.second = node.penultimate = cp;
    return node;
}

// Merge two nodes representing consecutive segments.
Node merge_node(const Node &L, const Node &R) {
    ...
    M.first = L.first;
    M.last = R.last;
    int cross = manhattan(L.last, R.first);
    M.total = L.total + cross + R.total;
    int bestCandidate = max(L.best, R.best);
    // Candidate: skipping the last point of L (if L has at least 2 points)
    if(L.len >= 2)
        bestCandidate = max(bestCandidate, manhattan(L.penultimate, L.last) + cross - manhattan(L.penultimate, R.first));
    // Candidate: skipping the first point of R (if R has at least 2 points)
    if(R.len >= 2)
        bestCandidate = max(bestCandidate, cross + manhattan(R.first, R.second) - manhattan(L.last, R.second));
    M.best = bestCandidate;
    M.second = (L.len >= 2 ? L.second : R.first);
    M.penultimate = (R.len >= 2 ? R.penultimate : L.last);
    return M;
}
...
int32_t main(){
    ...
    // Build initial segtree leaves.
    vector<Node> init;
    for (auto cp : checkpoints)
        init.push_back(make_node(cp));

    Node zero; // zero node for segtree (empty segment)
    zero.len = 0; zero.total = 0; zero.best = 0;
    zero.first = zero.second = zero.penultimate = zero.last = {0, 0};

    Segtree<Node> seg(N, zero, merge_node, init);

    // Process queries and updates.
    // Query: "Q I J" (1-indexed). Answer is total distance minus best saving.
    // Update: "U I X Y" changes checkpoint I to (X, Y).
    for (int i = 0; i < Q; i++){
        char type;
        cin >> type;
        if(type == 'Q'){
            ...
            cin >> I >> J;
            I--; J--; // make 0-indexed
            Node res = seg.query(I, J);
            cout << res.total - res.best << "\n";
        } else { // update
            int idx, x, y;
            cin >> idx >> x >> y;
            idx--; // 0-indexed
            checkpoints[idx] = {x, y};
            seg.update(idx, make_node({x, y}));
        }
    }
    return 0;
}
