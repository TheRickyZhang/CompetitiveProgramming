...
int32_t main() {
    ...
    auto winner = [&](int x) { return (x+1) % 3; };
    v<vvi> dp(n, vvi(3, vi(kk+1, 0)));
    f(j, 3) dp[0][j][0] = (j == winner(a[0]));
    fe(i, n-1) {
        f(j, 3) {
            int x = (j == winner(a[i]));
            rep(k, 0, kk) {
                ckmx(dp[i][j][k], dp[i-1][j][k] + x);
                if(k > 0) ckmx(dp[i][winner(a[i])][k], dp[i-1][j][k-1]+1);
            }
        }
    }
    int res = 0;
    f(j, 3) f(k, kk+1) ckmx(res, dp[n-1][j][k]);
    cout<<res<<en;
}
