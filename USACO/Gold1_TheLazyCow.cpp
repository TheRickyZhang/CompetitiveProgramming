...
vpii dirs={{1,0},{0,-1},{0,1},{-1,0}}; map<char, int> dirMap={{'E',0},{'S',1},{'N',2},{'W',3}}; auto bound=[](auto y,auto x,auto m,auto n){return y>=0&&y<m&&x>=0&&x<n;};
    ...
    cx_ int N = 1000000; cx_ int MOD=1e9+7; cx_ int INF=1e9; cx_ ll INFL=0x3f3f3f3f3f3f3f3f; cx_ auto en = "\n"; cx_ auto sp = " ";
...
int32_t main() {
    ...
    f(i, n) {
        int g, x, y; cin>>g>>x>>y;
        int xx = x + y, yy = x - y + N;
        int ly = max(0LL, yy - k), ry = min(2*N, yy + k);
        points.pb({xx-k, ly, ry, g});
        points.pb({xx+k+1, ly, ry, -g});
        yvals.pb(ly); yvals.pb(ry);
    }
    sort(all(yvals));
    ...
    for(auto& [_, ly, ry, __] : points) {
        ly = mp[ly], ry = mp[ry];
    }
    sort(all(points), [&](iiii a, iiii b) {
        if(a[0] == b[0]) return a[3] < b[3]; // Process removals first
        return a[0] < b[0];
    });
    int sz = mp.size();
    // { max value, addition
    LazySegtree<int, int> tree(sz,
        [&](int a, int b) { return max(a, b);},
        [&](int a, int op, int l, int r) { return a + op; },
        [&](int o, int p) {
            return o + p;
        });
    int res = 0;
    for(auto [x, ly, ry, g] : points) {
        tree.add(ly, ry, g);
        ckmx(res, tree.query(0LL, sz-1));
    }
    cout<<res<<en;
}
