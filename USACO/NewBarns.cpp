...
int32_t main() {
    ...
    auto build = [&](int u, int v) {
        if(v < 0) {
            ...
            auto [a, b] = diam[r];
            int curr = dist(a, b);
            if(dist(u, a) > curr) diam[r] = {u, a};
            else if(dist(u, b) > curr) diam[r] ={u, b};
        }
    };
    auto query = [&](int u) {
        auto [a, b] = diam[root[u]];
        cout<<max(dist(u, a), dist(u, b))<<en;
    };
    int pos=0;
    f(i, n) {
