...
int32_t main() {
    ...
    Segtree<int> north(n, ad), south(n, ad);
    f(i, n) north.add(a[i].ff, 1);
    int res = n;
    ...
    auto query = [&]() {
        ...
        while(m < 4*n) {
            int l = m*2, r = m*2 + 1;
            if(max(a + north.t[l], c + south.t[l]) < max(b + north.t[r], d + south.t[r])) {
                m = r;
                a += north.t[l], c += south.t[l];
            } else {
                m = l;
                b += north.t[r], d += south.t[r];
            }
        }
        return max({a, b, c, d});
    };
