...
int32_t main() {
    ...
    vi left(n+1, INFL), right(n+1, INFL);
    f(i, n) {
        if(left[a[i]] == INFL) left[a[i]] = i;
        right[a[i]] = i;
    }

    // We might think of using the greedy interval merging strategy
    // But this is actually more like balanced parentheses! Use stack to track possible "partial" overlaps
    stack<int> s;
    s.push(0); left[0] = -1, right[0] = n;
    int res = 0;
    f(i, n) {
        if(i==left[a[i]]) {
            s.push(a[i]);
