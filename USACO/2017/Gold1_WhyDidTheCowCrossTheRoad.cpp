...
void read(vvi &g, int m, bool decrement=true, bool ww=false, bool dir=false){for(int i=0;i<m;i++){int u,v; if(ww){int w;cin>>u>>v>>w;} else cin>>u>>v; if(decrement){u--;v--;} g[u].push_back(v); if(!dir)g[v].push_back(u);}}
...
int32_t main() {
    ...
    f(i, n) {
        f(j, n) {
            rep(ii, 0, 3) {
                ...
                int ni = i + ii, nj = j + jj;
                if(check(ni, nj, n, n)) {
                    adj[to(i, j)].pb({to(ni, nj), a[ni][nj] + 3 * k});
                }
                ni = i - ii, nj = j + jj;
                if(check(ni, nj, n, n)) {
                    adj[to(i, j)].pb({to(ni, nj), a[ni][nj] + 3 * k});
    ...
    rep(ii, 0, 2) {
        rep(jj, 0, 2-ii) {
            ...
            int ni = (n-1) - ii, nj = (n-1) - jj;
            if(check(ni, nj, n, n)) {
                adj[to(ni, nj)].pb({to(n-1, n-1), (ii+jj) * k});
            }
        }
    }

    // Now we just run dijkstra over this new graph
    vi dist(n*n, INFL);
    mpq<pii> q;
    q.push({0, 0}), dist[to(0, 0)] = 0;
    while(!q.empty()) {
