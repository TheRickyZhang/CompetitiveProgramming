...
int main() {
    ...
    cin>>n>>m;
    vpii pos1(n+1), pos2(m+1);
    cin>>pos1[0].ff>>pos1[0].ss>>pos2[0].ff>>pos2[0].ss;
    str a, b; cin>>a>>b;
    f(i, n) {
        pos1[i+1] = pos1[i] + dirs[dirMap[a[i]]];
    }
    f(j, m) {
        pos2[j+1] = pos2[j] + dirs[dirMap[b[j]]];
    }
    ...
    dp[0][0] = 0;
    f(i, n+1) {
        f(j, m+1) { // Note that since we are retrieving values to standardize dp[i][j], the dist is with updated values
            if(i) ckmn(dp[i][j], dp[i-1][j] + dist(pos1[i], pos2[j]));
            if(j) ckmn(dp[i][j], dp[i][j-1] + dist(pos2[j], pos1[i]));
            if(i && j) ckmn(dp[i][j], dp[i-1][j-1] + dist(pos1[i], pos2[j]));
        }
    }
    // cout<<dp<<en;
    cout<<dp[n][m]<<en;
