...
int32_t main() {
    ...
    ump<int, vi> xx, yy;
    vpii points(n+2);
    auto process = [&](int x, int y, int i) {
        points[i] = {x, y};
        xx[x].pb(i); yy[y].pb(i);
    };
    process(a, b, 0);
    ...
    // // Begin is -1, end is n
    // map<int, vi> hor, ver;
    // hor[a].pb(b); ver[b].pb(a);
    // hor[c].pb(d); ver[d].pb(c);
    // map<pii, int> mp;
    // mp[{a, b}] = 0;
    // mp[{c, d}] = n+1;
    // f(i, n) {
    //     int x, y; cin>>x>>y;
    //     hor[x].pb(y); ver[y].pb(x);
    //     mp[{x, y}] = i+1;
    // }
    // vvi adj(n+2);
    // // auto to = [&](int x, int y) {
    ...
    // // for(auto [x, yy] : hor) {
    // //     vi temp;
    // //     for(int y : yy) if(ver.count(y) && ver[y].size() >=1) temp.pb(y);
    // //     for(int i : temp) for(int j : temp) {
    // //         if(i==j) continue;
    // //         adj[mp[{x, i}]].pb(mp[{x, j}]);
    // //     }
    // // }
    // // for(auto [x, yy] : ver) {
    // //     vi temp;
    // //     for(int y : yy) if(hor.count(y) && hor[y].size() >=1) temp.pb(y);
    // //     for(int i : temp) for(int j : temp) {
    // //         if(i==j) continue;
    // //         adj[mp[{i, x}]].pb(mp[{j, x}]);
    // //     }
    // // }
    // // cout<<adj<<en;
    // queue<int> q;
