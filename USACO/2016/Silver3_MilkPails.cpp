...
int32_t main() {
    ...
    setIO("pails");
    int a, b, c, kk; cin>>a>>b>>kk>>c;
    if(a > b) swap(a, b);
    auto to = [&](int x, int y, int z) {
        return x + 101*y + 101*101*z;
    };
    function<iii(int)> get = [&](int x) {
        return make_tuple(x % 101, x/101, x/(101*101));
    };
    vvi adj(301 * 101 * 101);
    // Pour to 3
    rep(i, 0, a) {
        rep(j, 0, b) {
            for(int k = 0; k + i <= 300; ++k) {
                adj[to(i, j, k)].pb(to(0, j,  i+k));
            }
            for(int k = 0; k+j <= 300; ++k) {
                adj[to(i, j, k)].pb(to(i, 0, j+k));
            }
        }
    }
    // Pour 2 to 1
    rep(i, 0, a) {
        rep(j, 0, b) {
            rep(k, 0, 300) {
                if(i+j > a) adj[to(i, j, k)].pb(to(a, i+j-a, k));
                else adj[to(i, j, k)].pb(to(i+j, 0, k));
                if(i+j > b) adj[to(i, j, k)].pb(to(i+j-b, b, k));
                else adj[to(i, j, k)].pb(to(0, i+j, k));
            }
        }
    }
    // Empty and fill each cup
    rep(i, 0, a) {
        rep(j, 0, b) {
            rep(k, 0, 300) {
                adj[to(i, j, k)].pb(to(0, j, k));
                adj[to(i, j, k)].pb(to(i, 0, k));
                adj[to(i, j, k)].pb(to(a, j, k));
                adj[to(i, j, k)].pb(to(i, b, k));
            }
        }
    }
    ...
    while(!q.empty()) {
        ...
        f(i, sz) {
            int u = q.front(); q.pop();
            vis[u]=true;
            auto temp = get(u); auto [x, y, z] = temp;
            // cout<<"checking "<<x<<sp<<y<<sp<<z<<en;
            ckmn(res, abs(c-z));
            for(int v : adj[u]) {
                if(!vis[v]) q.push(v);
            }
