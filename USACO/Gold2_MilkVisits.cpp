...
// NOTE: Euler tour cannot be used here!!
int32_t main(){
    setIO("milkvisits");
    ...
    vi in(n, -1), out(n, -1), depth(n, 0), par(n, -1);
    int t = 0;
    function<void(int,int,int)> dfs = [&](int u, int p, int d){
        par[u] = p; depth[u] = d; in[u] = t++;
        for(auto v: adj[u]){
            if(v == p) continue;
            dfs(v, u, d+1);
        }
    ...
    // Persistent Segment Tree over cow types in [1, n]
    struct Node { int sum, l, r; };
    vector<Node> seg; int segSize = 0;
    function<int(int,int)> build = [&](int L, int R) -> int {
        int idx = segSize++;
        seg.push_back({0, -1, -1});
        if(L == R) return idx;
        int mid = (L+R)/2;
        seg[idx].l = build(L, mid);
        seg[idx].r = build(mid+1, R);
        return idx;
    };
    function<int(int,int,int,int)> update = [&](int prev, int L, int R, int pos) -> int {
        ...
        if(L == R){
            seg[idx].sum = seg[prev].sum + 1;
            return idx;
        }
        int mid = (L+R)/2;
        if(pos <= mid) seg[idx].l = update(seg[prev].l, L, mid, pos);
        else seg[idx].r = update(seg[prev].r, mid+1, R, pos);
        seg[idx].sum = seg[ seg[idx].l ].sum + seg[ seg[idx].r ].sum;
        return idx;
    };
    function<int(int,int,int,int,int)> query = [&](int idx, int L, int R, int ql, int qr) -> int {
        if(ql > R || qr < L) return 0;
        if(ql <= L && R <= qr) return seg[idx].sum;
        int mid = (L+R)/2;
        return query(seg[idx].l, L, mid, ql, qr) + query(seg[idx].r, mid+1, R, ql, qr);
    };
    seg.reserve(40*n); seg.clear(); segSize = 0;
    int base = build(1, n);
    vi ver(n);
    function<void(int,int)> dfs2 = [&](int u, int p){
        if(p == -1) ver[u] = update(base, 1, n, a[u]);
        else ver[u] = update(ver[p], 1, n, a[u]);
        for(auto v: adj[u]){
            if(v == p) continue;
            dfs2(v, u);
        }
    ...
    f(i, k){
        ...
        int l = getLCA(u, v);
        int cnt = query(ver[u], 1, n, x, x) + query(ver[v], 1, n, x, x) - 2 * query(ver[l], 1, n, x, x) + (a[l]==x);
        cout << (cnt > 0 ? '1' : '0');
    }
}
