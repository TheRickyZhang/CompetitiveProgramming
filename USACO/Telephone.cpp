...
int32_t main() {
    ...
    vvb grid(k, vb(k)); // 1-indexed
    f(i, k) {
        string s; cin>>s;
        f(j, k) if(s[j]=='1') grid[i][j] = true;
    }
    vvpii adj(n*k); // Note that is pos n, breed k. Weight is 1 if dist, else 0

    // Movement edges - keep breed but change pos
    f(i, n-1) {
        f(j, k) {
    ...
    // Transition edges - keep post but change breed
    f(i, n) {
        int a = nums[i];
        f(b, k) {
            if(grid[a][b]) adj[i*k + a].pb({(i*k) + b, 0});
        }
    }
    // cout<<adj<<en;

    // 0-1 BFS - Important! Process 0 nodes first by placing in deque front
    deque<int> q;
    vi dist(n*k, INFL);
    q.push_front(nums[0]); dist[nums[0]] = 0;
    while(!q.empty()) {

        int u = q.front(); q.pop_front();
        for(auto [v, w] : adj[u]) {
            int nw = dist[u] + w;
            if(nw >= dist[v]) continue;
            dist[v] = nw;
    ...
    //     // Iterate forwards
    //     f(j, i) {
    //         if(grid[nums[j]][nums[i]]) ckmn(dp[i], dp[j] + i-j);
    //     }
    //     // Iterate backwards
    //     repr(j, i-1, 0) {
    //         if(grid[nums[i]][nums[j]]) ckmn(dp[j], dp[i] + i-j);
    //     }
    //     // cout<<dp<<en;
    // }
    // if(dp[n-1]==INF) cout<<-1<<en;
    // else cout<<dp[n-1]<<en;
}
