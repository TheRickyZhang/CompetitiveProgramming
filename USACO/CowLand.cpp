...
int32_t main() {
    ...
    BIT<int> bit(n+1, 0LL, [&](int a, int b) {
        return a^b;
    });
    f(u, n) {
        bit.add(start[u], nums[u]);
        bit.add(end[u], nums[u]);
        // cout<<bit<<en;
    }
    ...
    f(i, k) {
        ...
        if(t==1) {
            ...
            // This adds x only to u's subtree
            bit.add(start[u], diff);
            bit.add(end[u], diff);
            nums[u] = x; // Note the properties of XOR for "updating"
            // cout<<"Tree for change: "<<bit<<sp<<x<<sp<<u<<en;
        } else {
            int u, v; cin>>u>>v; u--; v--;
            int p = LCA(u, v);
            // cout<<bit.query(start[u]) <<sp<< bit.query(start[v]) <<sp<< nums[p]<<en;
            int res = bit.query(start[u]) ^ bit.query(start[v]) ^ nums[p]; // Since LCA(u, v) is over counted when combining path-to-root
            cout<<res<<en;
        }
    }
