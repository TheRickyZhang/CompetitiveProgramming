...
void merge(node& a, node& b) {
    if(a.q.size() < b.q.size()) {
        while(!b.q.empty()) {
            auto p = b.q.top(); b.q.pop();
            if(b.s.count(p.ss)) add(a, p);
        }
    } else {
        while(!a.q.empty()) {
            auto p = a.q.top(); a.q.pop();
            if(a.s.count(p.ss)) add(b, p);
        }
        swap(a, b);
    }
}
...
int32_t main() {
    ...
    autotree dfs = [&](int u, int p) {
        for(auto pp : tokens[u]) add(nodes[u], pp);
        auto& x = nodes[u];
        for(int v : adj[u]) {
            if(v==p) continue;
            dfs(v, u);
        ...
        while(!x.q.empty() && !x.s.count(x.q.top().ss)) {
            x.q.pop();
        }
        if(p != -1) res[mp[{p ,u}]] = x.q.empty() ? -1 : x.q.top().ff;
    };
    dfs(0, -1);
    f(i, n-1) cout<<res[i]<<en;
}
