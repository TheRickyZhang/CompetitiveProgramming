...
int32_t main() {
    setIO();
    cin>>n>>k;
    vi a(n); f(i, n) cind>>a[i];
    vi cnt(n, 0);
    map<int, int> mp; // Map to the count -> increasing count of frequencies
    v<set<int>> vals(n+1); // Values that have this frequency
    ...
    // We're passing in x as a value and usually operating on the changing frequency of x!
    ...
    f(_, k) {
        ...
        vi mn(m), mx(m);
        f(i, m) {
            mn[i] = *vals[distinct[i]].begin(), mx[i] = *vals[distinct[i]].rbegin();
        }
        repr(i, m-2, 0) ckmx(mx[i], mx[i+1]);
        int res = 0;
        f(l, m) {
            int r = m-1;
            // Since we did the prefix max we want to have r be the smallest where l+r >= max is satisfied
            // This is the most critical piece of code to understand: because distinct build from set,
            // Values of distinct are monotonic, and taking mx as a suffix allows us to only take O(sz = sqrt(n)) time
            while(r-1 >= 0 && distinct[r-1] + distinct[l] >= distinct.back()) {
                r--;
            }
            ckmx(res, mx[r]-mn[l]);
        }
