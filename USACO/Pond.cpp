...
int main() {
    ...
    f(i,n) f(j,n) cin>>grid[i][j];
    int lim = k*k/2 + 1;

    // Returns whether x could be the median
    auto check = [&](int x) {
        f(i, n) {
            f(j, n) {   // NOTE: this replaces each time so no need ot memory wipe
                pre[i+1][j+1] = pre[i+1][j] + pre[i][j+1] - pre[i][j] + (grid[i][j] > x ? 1 : 0);
            }
        }
        // cout<<"pre for "<<x<<en;
        // cout<<pre<<en;
        f(i, n-k+1) {
            f(j, n-k+1) {
                if(pre[i+k][j+k] -pre[i+k][j] - pre[i][j+k] + pre[i][j] < lim) {
                    // cout<<"square from "<<i<<sp<<j<<", "<<i+k<<sp<<j+k<<" has sum "<<pre[i+k][j+k] -pre[i+k][j] - pre[i][j+k] + pre[i][j]<<en;
                    return true;
                }
            }
        }
