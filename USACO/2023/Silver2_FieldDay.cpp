...
// NOTE THAT you need to do the inverse here - the way bfs xor propogates lends itself to min, not max
int main() {
    ...
    f(i, n) {
        ...
        int curr=0;
        f(j, k) {
            if(s[j]=='G') curr |= 1<<(k-j-1);
        }
        nums[i]=curr;
        dist[nums[i]] = 0;
    }
    f(i, k) { // Need to iterate over i first to not overwrite values
        f(j, 1<<k) {
            dist[j ^ (1<<i)] = min(dist[j ^ (1<<i)], dist[j]+1);
        }
    }
    f(i, n) {
        int flipped = nums[i] ^ (1<<k) - 1;
        cout<<k-dist[flipped]<<en;
    }
}
