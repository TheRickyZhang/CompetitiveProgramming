...
constexpr ll N = 500; ll MOD=1000000007; constexpr int INF=1000000000; constexpr ll INFL=0x3f3f3f3f3f3f3f3f; constexpr auto en = "\n"; constexpr auto sp = " ";
    ...
    for(int a = 1; a<n; ++a) {
        ...
        f(row1, n) {
            ...
            f(row2, n) {
                ...
                if(row1+1 < n && row2-1 >= 0) dp[row1][row2] += prev[row1+1][row2-1];
                dp[row1][row2] %= MOD;
            }
        }
        swap(prev, dp);
