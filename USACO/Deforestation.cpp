...
// REVIEW: Reasoning behind priority queue and points setup - why not coordinate compress?
int t, k, n, m;
void solve() {
    cin>>n>>k;
    vi v(n);
    ...
    f(i, n) {
        int x; cin>>x;
        v[i] = x;
        nums.emplace_back(x, INF, INF);
    }
    sort(all(v));
    f(i, k) {
        int l, r, t; cin>>l>>r>>t;
        int curr = upper_bound(all(v),r)-lower_bound(all(v),l);
        nums.emplace_back(l, r, curr-t);
    }
    sort(all(nums));

    int curr=0;
    priority_queue<pii, vpii, greater<>> q; // restriction, intervals
    for(auto [l, r, d] : nums) {
        if(r !=INF) {
            q.emplace(d+curr, r); // Max trees we can cut at this point
        } else {
            while(!q.empty() && q.top().ss < l) q.pop();
            if(q.empty() || curr != q.top().ff) curr++;
        }
    }
    cout<<curr<<en;
}
