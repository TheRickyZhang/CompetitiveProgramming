...
int32_t main() {
    ...
    str s; cin>>s;
    vvi cost(m, vi(m, 0));
    f(i, m) f(j, m) cin>>cost[i][j];
    f(k, m) f(i, m) f(j, m) ckmn(cost[i][j], cost[i][k] + cost[k][j]);

    // Cost to change
    vvi change(n+1, vi(m, 0));
    f(i, n) {
        f(j, m) change[i+1][j] = change[i][j] + cost[s[i]-'a'][j];
    }
    auto query = [&](int l, int r, int j) {
        return change[r+1][j] - change[l][j];
    };
    // cout<<change<<en;
    vvi dp(n+1, vi(m, INFL));
    vi mn(n+1, INFL);
    mn[0] = 0; // Subtlety: since we must have chucks of k, only set mn[0] (1-indexed) to 0, not any dp[0][x]
    f(i, n) {
        f(j, m) {
            ckmn(dp[i+1][j], dp[i][j] + query(i, i, j));
            if(i-k+1 >= 0) ckmn(dp[i+1][j], mn[i-k+1] + query(i-k+1, i, j));
        }
        f(j, m) ckmn(mn[i+1], dp[i+1][j]);
    }
    // cout<<dp<<en<<mn<<en;
    cout<<mn[n]<<en;
}
