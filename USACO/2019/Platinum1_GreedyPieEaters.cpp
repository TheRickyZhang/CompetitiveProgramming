...
int32_t main() {
    ...
    cin>>n>>m;
    v<vvi> best(n, vvi(n, vi(n, 0))); // best in range [l, r] containing i
    f(i, m) {
        int w, l, r; cin>>w>>l>>r; l--; r--;
        rep(i, l, r) best[i][l][r] = w;
    }
    f(i, n) {
        repr(l, i, 0) {
            rep(r, i, n-1) {
                if(l-1 >= 0) ckmx(best[i][l-1][r], best[i][l][r]);
                if(r+1 < n) ckmx(best[i][l][r+1], best[i][l][r]);
            }
        }
    }
    // f(i, n) cout<<best[i]<<en;
    vvi dp(n+1, vi(n+1, 0));
    for (int i = 0; i < n; ++i) dp[i][i] = best[i][i][i];
    fe(x, n) {
        f(i, n-x+1) {
            int j = i+x-1;
            rep(k, i, j) {
                ckmx(dp[i][j], (k == i ? 0 : dp[i][k-1]) + best[k][i][j] + (k == j ? 0 : dp[k+1][j]));
            }
            for(int k = i; k < j; ++k) {
                ckmx(dp[i][j], dp[i][k] + dp[k+1][j]);
            }
        }
    }
    // cout<<dp<<en;
    ...
    // Not correct - we need to consider every add point
    // for(auto [r, w] : right[i]) {
    //     if(r <= j) ckmx(dp[i][j], w + dp[i+1][j]);
    // }
    // for(auto [l, w] : left[j]) {
    //     if(l >= i) ckmx(dp[i][j], dp[i][j-1] + w);
    // }
}
