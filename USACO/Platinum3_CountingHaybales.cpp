...
int32_t main() {
    ...
    LazySegtree<node, int> tree(n,
        [&](node a, node b) {
            return node(a.x + b.x, min(a.mn, b.mn));
        },
        [&](node a, int x, int l, int r) {
            int len = r-l+1;
            return node(a.x + len * x, a.mn + x);
        },
        [&](int o, int p) {
            return o+p;
        }, a);
    f(i, k) {
        ...
        int l, r; cind>>l>>r;
        if(t == 'M') {
            cout<<tree.query(l, r).mn<<en;
        } else if(t == 'P') {
            int x; cin>>x;
            tree.add(l, r, x);
        } else if(t == 'S') {
            cout<<tree.query(l, r).x<<en;
        }
    }
}
