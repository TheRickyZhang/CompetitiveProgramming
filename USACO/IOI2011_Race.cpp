...
int best_path(int n, int k, int h[][2], int ww[]) {
#define int long long
    vvpii adj(n);
    f(i, n-1) {
        int u = h[i][0], v = h[i][1], w=ww[i];
        adj[u].pb({v, w});
        adj[v].pb({u, w});
    }
    vi dist(n, 0), len(n, 0);
    fn<void(int, int, int, int)> dfs = [&](int u, int p, int d, int l) {
        dist[u] = d; len[u] = l;
        for(auto [v, w] : adj[u]) {
            if(v==p) continue;
            dfs(v, u, d+w, l+1);
    ...
    v<map<int, int>> mps(n);
    int res = INFL;
    fvii populate = [&](int u, int p) {
        mps[u][dist[u]] = len[u];
        for(auto [v, w] : adj[u]) {
            if(v == p) continue;
            populate(v, u);
            if(mps[v].size() > mps[u].size()) swap(mps[u], mps[v]);
            for(auto [d, l] : mps[v]) {
                if(mps[u].count(k + 2*dist[u] - d)) {
                    ckmn(res, l + mps[u][k + 2*dist[u] - d] - 2 * len[u]);
                }
            }
            for(auto [d, l] : mps[v]) {
                if(mps[u].count(d)) ckmn(mps[u][d], l);
                else mps[u][d] = l;
            }
        }
    };
