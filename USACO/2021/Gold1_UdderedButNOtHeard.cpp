...
int main() {
    ...
    int pos = 0;
    for(char c : s) {
        if(!mp.count(c)) mp[c] = pos++;
        freq[mp[c]]++;
    }
    m = mp.size();

    vvi adj(m, vi(m, 0)); // Compare s[i], s[i+1]
    f(i, n-1) adj[mp[s[i]]][mp[s[i+1]]]++;

    vi dp(1<<m, n); // Max value for each state
    dp[0] = 1;
    f(i, 1<<m) {
        f(j, m) { // new letters considering
            // Removing version more intuitive than adding. Works both ways
            if(! (i & (1<<j))) continue;
            int prev = i ^ (1<<j);
            int tot = dp[prev];
            f(k, m) if(i & (1<<k)) tot += adj[j][k];
            ckmn(dp[i], tot);
        }
    }
    // cout<<dp<<en;
