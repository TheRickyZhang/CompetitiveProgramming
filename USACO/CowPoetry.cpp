...
constexpr ll N = 5000; constexpr ll MOD=1000000007; constexpr int INF=1000000000; constexpr ll INFL=0x3f3f3f3f3f3f3f3f; constexpr auto en = "\n"; constexpr auto sp = " ";
namespace Modular {ll operator+(ll a, ll b) { return (a + b) % MOD; } ll operator*(ll a, ll b) { return (a % MOD * b % MOD) % MOD; } }
...
// Go from bottom to top to see motivations behind calculations
int main() {
    setIO("poetry");
    ios::sync_with_stdio(false); cin.tie(nullptr);
    cin>>n>>m>>k;

    vll freq(k+1, 0); // Query with length
    vvll typeFreq(N+1, vll(k+1, 0)); // Query with type, freq

    // f(i) -> how many ways to create line ending with scheme ti
    // dp[i] -> how many ways to create words of length i
    f(i, n) {
        int x, t; cin>>x>>t;
        freq[x]++;
        typeFreq[t][x]++;
    ...
    // Iterate over lengths first (Note the order/direction; bottom up since already calculate before)
    rep(i, 0, k) {
        fe(j, i) {
            dp[i] += dp[i-j] * freq[j];
        }
    ...
    vi endCount(N+1, 0); // # ways to end line size k with
    fe(i, N) {
        f(j, k) { // Iterate over all lengths
             endCount[i] += dp[k-j] * typeFreq[i][j];
        }
    }
    // cout<<endCount<<en;

    // Assigning map rhymes to groups = enumerating combinations of independent choices
    // Same concept as generating divisors of a number based on prime factorization1
    // So, if we have number of ways to have a line ending in e, and total number of syllables in each line,
    ...
    for(auto [c, cnt] : mp){
        ll curr = 0;
        fe(i, N){
            curr = curr + fastPow(endCount[i], cnt);
        }
        res = res * curr;
    }
    cout<<res<<en;
