...
tpl_<tn_ C, tn_ T = enable_if_t<!is_same_v<C, string>, typename C::value_type>> ostream& operator<<(ostream& os, const C& v) { os<<"{"; string sep; for(const T& x : v) os<<sep<<x, sep=", "; return os<<"}";}
 map<char, int> dirMap={{'E',0},{'S',1},{'N',2},{'W',3}}; auto check=[](auto y,auto x,auto m,auto n){return y>=0&&y<m&&x>=0&&x<n;};
...
int32_t main() {
    ...
    f(i, n) {
        f(j, n) {
            // Only go over positive dirs to prevent duplicates
            int ni = i+1, nj = j;
            if(check(ni, nj, n, n)) {
                edges.pb({abs(a[i][j] - a[ni][nj]), to(i, j), to(ni, nj)});
            }
            ni = i, nj = j+1;
            if(check(ni, nj, n, n)) {
                edges.pb({abs(a[i][j] - a[ni][nj]), to(i, j), to(ni, nj)});
    ...
    for(auto v : edges) {
        d.merge(v[1], v[2]);
        if(d.sz[d.par(v[1])] >= goal) {
            cout<<v[0]<<en; return 0;
        }
    }
    cout<<-1<<en;
}
