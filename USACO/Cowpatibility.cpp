...
int32_t main() {
    ...
    cin>>n;
    vvi flavor(N+1);
    v<B> adj(50000); // Bitset of matching flavor cows
    int res = 0;
    f(i, n) {
        f(j, 5) {
            int x; cin>>x; flavor[x].pb(i);
        }
    }
    fe(f, N) {
        if(flavor[f].empty()) continue;
        B b;
        for(int c : flavor[f]) b[c]=true;
        for(int c : flavor[f]) adj[c] |= b;
    }
    f(i, n) res += adj[i].count();
    cout<<(n*n - res)/2<<en;
}
