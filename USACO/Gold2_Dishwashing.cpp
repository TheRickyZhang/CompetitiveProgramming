...
int32_t main() {
    ...
    vi a(n);
    f(i, n) cin>>a[i];
    // Very clever stack problem
    vi id(N+1, 0); // This is the stack id that we will put it on
    v<stack<int>> stacks(N+1); // Holds the stack, where s[id[i]] = { -> ...
    int curr = 0, res = n;
    f(i, n) {
        int x = a[i];
        // This check is very clever. We don't necessarily have to set curr
        // In order BUT once are forced to, any < will mean that there is no way to
        // Arrange stacks
        if(x < curr) {
            res = i; break;
        }
        repr(j, x, 1) {
            if(id[j] != 0) break;
            id[j] = x; // Any j <= x gets directed to the x stack; easy way to designate
        }
        auto& s = stacks[id[x]];
