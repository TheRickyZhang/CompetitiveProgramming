...
int32_t main() {
    ...
    cin>>n>>k;
    vi xx, yy;
    v<array<int, 4>> points;
    f(i, k) {
        int a, b, c, d; cin>>a>>b>>c>>d;
        points.pb({a, b, 1, i});
        points.pb({c, d, 0, i}); // Handle endings earlier in processing
        xx.pb(a); xx.pb(c);
        yy.pb(b); yy.pb(d);
    }
    // Ensure the final points are accounted for
    points.pb({n, n, 1, k});
    xx.pb(n); yy.pb(n);
    ...
    f(i, n) xmp[xx[i]] = i;
    f(i, m) ymp[yy[i]] = i;
    for(auto& [x, y, t, i] : points) {
        x = xmp[x], y = ymp[y];
    }
    sort(all(points));
    Segtree<int> tree(m, [&](int a, int b) {
        return min(a, b);
    }, {}, INFL);
    vi res(k+1, INFL);
    for(auto [x, y, t, i] : points) {
        if(t == 0) {
            tree.add(y, res[i] - xx[x] - yy[y]);
        } else {
            res[i] = xx[x] + yy[y] + min(0LL, tree.query(0, y)); // Note this should always be a valid ckmx
        }
    }
    // cout<<res<<en;
    cout<<res[k]<<en;
