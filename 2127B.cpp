...
void solve() {
    ...
    // cout<<l<<sp<<r<<en;
    if(l == -1 && r == -1) quit(1);
    if(l == -1 || l < r) quit(min(k+1, r));
    if(r == -1 || r <= l) quit(min(n-k, l));
    // cout<<"HUH"<<en;
    // vi pre(n+1, 0);
    // f(i, n) pre[i+1] = pre[i] + s[i]=='#';
    // auto query = [&](int l, int r) {
    //     cmn(r, n-1); cmx(l, 0LL);
    //     return pre[r+1] - pre[l];
    // };
    // int res = fstTrue(1LL, min(k+1, n-k+1), [&](int x) {
    //     return (x-1) - query(k-x+1, k-1) + (x-1) - query(k+1, k+x-1) <= k;
    // });
    // cout<<res<<en;
}
