...
int main() {
    ...
    f(i, m) cin>>b[i].ff>>b[i].ss;
    auto p = [&](ll x) {
        return x*x;
    };
    auto dist = [&](pll& x, pll& y) {
        return p(x.ff-y.ff) + p(x.ss-y.ss);
    };
    // idx, idx, currently at
    v<vvll> dp(n+1, vvll(m+1, vll(2, INFL)));
    dp[1][0][0] = 0; // Must start at a
    rep(i, 0, n) {
        rep(j, 0, m) {
            if(i-2>=0) chkmn(dp[i][j][0], dp[i-1][j][0] + dist(a[i-1], a[i-2]));
            if(j-2>=0) chkmn(dp[i][j][1], dp[i][j-1][1] + dist(b[j-1], b[j-2]));

            if(i>0 && j>0) {
                chkmn(dp[i][j][0], dp[i-1][j][1] + dist(b[j-1], a[i-1]));
                chkmn(dp[i][j][1], dp[i][j-1][0] + dist(a[i-1], b[j-1]));
            }
        }
    }
    cout<<dp[n][m][0]<<en;
