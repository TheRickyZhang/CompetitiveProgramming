...
int32_t main() {
    ...
    // Think carefully about WHY we need these
    // For the basic small to large distinct colors, we have a set because counting total colors present -> Only use set.size();
    // Dominating color -> need to know frequency of each color and sum of frequencies (for metric of merging small to large)

    // {root u, color} -> freq
    v<map<int, int>> freq(n);
    // {root u, freq} -> sum
    v<map<int, int>> sum(n);
    vi res(n, 0); // Sum of the dominating colors
    ...
    function<void(int, int)> dfs = [&](int u, int p) {
        for(int v : adj[u]) {
            if(v==p) continue;
            dfs(v, u);
            // Still swap using size of map (ie improved set)
            ...
            // Insertion
            for(auto [c, f] : freq[v]) {
                // Update the sum since the old freq will be changing (see parity with last statement)
                if(freq[u].count(c)) sum[u][freq[u][c]] -= c;
                freq[u][c] += f;
                sum[u][freq[u][c]] += c;
            }
        }
