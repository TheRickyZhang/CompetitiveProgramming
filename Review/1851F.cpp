...
// Note: No !it or end checks since always fixed length
struct node {
...
void solve() {
    ...
    ll mn=INFL, a=-1, b=-1;
    Trie t; t.insert(nums[0], 0);

    fe(i, n-1) {
        auto [closest, j] = t.findClosest(nums[i]);
        ll minXor = closest ^ nums[i];
        if(minXor < mn) {
            a=j, b=i; mn = minXor;
        }
        t.insert(nums[i], i);
    ...
    ll res=0;
    f(i, k) {
        ll x=nums[a]>>i & 1, y=nums[b]>>i & 1;
        if(x==y) {
            res |= (1-x)<<i;
        } // else: doesn't matter since won't affect xnor value. Choose 0 arbitrarily
    }
    cout<<a+1<<sp<<b+1<<sp<<res<<en;
}

int main() {
    ios::sync_with_stdio(false); cin.tie(nullptr);
    ll t; cin>>t; f(i, t) solve();
}
