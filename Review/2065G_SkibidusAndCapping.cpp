...
void solve() {
    ...
    vvi factors(n+1);
    for(int i = 2; i<=n; ++i) { // NOTE this excludes 1 and n from the factor count
        for(int j = 2*i; j<=n; j += i) {
            factors[j].pb(i);
        }
    }
    int sump=0, sums=0; // Total counts for primes and semiprimes
    map<int, int> p, pp, pq; // primes, p^2, p * q
    for(int x : nums) {
        ...
        if(sz==0) {
            ...
            sums++; pp[x]++;
        } else if(sz==2 && factors[factors[x][1]].empty()) {
            sums++; pq[x]++;
        }
    }
    // cout<<p<<en<<pp<<en<<pq<<en;

    int res = 0;
    for(auto [x, f] : p) res += f * (sump - f);
    res /= 2; // NOTE: it is important to do this division after, not before
    ...
    for(auto [x, f] : pq) {
        res += f * (f+1) / 2;
        res += f * (p[factors[x][0]] + p[factors[x][1]]); // Sum up the pairings for p, q
    }
    cout<<res<<en;
}
