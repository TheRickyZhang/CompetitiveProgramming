...
// IMPORTANT: View this foundational logic
// int32_t main() {
//     ios::sync_with_stdio(false); cin.tie(nullptr);
//     cin>>n>>k;
//     int m = sqrt(n);
//     vi types(n); f(i, n) cin>>types[i];
//     vi nums(n);
//     f(i, n) {
//         cin>>nums[i];
//         if(types[i]==2) nums[i] = -nums[i];
//     }
//     vi a(n+1, 0);
//     f(i, n) a[i+1] = a[i] + nums[i];
//     vi b = a;
//
//     map<int, int> mp;
...
//     f(i, q) {
//         int l, r; cin>>l>>r; l--;
//         qq[i] = {l, r, i}; // NOTE that we keep l, r intact and perform Mo's using the sorting
//     }
//     sort(all(qq), [&](iii& a, iii& b) {
//         int x = get<0>(a)/m, y = get<0>(b)/m;
//         if(x == y) return get<1>(a) < get<1>(b);
//         return x < y;
//     });
//
//     int cl = 0, cr = 0;
//     vi freq(sz, 0); freq[b[0]] = 1;
//     int curr = 0;
//     vi res(q);
//     for(auto [l, r, i] : qq) {
//         // Note the order: we have r-l = k;
//         // So for dealing with r, we count number of l=r-k
//         // And for l, we count number of r = l+k
//         // If expanding the window, we don't want to include current, so update cnt, then freq
//         // If contracting the window, we don't want to include current, so update freq first, then cnt
//
//         while(cr < r) {
//             cr++;
//             curr += freq[mp[a[cr] - k]];
