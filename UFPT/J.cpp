...
void readg(vvi &g,int m,bool o=true,bool d=false){f(i, m){int u,v;cin>>u>>v;if(o){u--;v--;}g[u].pb(v);if(!d)g[v].pb(u);}}
void readg(vvpii &g,int m,bool dec=true,bool dir=false){f(i, m) {
...
constexpr int TOT_CHARS = 61;
constexpr int MAX_BASE = 7500;
...
int eval(const string& s, int base) {
  int res = 0;
  for(char c : s) {
    assert(mp.count(c));
    res *= base;
...
void solve() {
  str s, t; cin >> s >> t;
  int s_min = minBase(s), t_min = minBase(t);  
  ump<int, int> mp;
  rep(i, s_min, MAX_BASE) {
    mp[eval(s, i)] = i;
  }
  rep(i, t_min, MAX_BASE) {
    int y = eval(t, i);
    if(mp.count(y)) {
      cout << y << sp << mp[y] << sp << i << en;
      return;
    }
  }
  cout << "CANNOT MAKE EQUAL" << en;
}
